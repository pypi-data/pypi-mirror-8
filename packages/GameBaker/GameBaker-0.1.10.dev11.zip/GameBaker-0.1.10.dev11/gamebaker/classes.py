...
class Grid:
    ...
    def select_region(self, x, y, width, height):
        ...
        """
        left = x//self.sector_size
        top = y//self.sector_size
        right = left + (width//sector_size) + 1
        bottom = top + (height//sector_size) + 1
        return list(set.union(self.array[y, x] for y in range(top, bottom) for x in range(left, right)))       
