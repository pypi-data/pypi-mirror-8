...
def get_active_stuff(objects, views, scenery):
    instances = []
    flag = False
    for index, instance in enumerate(objects):
        if any(v.left <= instance.x <= v.right for v in views):
            flag = True
            if any(v.top <= instance.y <= v.bottom for v in views):
    ...
    scenery = []
    flag = False
    for index, instance in enumerate(scenery):
        if any(v.left <= instance.x <= v.right for v in views):
            flag = True
            if any(v.top <= instance.y <= v.bottom for v in views):
                scenery.append(instance)
        elif flag:
            break

    return set(instances), tuple(scenery)
    
def sort_stuff(active_instances, active_scenery):
    return sorted(chain(active_instances, active_scenery), key=attrgetter("draw_depth"))
...
def main(events, views, objects, scenery, settings, blueprints):
    ...
    possible_collisions = set(blueprints.possible_collisions) | set(c[::-1] for c in blueprints.possible_collisions)
    ...
    active_check = settings.active_check
    ...
    while True:
        ...
        x_sorted = sorted(objects, key=attrgetter("x"))
        
        check_get_active += 1
        if check_get_active >= active_check:
            active_instances, active_scenery = get_active_stuff(x_sorted, views, scenery)
            check_get_active = 0
        ...
        collision_list = [i for i in x_sorted if i in active_instances]
        ...
        for index, instance in enumerate(collision_list):
            ...
            for second_instance in collision_list[index+1:]:
                if temp_x + bbw < second_instance.x:
                    break
                elif ((type(instance), type(second_instance)) in possible_collisions) and instance.get_rect().colliderect(second_instance.get_rect()):
