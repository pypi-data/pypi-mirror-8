...
class Auth():
    ...
    def connect(self, uid):
        https_con = HTTPSClientAuthHandler(
            os.path.join(self.directory, '/private_key.pem'),
            os.path.join(self.directory, '/cert.pem')
        )
        opener = request.build_opener(https_con)
        url_params = "?client_id=%s&client_secret=%s&uid=%s" \
            % self.client_id, self.client_secret, uid
        response = opener.open(path.join(self.url,  url_params))
