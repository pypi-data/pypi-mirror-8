...
class Database:
	...
	def Update(self, table, olddata, *args, **kwargs):
		...
		for x in olddata:
			...
			for arg in args:
				x[arg[0]] = arg[1]
			self._runTrigger("BEFORE UPDATE",table,x,dict(itertools.chain(**kwargs.items(), temp.items())))
			...
			self._runTrigger("AFTER UPDATE",table,self.master[table][self.master[table].index(x)],dict(itertools.chain(**kwargs.items(), temp.items())))
		self._write()
	...
	def Insert(self, table, *args, **kwargs):
		...
		for arg in args:
			temp[arg[0]] = arg[1]
		self._runTrigger("BEFORE INSERT",table,[args],dict(itertools.chain(**kwargs.items(), temp.items())))
		self.master[table].append(dict(itertools.chain(**kwargs.items(), temp.items())))
		self._runTrigger("AFTER INSERT",table,[args],dict(itertools.chain(**kwargs.items(), temp.items())))
		self._write()
