...
class Database:
	...
	def Update(self, table, olddata, *args, **kwargs):
		...
		if not self.TableExists(table):
			raise TableDoesNotExistException
		for x in olddata:
			self._runTrigger("BEFORE UPDATE",table,x,[args],dict(**kwargs))
			...
			self._runTrigger("AFTER UPDATE",table,self.master[table][self.master[table].index(x)],[args],dict(**kwargs))
		self._write()
	...
	def Insert(self, table, *args, **kwargs):
		...
		if not self.TableExists(table):
			raise TableDoesNotExistException
		self._runTrigger("BEFORE INSERT",table,[args],dict(**kwargs))
		temp = {}
		for arg in args:
			temp[arg[0]] = arg[1]
		self.master[table].append(dict(**kwargs).append(temp))
		self._runTrigger("AFTER INSERT",table,[args],dict(**kwargs))
		self._write()
