        ...
        for service in config.get_services().values():
            ...
            if not service.is_running():
                ...
                if not service.wait is False:
                    ...
                    event_received = yield self.event_bus.wait_for_event('api.%s.*' % service.name, wait)
                    ...
                    if timeout_happenned:
                        ...
                        self.task_log(ticket_id, 'Container is waiting 0.5s to make sure container is started.')
                        yield sleep(0.5)
