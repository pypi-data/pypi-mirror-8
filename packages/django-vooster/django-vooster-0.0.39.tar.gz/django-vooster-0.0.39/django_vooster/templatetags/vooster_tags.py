...
def scripts_output(context):
	...
	if hasattr(request, 'script_config'):
		request.scripts.append("""(function(){if (window.root === undefined) window.root = {}; if (window.root.config === undefined) window.root.config = {}; var additionalConfig = %s; for (k in additionalConfig) window.root.config[k] = additionalConfig[k];})();""" % json.dumps(request.script_config))
	return mark_safe("".join([
