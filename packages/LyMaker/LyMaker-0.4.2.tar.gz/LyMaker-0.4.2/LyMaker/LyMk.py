...
class LyMaker(object):
    ...
    def _addScorePart(self,instanceCount,harmonies,perc=0):
            ...
            if self.synthMode == 0:
                ...
                text += "SynthI = \\transpose c c' {\n\\clef treble\n\\global\n\\Key \n" 
                i = 0
                while i < instanceCount:
                    text += "\\SynthI_%s   " % instances[i]
                    i += 1
            text += "\n}\n"    
            ...
            if self.synthMode == 0:
                ...
                text += "SynthIII = \\transpose c c'' {\n\\clef treble\n\\global\n\\Key\n"
                i = 0
                while i < instanceCount:
                    text += "\\SynthIII_%s   " % instances[i]
                    i += 1
                text += "\n}\n"    
                text += "SynthIV = {\n\\clef bass\n\\global\n\\Key\n"
                i = 0
                while i < instanceCount:
                    text += "\\SynthIV_%s   " % instances[i]
                    i += 1
                text += "\n}\n"    

                text += "SynthII = \\transpose c c' {\n\\clef treble\n\\global\n\\Key\n" 
                i = 0
                while i < instanceCount:
                    text += "\\SynthII_%s   " % instances[i]
                    i += 1
                text += "\n}\n"    
