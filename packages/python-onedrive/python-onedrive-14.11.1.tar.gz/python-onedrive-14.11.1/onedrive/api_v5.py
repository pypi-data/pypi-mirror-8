...
class OneDriveAuth(OneDriveHTTPClient):
	...
	def _auth_token_request(self):
		post_data = dict(client_id=self.client_id,
						client_secret=self.client_secret, #!as1
						redirect_uri=self.auth_redirect_uri) #!as1
		if not self.auth_refresh_token:
			...
			post_data.update(code=self.auth_code,
							grant_type='authorization_code') #!as1
			...
			post_data.update(refresh_token=self.auth_refresh_token,
							grant_type='refresh_token') #!as1

		post_data_missing_keys = list(k for k in ['client_id', 'client_secret',
												'code', 'refresh_token', #!as2
												'grant_type'] #!as2
									if k in post_data and not post_data[k]) #!as2
		if post_data_missing_keys:
			raise AuthenticationError('Insufficient authentication'
									' data provided (missing keys: {})' #!as2
									.format(post_data_missing_keys)) #!as2
...
class OneDriveAPIWrapper(OneDriveAuth):
	...
	def _api_url(self, path, query=dict(),
				pass_access_token=True, pass_empty_values=False): #!as1
		query = query.copy()
	...
	def _translate_api_flag(self, val, name=None, special_vals=None):
		...
		flag_val_dict = {None: None, 'false': 'false',
						False: 'false', 'true': 'true', True: 'true'} #!as1
		try:
	...
	def __call__(self, url='me/skydrive', query=dict(), query_filter=True,
		...
		api_url = ft.partial(self._api_url,
							url, query, pass_access_token=not auth_header) #!as1
		try:
	...
	def put(self, path_or_tuple, folder_id='me/skydrive', overwrite=None, downsize=None):
		"""Upload a file (object), possibly overwriting (default behavior)
			...
			of "(name, file)", where "file" can be either a file-like object #!as1
			or just a string of bytes. #!as1

			overwrite option can be set to False to allow two identically-named
			files or "ChooseNewName" to let OneDrive derive some similar #!as1
			unique name. Behavior of this option mimics underlying API. #!as1
	...
	def link(self, obj_id, link_type='shared_read_link'):
		...
		link_type can be either "embed" (returns html), "shared_read_link" #!as3
			or "shared_edit_link"."""
	...
	def copy(self, obj_id, folder_id, move=False):
		...
		Folders cannot be copied; this is an API limitation.""" #!as3
		return self(obj_id,
...
class OneDriveAPI(OneDriveAPIWrapper):
	...
	def resolve_path(self, path,
					root_id='me/skydrive', objects=False): #!as1
		...
		Requires many calls to resolve each name in path, so use with care. #!as3
		...
		if path:
			if isinstance(path, types.StringTypes):
				if not path.startswith('me/skydrive'):
					...
					path = filter(None,
								it.chain.from_iterable(p.split('\\') for p in path.split('/'))) #!as2
				else:
