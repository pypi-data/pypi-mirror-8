...
#
#  @Update Date: 2014/09/28
#
#   Version 1.7.2, change logs:
#
#     * Add option --with-extension in order to support cross publish.
#
#     * Implement command "capsule" and add option --with-capsule so
#       that we can encrypt scripts with same capsule.
#
#     * Remove command 'convert'
#
#     * Remove option '-K/--key' in the online help
#
import fnmatch
import getopt
import gettext
...
Support URL: http://www.dashingsoft.com/products/pyarmor.html

'''.format(__version__)

__helpfooter__ = '''
Report (and track progress on fixing) bugs via the web page
at http://www.dashingsoft/products/pyarmor.html or, by
sending email to <jondy.zhao@gmail.com>.
'''

__errormsg__ = '''
There is no valid license to import pytransform.

Download trial license from

  http://dashingsoft.com/products/pyarmor/license.lic

Put it into the same path of pytransform extension. By this trial license,
you can use pyarmor by the end of this quarter. In case the trial
license doesn't work, please download the latest pyarmor and check the
change log. The verify algorithm of trial license may be changed in later.

For more information, visit http://dashingsoft.com/products/pyarmor.html
'''
...
def import_pytransform():
    ...
    logging.info('search pytransform extension "%s"' % src)
...
'''))
            ...
            print(_('''
Usage: pyarmor encrypt [OPTIONS] [File Patterns]
...
def format_extension_filename():
    ...
    src = os.path.join(
        sys.rootdir,
        'extensions',
        'pytransform-%s.%s-%s-py%s.%s' % (
            __version__, osname, arch, pyver, ext
            )
        )
    return target, src
...
def make_capsule(rootdir=None, kv=None, filename='project.zip'):
    ...
    kv             32 Bytes as encrypt key, which used to encrypt scripts.
    ...
    try:
        ...
        if kv is not None:
            myzip.writestr('module.key', pytransform.generate_module_key(pubx, kv))
    finally:
        myzip.close()
...
def get_kv(value):
    if os.path.exists(value):
        ...
        f.close()
    return unhexlify(value.replace(' ', ''))
...
def do_encrypt(argv):
    try:
        opts, args = getopt.getopt(
            argv,
            'C:K:O:S:',
            ['key=', 'output=', 'with-capsule=', 'with-extension=']
            )
    ...
    for o, a in opts:
        if o in ('-K', '--key'):
            kv = get_kv(a)
            if not (len(kv) == 32):
                logging.error(_('key size must be 32'))
                usage('encrypt')
                sys.exit(2)
        elif o in ('-O', '--output'):
            output = a
    ...
    if capsule is None:
        ...
        make_capsule(sys.rootdir, kv, filename)
