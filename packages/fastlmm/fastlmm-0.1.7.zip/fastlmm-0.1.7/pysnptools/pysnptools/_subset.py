...
class _Subset(SnpReader):
    ...
    def __repr__(self):
        s = "{0}[{1},{2}]".format(self._internal,self.nice_string(self._iid_indexer),self.nice_string(self._sid_indexer))
        return s
    ...
    def nice_string(self, some_slice):
    ...
    def read(self, order='F', dtype=SP.float64, force_python_only=False, view_ok=False):
        if view_ok and _Subset._is_all_slice(self._iid_indexer) and _Subset._is_all_slice(self._sid_indexer): #!!!cmk04072014 regression test
            return self
        else:
    ...
    def _read(self, iid_index_or_none, sid_index_or_none, order, dtype, force_python_only):
        ...
        val = self._internal._read(composed_iid_index_or_none, composed_sid_index_or_none, order, dtype, force_python_only)
