...
class LMM(object):
    ...
    def setG(self, G0=None, G1=None, a2=0.0, K0=None,K1=None):
        ...
        If k0+k1>>N and similiar kernels are used repeatedly, it is beneficial to precompute
    ...
    def innerLoopTwoKernel(self, a2 = 0.5, min_log_delta=-5, max_log_delta=10, nGrid_log_delta=10, **kwargs):
        '''
        ...
        self.setX(self.X)
        self.sety(self.y)
        return self.find_log_delta(len(self.y), min_log_delta=min_log_delta, max_log_delta=max_log_delta, nGrid=nGrid_log_delta, **kwargs)


    def findA2(self, nGridA2=10, minA2=0.0, maxA2=1.0, min_log_delta=-5, max_log_delta=10, nGrid_log_delta=10, verbose=False, **kwargs):
        '''
        ...
        self.numcalls=0
        resmin=[None]
        def f(x,resmin=resmin, min_log_delta=min_log_delta, max_log_delta=max_log_delta, nGrid_log_delta=nGrid_log_delta,**kwargs):
            self.numcalls+=1
            t0=time.time()
            res = self.innerLoopTwoKernel(a2=x, min_log_delta=min_log_delta, max_log_delta=max_log_delta, nGrid_log_delta=nGrid_log_delta,**kwargs)
    ...
    def find_log_delta_chris(self, n_grid_log_delta=10, min_log_delta = -5.0, max_log_delta=10.0, brent=False, delta_values=None, **kwargs):
        '''
        Find the optimal h2 for a given K. Note that this is the single kernel case. So there is no a2.
        (default maxH2 value is set to a value smaller than 1 to avoid loss of positive definiteness of the final model covariance)
        --------------------------------------------------------------------------
        ...
        resmin = [None]
        def f(x):
            myres = self.nLLeval(delta=x, REML=True)
        ...
        if delta_values is None:
            delta_values = NP.logspace(min_log_delta, max_log_delta, num=n_grid_log_delta, endpoint=True, base=NP.exp(1))
        res = mingrid.minimize1D(f, evalgrid=delta_values, brent=False)
        return resmin[0]
    ...
    def nLLeval_test(self, y_test, beta, h2=0.0, logdelta=None, delta=None, sigma2=1.0, Kstar_star=None):
        ...
        #TODO: verify that scale is not needed to compute mu
        mu = self.predictMean(beta, h2=h2, logdelta=logdelta, delta=delta) #, scale)
        res = y_test - mu
        ...
        if False:
            ...
            NP.testing.assert_array_almost_equal(res_sig, res_sig2, decimal=2)
            NP.testing.assert_array_almost_equal(res_sig, res_sig3, decimal=2)
            NP.testing.assert_almost_equal(logdetK, logdetK2, decimal=1)
        ...
        #print "largest eigv:", SS[-1], "second largest eigv:", SS[-2], "smallest eigv:", SS[0]
        #print "ratio 1large/2large:", SS[-1]/SS[-2], "ratio lrg/small:", SS[-1]/SS[0]
        
        neg_log_likelihood = -(term1 + term2 + term3)

        #print "LL", neg_log_likelihood
