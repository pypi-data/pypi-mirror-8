"""
example of how to use feature selection from python using .dat files (see also command line interface)
"""

import numpy as np
from fastlmm.feature_selection import FeatureSelectionStrategy
from fastlmm.pyplink.snpreader.Dat import Dat
...
def runselect(bed_fn=None, pheno_fn=None, strategy=None, select_by_ll=True, output_prefix=None,num_pcs=0, random_state=3, num_snps_in_memory=1000, cov_fn=None, k_values=None, delta_values=None,num_folds=10,penalty=0.0):
    ...
    ##############################
    if bed_fn is None:
        bed_fn = Dat(r"\\erg00\genetics\dbgap\aric\autosomes-500pcs.dat")
        pheno_fn = r"\\erg00\genetics\dbgap\aric\autosomes.txt"
