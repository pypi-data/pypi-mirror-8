...
class TestTwoKernelFeatureSelection(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.snp_fn = currentFolder + "/examples/toydata"
        ...
        pheno = plink.loadOnePhen(pheno_fn)
        cov = plink.loadPhen(cov_fn)
        
        # intersect sample ids
        snp_reader, pheno, cov = pysnptools.pysnptools.util.util.intersect_apply([snp_reader, pheno, cov])
    ...
    def test_regression_lmm(self):
        ...
        # invoke fs
        select = FeatureSelectionInSample(max_log_k=7, order_by_lmm=True, measure="mse")
    ...
    def test_regression_lr(self):
        ...
        # invoke fs
        select = FeatureSelectionInSample(max_log_k=7, order_by_lmm=False, measure="mse")
        best_k, feat_idx, best_mix, best_delta = select.run_select(self.G, self.G, self.y, cov=self.G_cov)    
...
class TestFeatureSelection(unittest.TestCase):
    ...
    def reference_path():
        ...
        if "Windows" in os_string:
            ...
            return file_path + '/expected-debian/'+outfile
...
def core_run(snpreader, pheno_fn, k, delta):
    ...
    for split_idx, (train_idx, test_idx) in enumerate(kf):
        ...
        #out)
        ll[split_idx] = model.nLLeval_test(fold_data["y_test"], res["beta"], scale=res["sigma2"], delta=delta)
...
if __name__ == '__main__':
    #TestTwoKernelFeatureSelection().test_regression_lmm()
    unittest.main()
