...
class FeatureSelectionStrategy(object):
    ...
    def __init__(self, snpreader, pheno_fn, num_folds, test_size=0.1, cov_fn=None, num_snps_in_memory=100000,
        ...
        #!!!cmk04072014 test speed of new vs old
        #!!!cmk04072014 make all readers take optional file extension
    ...
    def load_covariates(self, pheno):
        if self.cov_fn == None:
            ...
            X = cov['vals']
            cov_iid = cov['iid']
            # add bias column if not present - #!!!cmk05192014 LATER -- Bug? should this test be done after intersection in case removing an iid makes it constant?
    ...
    def load_data(self):
        ...
        self.ind_iid = pheno['iid'] #!!!cmk05192014 LATER: bug? It looks like we record the pre-intersect iids only to write out the pcs later? Why?
        ...
        #!!!cmk05192014 LATER Should we give preference to self.G since reording it is the most expensive?
        ...
        if strategy == "lmm_full_cv":
            ...
            if create_pdf and (output_prefix != None):
                ...
                try:
                    ...
                    if k_values[0] != 0: logging.warn("Expect the first k value to be zero") #!!!cmk04072014 move this change earlier
...
class InMemory(GClass):
    ...
    def val(self):
        if self._val is None:
            self._snpreader = self._snpreader.read(order='C') #!!!cmk05192014 LATER when should this be order='F' and when order='C'?
