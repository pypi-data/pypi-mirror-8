...
class TestEpistasis(unittest.TestCase):
    ...
    def test_match_cpp(self):
        ...
        snps = Bed(os.path.join(self.pythonpath, "doc/FastLmm-SELECT/demo/snps"))
        phen = os.path.join(self.pythonpath, "doc/FastLmm-SELECT/demo/pheno.txt")
        cov = os.path.join(self.pythonpath, "doc/FastLmm-SELECT/demo/covariate.txt")
        ...
        sid0,sid1,pvalue_list = epistasis(snps[:,test_idx], phen,cov=cov, G0 = snps[:,sim_idx],log_delta=0)
    ...
    def test_one(self):
        ...
        sid0,sid1,pvalue_list = epistasis(test_snps, phen,
                                  cov=cov, 
    ...
    def test_preload_files(self):
        ...
        phen = plink.loadOnePhen(self.phen_fn,vectorize=True)
    ...
    def test_no_cov_b(self):
        ...
        cov = plink.loadPhen(self.cov_fn)
        cov['vals'] = np.delete(cov['vals'], np.s_[:],1) #Remove all the columns
    ...
    #    sid0,sid1,pvalue_list = epistasis(snps[:,test_idx], phen,cov=cov, G0 = snps[:,sim_idx],log_delta=np.log(1),REML=True, G1=cov, mixing=.5,output_file=output_file)
    ...
    def test_unknown_sid(self):
        ...
        try:
            sid0,sid1,pvalue_list = epistasis(test_snps, phen,cov=cov,sid_list_0=['1_4','bogus sid','1_9'],sid_list_1=test_snps.sid[5:15]) #Skip 5 snps, use next 10
            failed = False
        except:
            failed = True
    ...
    def test_cid_intersect(self):
        ...
        test_snps = Bed(self.bedbase)
        phen = plink.loadOnePhen(self.phen_fn,vectorize=True)
        phen['iid'] = np.vstack([phen['iid'][::-1],[['Bogus','Bogus']]])
        phen['vals'] = np.hstack([phen['vals'][::-1],[-34343]])
    ...
    def test_doctest(self):
        import fastlmm.association.epistasis
        doctest.testmod(fastlmm.association.epistasis)
