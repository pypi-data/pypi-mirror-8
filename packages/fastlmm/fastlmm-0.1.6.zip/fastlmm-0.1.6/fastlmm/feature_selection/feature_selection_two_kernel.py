...
class FeatureSelectionInSample(object):
    ...
    def __init__(self, n_folds=5, measure="mse", max_log_k=10, order_by_lmm=False, random_state=None):
    ...
    def run_select(self, G0, G_bg, y, cov=None):
        ...
        G_bg = 1./np.sqrt(G_bg.shape[1]) * G_bg
        ...
        for kfold_idx, (train_idx, test_idx) in enumerate(KFold(num_ind, n_folds=self.n_folds, random_state=self.random_state, shuffle=True)):
            ...
            for k_idx, max_k in enumerate(self.grid_k):
                ...
                #G_fs_train = 1./np.sqrt(max_k) * G0_train[:,feat_idx[0:max_k]]
                #G_fs_test = 1./np.sqrt(max_k) * G0_test[:,feat_idx[0:max_k]]
                ...
                norm_factor = 1.0 / np.sqrt(max_k) #1./np.sqrt((G_fs_train**2).sum() / float(G_fs_train.shape[0]))
                ...
                # do final prediction using old code
                ...
                # we take an additional step to estimate betas (not given from new model)
                ...
                if self.measure != "mse":
                    K_test_test = (1.0-a2) * G_fs_test.dot(G_fs_test.T) + a2 * K_bg_test 
                    ll = lmm.nLLeval_test(y_test, res["beta"], scale=res["sigma2"], delta=delta, Kstar_star=K_test_test)
