...
from hgross.ac.ac_server import ACDedicatedServer, SERVER_WORKING_DIR_FILENAME_PREFIX, SERVER_PIDFILE_NAME, SERVER_PRESET_INDICATOR_NAME
...
def getStatusForServers(wrapperConfig):
    ...
    try:
        ...
        def is_running(working_dir):
            "checks wether the server is running by it's PID file"
...
def main_func():
    ...
    argParser.add_argument('--wrapper-config-file', default="ACServerWrapperConfig.ini", help='This wrapper\'s config file path.')
    args = argParser.parse_args()
    command = args.command

    if not os.path.isfile(args.wrapper_config_file):
        print (u"Error: Config file does not exist (%s).\nUse --wrapper-config-file path/to/your/ACWrapperConfig.ini to specify the correct path." % args.wrapper_config_file)
        sys.exit(1)

    wrapperConfig = ACServerWrapperConfig(args.wrapper_config_file)
