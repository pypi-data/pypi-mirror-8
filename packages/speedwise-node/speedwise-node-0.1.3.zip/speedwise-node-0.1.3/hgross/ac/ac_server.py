    ...
    def run(self):
        ...
        while self.started:
            if last_time + self.keep_alive_interval < time.time():
                result = self.client.send_keep_alive()
...
class ACServerStdoutProcessingThread(threading.Thread):
    ...
    def run(self):
        # start keep alive
        ka_sender = KeepAliveSender(self.speedwise_client)
        ka_sender.start()
