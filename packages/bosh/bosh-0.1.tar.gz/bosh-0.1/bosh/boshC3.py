...
class C3(BaseHtml,object):
     def __init__(self,jsonData,chartType="bar"):
           self._strHTML=self._genHTML(self.checkQuote(jsonData),chartType) 
     ...
     def _loadJSDocReady(self,data,**kwargs):
           ...
           for bindID,chartType in kwargs.items():
                  strDocReady += 'bo.draw_LocalData(data,"{}","#{}");\n'.format(chartType,bindID)       
                  bindID
     ...
     def _insertJSCSSlib(self):
           ...
           strJSCSS+=self._insertOneline("""<script src="http://cdn.rawgit.com/masayuki0812/c3/master/c3.min.js"></script>""")
     ...
     def _genHTML(self,jsonData,chartType):
           ...
           pieDIV=self._createDiv(""" id="defaultPie" """)
           strDIV=self._createDiv(""" class="containerLeft" """,divElements=chartDIV+pieDIV)
           listChart={"chart1":chartType,"defaultPie":"pie"}
...
class D3_Chord(BaseHtml,object):
     def __init__(self,matrix,datagroups,chartType="bar"):
           self._strHTML=self._genHTML(matrix,self.checkQuote(datagroups),chartType) 
