...
import posixpath
from fresco.compat import quote_plus, urlparse, urlunparse, ustring_type
...
def _qs_frag(key, value, charset=None):
    ...
    key = ustring_type(key).encode(charset)
    value = ustring_type(value).encode(charset)
...
def _repeat_keys(iterable):
    ...
    for key, value in iterable:
        if isinstance(value, (ustring_type, bytes)):
            value = [value]
        else:
            try:
                value = iter(value)
