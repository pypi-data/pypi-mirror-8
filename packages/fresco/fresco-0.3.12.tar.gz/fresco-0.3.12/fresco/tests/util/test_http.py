...
from fresco.compat import quote, ustring_type
...
["empty-text-input", "file-upload", "text-input-ascii", "text-input-unicode"]
    ...
    def test_fileupload_too_big(self):
        ...
        r = Agent(app).post_multipart(
            files=[('f1', 'filename.txt', 'text/plain', 'x' * 1000)],
            check_status=False
        )
        assert r.response.status == "413 Payload Too Large"
    ...
    def test_fileupload_with_invalid_content_length(self):
        ...
        assert r.response.status == "400 Bad Request"
    ...
    def test_multipart_field_too_big(self):
        ...
        r = Agent(app).post_multipart(data=[('f1', 'x' * 200)],
                                          check_status=False)
        assert r.response.status == "413 Payload Too Large"
...
class TestParseFormEncodedData(object):
    ...
    def test_formencoded_data_too_big(self):
        ...
        r = Agent(app).post(data=[('f1', 'x' * 200)], check_status=False)
        assert r.response.status == "413 Payload Too Large"
    ...
    def test_posted_data_contains_non_ascii_chars(self):
        ...
        with app.requestcontext(
                '/',
                REQUEST_METHOD='POST',
                CONTENT_LENGTH=ustring_type(len(data)),
...
class TestParseBody(object):

    def test_formencoded_data_too_big(self):
        """
        POST data raises an exception if it exceeds request.MAX_SIZE bytes
        """
        def view():
            request = context.request
            request.MAX_SIZE = 100
        ...
        r = Agent(app).post(data=b'x' * 200, check_status=False)
        assert r.response.status == "413 Payload Too Large"
