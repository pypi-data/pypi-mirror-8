...
from alpaca.stats import FisherExact
...
class Caller(CLAlgorithm):
    def __init__(
        ...
        self.fisher_exact = FisherExact(cl_queue.context)
        ...
        for seq_view in self.index.get(self.query_samples):
            for index_slice in seq_view:
                ...
                if called_sites.size:
                    # TODO disabling indel filtering because BAQ should take care of that
    ...
    def build_calling_tree(self, heterozygosity=0.001):
        ...
        def _build_calling_tree(node):
            if isinstance(node, algebra.SamplePlus):
                sample_ids = np.array(
            ...
            if isinstance(node, algebra.Plus):
                _node = Plus(self.cl_queue, self.cl_mem_pool)
            else:
                _node = Minus(self.cl_queue, self.cl_mem_pool)
