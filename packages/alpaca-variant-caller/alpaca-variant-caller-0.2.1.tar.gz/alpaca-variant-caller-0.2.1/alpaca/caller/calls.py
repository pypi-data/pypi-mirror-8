...
class Calls(CLAlgorithm):
    ...
    def __init__(self, cl_queue, cl_mem_pool, fisher_exact, index_slice, quals, called_sites, genotypes, genotype_ids):
    ...
    def calc_strand_bias(self, alleles, allele_depth, allele_rev_depth):
        ...
        logging.debug("transferring to device")
        to_device = lambda a: self.to_device(np.asarray(a, dtype=np.int32), async=True)
        cl_a = to_device(a)
        cl_b = to_device(b)
        cl_c = to_device(c)
        cl_d = to_device(d)
        cl_pvals = self.empty(cl_a.shape, np.float32)

        logging.debug("calculating fishers exact test")
        self.fisher_exact.test(cl_a, cl_b, cl_c, cl_d, cl_pvals)
