...
TRANSCRIPT_CONSEQUENCE_FIELDS = "alt transcript_id consequence".split()
PROTEIN_CONSEQUENCE_FIELDS = (
    "alt hgsvp polyphen_prediction polyphen_score "
    "sift_prediction sift_score".split()
)
...
def annotate(records, species="human"):
    def transcript_effects(alt, transcript_annotation):
        ...
        for consequence in transcript_annotation["consequence_terms"]:
            yield "{alt}:{transcript_id}:{consequence}".format(
                alt=alt,
                transcript_id=transcript_id,
                consequence=consequence
            )

    def protein_effect(alt, transcript_annotation):
        get = lambda key: transcript_annotation.get(key, ".")
        return "{alt}:{hgvsp}:{polyphen_prediction}:{polyphen_score}:{sift_prediction}:{sift_score}".format(
            alt=alt,
