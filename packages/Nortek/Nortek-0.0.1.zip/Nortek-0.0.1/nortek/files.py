            ...
            while True:
                ...
                if not sync:
                    ...
                    if id == '\x05':
                        ...
                        hardwareConfiguration = nortekdata.structures.Header(
                                instrumentDataFile.read(48)) # always
                        headConfiguration = nortekdata.structures.Header(
                                instrumentDataFile.read(224)) # always
                        userConfiguration = nortekdata.structures.Header(
...
class VectrinoFile(DataFile):
    ...
    def read_data(self, assignToArrays = False):
        if assignToArrays is False:
            self[ '\x50' ] = nortekdata.structures.VectrinoVelocityHeader_binary()
            self[ '\x51' ] = nortekdata.structures.VectrinoVelocityData_binary()
            self[ '\x07' ] = nortekdata.structures.VectrinoProbeCheck_binary()
            self[ '\x0f' ] = nortekdata.structures.VectrinoFileInfo_binary()
            self[ '\x02' ] = nortekdata.structures.VectrinoDistanceMeasurement_binary()
        else:
...
class AD2CP(DataFile):
		...
		with open(os.path.join(self.pathToSource, self.filename + self.sourceExtension), 'rb') as instrumentDataFile:
			while True:
				...
				if not sync:
					...
					dataRecordHeader = nortekdata.structures.AD2CPHeader(
	...
	def read_data(self, instrumentDataFile, 
		...
		if assignToArrays is False and id not in self:
			binaryStructureGenerator = nortekdata.structures.generateAD2CP_DataRecord_binary(
				version,
				sizeInBytes)
		...
		if (instrumentDataFile.tell() + self[ id ]._sizeInBytes) < self.filesize:
			...
			if not self[ id ].calculateChecksum(instrumentDataFile):
				...
				dataBlockGenerator = nortekdata.structures.generateAD2CP_DataBlock_binary(
...
class HRProfiler(DataFile):
	...
	def read_data(self, assignToArrays = False):
		if assignToArrays is False:
			binaryStructureGenerator = nortekdata.structures.generateHRProfiler_DataRecord_binary(
