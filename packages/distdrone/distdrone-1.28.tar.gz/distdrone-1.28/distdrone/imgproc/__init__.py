...
def getalignface(facefinder,eyefinder,nosefinder,img=[],cam=False):
	...
	if len(dims)>0:
		...
		if len(biggestface)>0:
			...
			cv2.waitKey(25)
			eyes=eyefinder.detectMultiScale(thisface,1.1,7,1,(20,20),img.shape[:2])
			noses=nosefinder.detectMultiScale(thisface,1.1,5,1,(20,20),img.shape[:2])
			print "eyes: "+str(eyes)
