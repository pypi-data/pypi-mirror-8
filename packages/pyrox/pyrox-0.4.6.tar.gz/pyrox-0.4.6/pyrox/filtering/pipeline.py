...
class HttpFilterPipeline(object):
    ...
    def on_request_head(self, request_head):
        _LOG.debug('Intercepted request head. Handing off to: {}'.format(self._req_head_chain))
        return self._on_head(self._req_head_chain, request_head)

    def on_request_body(self, body_part, output):
        _LOG.debug('Intercepted request body. Handing off to: {}'.format(self._req_body_chain))
        return self._on_body(self._req_body_chain, body_part, output)

    def on_response_head(self, response_head):
        _LOG.debug('Intercepted response head. Handing off to: {}'.format(self._resp_head_chain))
        return self._on_head(self._resp_head_chain, response_head)

    def on_response_body(self, body_part, output):
        _LOG.debug('Intercepted response body. Handing off to: {}'.format(self._resp_body_chain))
        return self._on_body(self._resp_body_chain, body_part, output)

    def __str__(self):
        return 'Request Head: {}\nRequest Body: {}\nResponse Head: {}\n Response Body: {}'.format(
            self._req_head_chain,
            self._req_body_chain,
            self._resp_head_chain,
            self._resp_body_chain
        )
