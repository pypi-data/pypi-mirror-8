...
class Metaroles_DictPack(BaseDictionaryActions):
    ...
    # определяем, чтоб вызвался get_rows_modified, в который передается необходимый request
    def modify_rows_query(self):
        pass

    def get_rows_modified(self, offset, limit, filter, user_sort, request, context):
        data = []
        user_sort = user_sort or ''
        exclude_by_code = request.REQUEST.get('exclude_by_code')
        exclude_by_code = simplejson.loads(exclude_by_code)
        for role in metarole_manager.get_registered_metaroles():
            ...
            if exclude_by_code:
                # Регистронезависимое вхождение строки
                if role.code not in exclude_by_code:
                    data.append(role)
            if not(filter or exclude_by_code):
                data.append(role)
        return {'rows': data}
