    ...
    try:
        ...
        calls = [(
            [config.samtools_exe, 'mpileup', '-d', str(depth) if depth else '',
             '-r', seq['SN'] + ':', '-t', 'DP,DPR', '-gu', '-f', tmp_ref_genome] + inputfiles,
        ...
        template_header.comments['MiModDCommand'] = ['mimodd varcall ' + ' '.join(sys.argv)]
        ...
        try:
            ...
            call = [config.bcftools_exe, 'concat', '-o', output_vcf, '-O', 'b', vcf_hdr_file.name] + [f.name for f in tmp_io]
...
def varextract (inputfile, output_vcf = None, vcf_pre = None,
                keep_alts = False, verbose = False, quiet = True):
    """
            ...
            for line in raw_vcf.raw_iter():    # the raw_iter() method of the VCFReader object provides fast, line-based access to the body of a vcf file
                ...
                try:
                    if (chrom, pos) in sites:
                        ...
                        for file_no, current_record_alt, current_record_sampleinfo in pre_info:
