...
version = mimodd_base.Version((0, 9, 4))

# bug fix version 0,9,3
# bug fix version 0,9,2
# new in 0,9,1:
# improved handling of header information (taken from input file or custom_header information)
# new in 0,9,0:
# improved handling of read pair overlaps (allowed fraction of overlap is now adjustable)
# made command line more similar to that of SNAP
# added multi-threading to snap_index
# bug fix version 0,8,5
# new in 0,8,4:
# complete handling of SIGTERM
# new in 0,8,3:
# (incomplete) handling of SIGTERM
# new in 0,8,2:
# object-based version handling
# --version switch on command line routes to stdout
# new in 0,8,1:
# snap_batch handles indexing intelligently now,
# creating repeatedly used index directories only once
# new in 0,8,0:
# snap_call can deal with reference genome or pre-existing index
# introduced optional permanent idx_out directory
# made snap index a separate function
# new in 0,7,2:
# introduced verbose and quiet arguments in snap_call
# started implementing piping
# new in 0,7,1:
# made all parameter names in snap_call match those of SNAP
# new in 0,7,0:
# introduced snap_batch (replacing the previous 'batch' mode inside snap_call)
# made make_snap_argdictlist accept any iterator instead of a batch file name
# added doc strings
# new in 0,6,4:
# takes tempfile_dir and threads number from config.py
# new in 0,6,3:
# improved read tag and header reconstitution
# bug fix version (0,6,2)
# bug fix version (0,6,1)
# new in version (0,5,4):
# paired-read filtering to remove small template fragments
# new in version (0,5,0):
# introduced batch mode for running several snap_caller jobs specified in a file

import sys, os
import signal
import shutil
import subprocess   # to call subprocesses
import argparse     # to parse arguments out of sys.argv
import gzip   # to handle compressed data
from collections import Counter

from . import config, pysamtools, samheader, pysam, tmpfiles
from .decox import parachute, FuncInfo
from .fasta import FastaReader

tempfile_dir = config.tmpfiles_path
snap_exe = os.path.normpath(
    os.path.join(config.__file__, config.bin_path, 'snap'))
...
def snap_call (mode, refgenome_or_indexdir, inputfiles, iformat = 'fastq', oformat = 'sam', verbose = False, quiet = True, **other_options):
    ...
    try:
        try:
            ...
            elif args.get("iformat") == "sam" and (inputfiles[0].split('.')[-1] != 'sam'): # SNAP relies on the .sam suffix to use sam files, but we fool it with a hard link
                hard_link = tmpfiles.tmp_hardlink(inputfiles[0], outfileprefix, '.sam', fallback = 'copy') # SNAP cannot use symbolic links, so if no hard link can be generated, we have to go for a copy of the file
                if verbose:
            ...
            if os.path.isdir(refgenome_or_indexdir):
                ...
                if args.get('idx_out'):
                    ...
                    idx_remove = True
                call, results, errors = snap_index(refgenome_or_indexdir, ref_index,
                                                   args.get('idx_seedsize'), args.get('idx_slack'), verbose = verbose, quiet = quiet)
        ...
        for sq in header['SQ']:                   # but we want to add MD5 tags to them
            sq['M5'] = md5dict[sq['SN']]
        header['PG'].extend(snap_header['PG'])    # add SNAP's PG header line
        ...
        for job_no, job_args in enumerate(job_arglist, 1):
            ...
            if not os.path.isdir(job_args['refgenome_or_indexdir']):
                ...
                if not os.path.exists(idx_key[1]):
                    call, results, errors = snap_index(*idx_key[:-1], verbose = job_args["verbose"], quiet = job_args["quiet"])
                refgenomes[idx_key] -= 1
                job_args['refgenome_or_indexdir'] = idx_key[1]
                for param in ('idx_out', 'idx_seedsize', 'idx_slack'):
...
def make_snap_argdictlist (commands=None, ifile=None):
    ...
    try:
        if not commands:
            assert ifile, 'Either the commands or the ifile argument MUST be specified.'
            ...
            for hdr in all_headers:
                for rg in hdr['RG']:
                    if rg['ID'] in rg_sm_mapping:
                        assert rg_sm_mapping[rg['ID']] == rg['SM'], 'The input files declare the read group {0} more than once with different sample names.'.format(rg['ID'])
                    else:
...
SnapCLParser.add_argument("-o", "--outputfile", metavar = 'OFILE', required = True, default = argparse.SUPPRESS, help = 'name of the output file (required)')
...
SnapCLParser.add_argument("-d", "--maxdist", metavar = 'EDIT DISTANCE', type = int, default = argparse.SUPPRESS, help = 'maximum edit distance allowed per read or pair (default: 15)')
...
SnapCLParser.add_argument("-c", "--confdiff", metavar = 'THRESHOLD', type = int, default = argparse.SUPPRESS, help = 'confidence threshold (default: 1)')
SnapCLParser.add_argument("-a", "--confadapt", metavar = 'THRESHOLD', type = int, default = argparse.SUPPRESS, help = 'confidence adaptation threshold (default: 7)')
...
SnapCLParser.add_argument("-b", "--bind_threads", action = 'store_true', default = argparse.SUPPRESS, help = 'bind each thread to its processor (off by default)')
SnapCLParser.add_argument("-e", "--error_rep", action = 'store_true', default = argparse.SUPPRESS, help = 'compute error rate assuming wgsim-generated reads')
SnapCLParser.add_argument('-P', '--no_prefetch', action = 'store_true', default = False, help = 'disables cache prefetching in the genome; may be helpful for machines with small caches or lots of cores/cache')
...
SnapCLParser.add_argument("-f", "--stop_on_first", action = 'store_true', default = argparse.SUPPRESS, help = 'stop on first match within edit distance limit (filtering mode)')
SnapCLParser.add_argument("-F", "--filter_output", metavar = 'FILTER', default = argparse.SUPPRESS, help = 'retain only certain read classes in output (a=aligned only, s=single hit only, u=unaligned only)')
...
SnapCLParser.add_argument("-M", "--mmatch_notation", action = 'store_true', default = argparse.SUPPRESS, help = 'indicates that CIGAR strings in the generated SAM file should use M (alignment match) rather than = and X (sequence (mis-)match)')
SnapCLParser.add_argument("-G", "--gap_penalty", type = int, default = argparse.SUPPRESS, help = 'specify a gap penalty to use when generating CIGAR strings')
SnapCLParser.add_argument("-s", "--spacing", nargs = 2, metavar = ('MIN','MAX'), default = argparse.SUPPRESS, help = 'min and max spacing to allow between paired ends (default: 100 1000)')
SnapCLParser.add_argument('--max_mate_overlap', '--mmo', metavar = 'FRACTION', type = float, default = argparse.SUPPRESS, help = 'for paired-end reads only retain reads with maximally this fraction of their length overlap with their mate; default: 0 (no overlap allowed)')
...
idx_group.add_argument("--idx_seedsize", metavar = 'SEED SIZE', type = int, default = argparse.SUPPRESS, help = 'Seed size used in building the index (default: 20)')
idx_group.add_argument("--idx_slack", metavar = 'SLACK', type = float, default = argparse.SUPPRESS, help = 'Hash table slack for indexing (default: 0.3)')
idx_group.add_argument('--idx_out', default = argparse.SUPPRESS, help = 'name of the index directory to be created; if given, the index directory will be permanent, otherwise a temporary directory will be used')
...
def snap_index (ref_genome, index_out, seedsize = None, slack = None, threads = None, verbose = False, quiet = True):
