        ...
        if grouped and len(expandedItems) > 0:
            ...
            if filter:
                if aggr:
                    ...
                    query = self.get_data().filter(
                        filter).values(field).distinct()
            else:
                if aggr:
                    query = self.get_data().values(field).annotate(
                        *aggr).annotate(count=Count("id"))
...
class GroupingRecordDataProvider(GroupingRecordProvider):
    ...
    def __read_data(self, grouped, offset, level_index, level_keys, begin, end, aggregates, sorting):
        ...
        # print 'out_data(): grouped=%s, offset=%s, level_index=%s,
        # level_keys=%s, begin=%s, end=%s' % (grouped, offset, level_index,
        # level_keys, begin, end)
        ...
        if level_index == -1:
            ...
            for rec in self.get_data():
                ...
                for agg in aggregates.keys():
                    ...
                    if agg_type == 'sum':
                        aggr_rec[agg] = agg_value + (aggr_rec[
                                                     agg] if agg in aggr_rec else 0)
                    elif agg_type == 'count':
                        aggr_rec[agg] = 1 + (aggr_rec[
                                             agg] if agg in aggr_rec else 0)
                    elif agg_type == 'min':
                        aggr_rec[agg] = agg_value if aggr_rec.get(
                            agg, 0) > agg_value else aggr_rec.get(agg, 0)
                    elif agg_type == 'max':
                        aggr_rec[agg] = agg_value if aggr_rec.get(
                            agg, 0) < agg_value else aggr_rec.get(agg, 0)
                    elif agg_type == 'avg':
            ...
            if aggr_rec:
                for agg in aggregates.keys():
                    ...
                    if aggregates[agg] == 'avg':
                        self.setattr(item, agg, aggr_rec[
                                     agg] / self.getattr(item, 'count'))
                    else:
            ...
            for i, rec in prepared:
                if field:
                    ...
                    for agg in aggregates.keys():
                        # для средних - посчитаем среднее
                        if aggregates[agg] == 'avg':
                            self.setattr(item, agg, aggregate_values[
                                         i][agg] / self.getattr(item, 'count'))
                        else:
    ...
    def __count_data(self, grouped, level_index, level_keys, expandedItems):
        ...
        # print 'count_exp_data(): grouped=%s, level_index=%s, level_keys=%s,
        # expandedItems=%s' % (grouped, level_index, level_keys, expandedItems)
        total_of_level = 0
        if grouped:
            if level_index == 0:
