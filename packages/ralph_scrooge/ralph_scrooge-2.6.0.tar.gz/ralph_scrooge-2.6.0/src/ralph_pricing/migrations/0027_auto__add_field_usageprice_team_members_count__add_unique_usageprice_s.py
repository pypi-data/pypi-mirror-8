...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'UsagePrice.team_members_count'
        db.add_column('ralph_pricing_usageprice', 'team_members_count',
                      self.gf('django.db.models.fields.IntegerField')(default=0, null=True, blank=True),
                      keep_default=False)

        # Adding unique constraint on 'UsagePrice', fields ['start', 'type', 'team']
        db.create_unique('ralph_pricing_usageprice', ['start', 'type_id', 'team_id'])

        # Adding unique constraint on 'UsagePrice', fields ['type', 'end', 'team']
        db.create_unique('ralph_pricing_usageprice', ['type_id', 'end', 'team_id'])

        # Adding unique constraint on 'UsagePrice', fields ['internet_provider', 'start', 'type']
        db.create_unique('ralph_pricing_usageprice', ['internet_provider_id', 'start', 'type_id'])

        # Adding unique constraint on 'UsagePrice', fields ['internet_provider', 'type', 'end']
        db.create_unique('ralph_pricing_usageprice', ['internet_provider_id', 'type_id', 'end'])


    def backwards(self, orm):
        # Removing unique constraint on 'UsagePrice', fields ['internet_provider', 'type', 'end']
        db.delete_unique('ralph_pricing_usageprice', ['internet_provider_id', 'type_id', 'end'])

        # Removing unique constraint on 'UsagePrice', fields ['internet_provider', 'start', 'type']
        db.delete_unique('ralph_pricing_usageprice', ['internet_provider_id', 'start', 'type_id'])

        # Removing unique constraint on 'UsagePrice', fields ['type', 'end', 'team']
        db.delete_unique('ralph_pricing_usageprice', ['type_id', 'end', 'team_id'])

        # Removing unique constraint on 'UsagePrice', fields ['start', 'type', 'team']
        db.delete_unique('ralph_pricing_usageprice', ['start', 'type_id', 'team_id'])

        # Deleting field 'UsagePrice.team_members_count'
        db.delete_column('ralph_pricing_usageprice', 'team_members_count')
