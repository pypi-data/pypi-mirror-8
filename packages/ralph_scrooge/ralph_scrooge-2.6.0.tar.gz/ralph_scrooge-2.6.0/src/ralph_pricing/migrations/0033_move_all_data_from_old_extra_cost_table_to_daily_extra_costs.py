...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for extra_cost in orm.ExtraCost.objects.all():
            for day in xrange((extra_cost.end - extra_cost.start).days + 1):
                date = extra_cost.start + datetime.timedelta(day)
                daily_extra_cost, created = orm.DailyExtraCost.objects.get_or_create(
                    date=date,
                    pricing_venture=extra_cost.pricing_venture,
                    type=extra_cost.type,
                )
                daily_extra_cost.value = (
                    extra_cost.monthly_cost/monthrange(date.year, date.month)[1]
                )
