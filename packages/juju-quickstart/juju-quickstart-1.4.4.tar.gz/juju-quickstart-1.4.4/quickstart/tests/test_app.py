...
class TestBootstrap(
    ...
    def test_success_upload_tools(self, mock_print):
        # The environment is bootstrapped with local tools.
        with self.patch_multiple_calls(self.make_side_effects()) as mock_call:
            already_bootstrapped, series = app.bootstrap(
                self.env_name, self.juju_command, upload_tools=True)
        self.assertFalse(already_bootstrapped)
        mock_call.assert_has_calls([
            mock.call(
                self.juju_command, 'bootstrap', '-e', self.env_name,
    ...
    def test_success_upload_series(self, mock_print):
        # The environment is bootstrapped with tools for specific series.
        with self.patch_multiple_calls(self.make_side_effects()) as mock_call:
            already_bootstrapped, series = app.bootstrap(
                self.env_name, self.juju_command, upload_series='hoary')
        self.assertFalse(already_bootstrapped)
        mock_call.assert_has_calls([
            mock.call(
                self.juju_command, 'bootstrap', '-e', self.env_name,
    ...
    def test_success_constraints(self, mock_print):
        # The environment is bootstrapped with given constraints.
        with self.patch_multiple_calls(self.make_side_effects()) as mock_call:
            already_bootstrapped, series = app.bootstrap(
                self.env_name, self.juju_command, constraints='mem=7G')
        self.assertFalse(already_bootstrapped)
        mock_call.assert_has_calls([
            mock.call(
                self.juju_command, 'bootstrap', '-e', self.env_name,
...
class TestJujuRequiresSudo(ProgramExitTestsMixin, unittest.TestCase):
    no_sudo_versions = [
        (1, 17, 2), (1, 17, 10), (1, 18, 0), (1, 18, 2), (2, 16, 1)]
    sudo_versions = [(0, 7, 9), (1, 0, 0), (1, 16, 42), (1, 17, 0), (1, 17, 1)]

    def test_non_local_returns_false(self):
        # A non-local provider does not require sudo.
        actual = app.juju_requires_sudo('aws', None, None)
        self.assertFalse(actual)

    def test_local_old_juju_returns_true(self):
        # A juju previous to 1.17.2 requires sudo.
        for version in self.sudo_versions:
            self.assertTrue(app.juju_requires_sudo('local', version, False),
                            version)

    def test_local_newer_juju_returns_false(self):
        # A juju 1.17.2 and newer does not require sudo.
        for version in self.no_sudo_versions:
            self.assertFalse(app.juju_requires_sudo('local', version, False),
                             version)

    def test_raises_programexit(self):
        for version in self.sudo_versions:
            with self.assert_program_exit('cannot use a customized Juju when '
                                          'it requires sudo'):
