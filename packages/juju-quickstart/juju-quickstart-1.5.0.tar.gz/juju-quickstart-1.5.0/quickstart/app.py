...
def check_bootstrapped(env_name):
    ...
    If so, return the environment API URL to be used to connect to the Juju API
    server. If not already bootstrapped, or if the API URL cannot be retrieved,
    return None.
    """
    ...
    try:
        ...
        logging.warn(b'cannot retrieve the Juju API URL: {}'.format(err))
        return None
    # Look for a reachable API URL.
    if not candidates:
        logging.warn('cannot retrieve the Juju API URL: no addresses found')
        return None
    for candidate in candidates:
        error = netutils.check_listening(candidate)
        if error is None:
            # Juju API URL found.
            return 'wss://{}'.format(candidate)
        logging.debug(error)
    logging.warn(
        'cannot retrieve the Juju API URL: cannot connect to any of the '
...
def status(env_name, juju_command):
    ...
    while time.time() < timeout:
        ...
        if agent_state == 'started':
            return utils.get_bootstrap_node_series(output)
...
def get_env_type(env_name):
    """Return the Juju environment type for the given environment name.

    Since the environment type is retrieved by parsing the jenv file, the
    environment must be already bootstrapped.

    Raise a ProgramExit if the environment type cannot be retrieved.
    """
    try:
        return jenv.get_value(env_name, 'bootstrap-config', 'type')
    except ValueError as err:
        msg = b'cannot retrieve environment type: {}'.format(err)
        raise ProgramExit(msg)


def get_admin_secret(env_name):
    """Return the Juju admin secret for the given environment name.

    Parse the jenv file to retrieve the admin secret.
    Raise a ProgramExit if the admin secret cannot be retrieved.
    """
    try:
        return jenv.get_value(env_name, 'bootstrap-config', 'admin-secret')
    except ValueError as err:
        msg = b'cannot retrieve environment admin-secret: {}'.format(err)
        raise ProgramExit(msg)
