...
class TestRun(unittest.TestCase):
    ...
    def test_run(self, mock_app, mock_open):
        ...
        mock_app.get_api_url.assert_called_once_with(
            options.env_name, self.juju_command)
        mock_app.get_admin_secret.assert_called_once_with(options.env_name)
        mock_app.connect.assert_has_calls([
            mock.call('wss://1.2.3.4:17070', 'Secret!'),
            mock.call().close(),
            mock.call('wss://1.2.3.4:443/ws', 'Secret!'),
            mock.call().close(),
        ])
    ...
    def test_already_bootstrapped(self, mock_app, mock_open):
        ...
        self.configure_app(mock_app, check_bootstrapped='wss://example.com')
        # Run the application.
        options = self.make_options()
        with self.patch_get_juju_command():
            manage.run(options)
        # The environment type is retrieved from the jenv.
        mock_app.get_env_type.assert_called_once_with(options.env_name)
    ...
    def test_already_bootstrapped_race(self, mock_app, mock_open):
        ...
        self.configure_app(mock_app, bootstrap=True)
        # Run the application.
        options = self.make_options()
        with self.patch_get_juju_command():
            manage.run(options)
