...
class TestGetEnvType(
    ...
    def test_success(self):
        ...
        with self.make_jenv('ec2', yaml.safe_dump(self.jenv_data)):
            env_type = app.get_env_type('ec2')
        self.assertEqual('ec2', env_type)

    def test_error(self):
        # A ProgramExit is raised if the environment type cannot be retrieved.
        with self.make_jenv('aws', '') as path:
            expected_error = (
                'cannot retrieve environment type: invalid YAML '
                'contents in {}: bootstrap-config key not found in the root '
                'section'.format(path))
            with self.assert_program_exit(expected_error):
                app.get_env_type('aws')


class TestGetAdminSecret(
        helpers.JenvFileTestsMixin, ProgramExitTestsMixin, unittest.TestCase):

    def test_success(self):
        # The admin secret is successfully retrieved.
        with self.make_jenv('ec2', yaml.safe_dump(self.jenv_data)):
            admin_secret = app.get_admin_secret('ec2')
        self.assertEqual('Secret!', admin_secret)

    def test_error(self):
        # A ProgramExit is raised if the admin secret cannot be retrieved.
        with self.make_jenv('ec2', '') as path:
            expected_error = (
                'cannot retrieve environment admin-secret: invalid YAML '
                'contents in {}: bootstrap-config key not found in the root '
                'section'.format(path))
            with self.assert_program_exit(expected_error):
                app.get_admin_secret('ec2')
