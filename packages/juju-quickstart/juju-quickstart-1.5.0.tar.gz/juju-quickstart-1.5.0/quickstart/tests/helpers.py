...
class JenvFileTestsMixin(object):
    ...
    def make_jenv(self, env_name, contents):
        ...
        """
        # Create a temporary JUJU_HOME.
        playground = tempfile.mkdtemp()
        self.addCleanup(shutil.rmtree, playground)
        environments_dir = os.path.join(playground, 'environments')
        os.mkdir(environments_dir)
        # Create the jenv file inside the temporary JUJU_HOME.
        jenv_path = os.path.join(environments_dir, '{}.jenv'.format(env_name))
