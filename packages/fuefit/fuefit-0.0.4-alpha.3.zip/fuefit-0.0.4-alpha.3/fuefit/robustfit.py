...
def curve_fit(f, xdata, ydata, p0=None, sigma=None, robust=4.685, **kw):
    '''Uses a non-linear iteratively-reweighted least-squares (IRLS) to robustly fit a function, f, to the data.
...
def _weighted_residual_function(params, xdata, ydata, function, s_weights): #, x_bounds
    residual = s_weights * (function(xdata, *params) - ydata)
    
    return residual 

def _reweighted_residual_function(params, xdata, ydata, function, s_weights, robust):
    residual    = _weighted_residual_function(params, xdata, ydata, function, s_weights)
    r_a         = abs(residual)
