...
class TemplateTagsTests(TestCase):
    ...
    def test_simple_humanize_join_with_two_items(self):
        t = Template('{% load formatting %}{{ mylist|humanized_simple_join }}')
        c = Context({"mylist": self.test_list})
        output = t.render(c)
        self.assertEqual(output, 'apples and oranges')

    def test_simple_humanize_join_with_more_items(self):
        t = Template('{% load formatting %}{{ mylist|humanized_simple_join }}')
