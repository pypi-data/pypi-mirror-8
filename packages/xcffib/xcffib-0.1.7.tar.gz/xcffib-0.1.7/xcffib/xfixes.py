...
class SelectionNotifyEvent(xcffib.Event):
    ...
    def pack(self):
        buf = six.BytesIO()
        buf.write(struct.pack("=BIIIII8x", self.subtype, self.window, self.owner, self.selection, self.timestamp, self.selection_timestamp))
        return buf.getvalue()
...
class CursorNotifyEvent(xcffib.Event):
    ...
    def pack(self):
        buf = six.BytesIO()
        buf.write(struct.pack("=BIIII12x", self.subtype, self.window, self.cursor_serial, self.timestamp, self.name))
        return buf.getvalue()
