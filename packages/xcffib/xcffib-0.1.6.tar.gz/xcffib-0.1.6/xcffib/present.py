...
class GenericEvent(xcffib.Event):
    ...
    def pack(self):
        buf = six.BytesIO()
        buf.write(struct.pack("=BIH2xI", self.extension, self.length, self.evtype, self.event))
        return buf.getvalue()
...
class ConfigureNotifyEvent(xcffib.Event):
    ...
    def pack(self):
        buf = six.BytesIO()
        buf.write(struct.pack("=2xIIhhHHhhHHI", self.event, self.window, self.x, self.y, self.width, self.height, self.off_x, self.off_y, self.pixmap_width, self.pixmap_height, self.pixmap_flags))
        return buf.getvalue()
...
class CompleteNotifyEvent(xcffib.Event):
    ...
    def pack(self):
        buf = six.BytesIO()
        buf.write(struct.pack("=BBIIIQQ", self.kind, self.mode, self.event, self.window, self.serial, self.ust, self.msc))
        return buf.getvalue()
...
class IdleNotifyEvent(xcffib.Event):
    ...
    def pack(self):
        buf = six.BytesIO()
        buf.write(struct.pack("=2xIIIII", self.event, self.window, self.serial, self.pixmap, self.idle_fence))
        return buf.getvalue()
...
class RedirectNotifyEvent(xcffib.Event):
    ...
    def pack(self):
        buf = six.BytesIO()
        buf.write(struct.pack("=BxIIIIIIIhhIIII4xQQQ", self.update_window, self.event, self.event_window, self.window, self.pixmap, self.serial, self.valid_region, self.update_region, self.x_off, self.y_off, self.target_crtc, self.wait_fence, self.idle_fence, self.options, self.target_msc, self.divisor, self.remainder))
