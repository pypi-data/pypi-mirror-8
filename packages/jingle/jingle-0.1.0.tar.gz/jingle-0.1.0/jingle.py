...
class Template(object):
    ...
    def __init__(self, configfile, template):
        self._config = AwesomeConfigParser()
        self._config.parse(configfile)
        self._template = template
        self.ctx = self._config.dict()

    def render(self):
        from jinja2 import Template
        return Template(self._template).render(self.ctx)
...
class CLI(object):
    ...
    def main(self):
        ...
        try:
            tpl = Template(self.argv[0], self.stdin.read())
            self._debug_context(tpl.ctx)
            self.stdout.write(tpl.render())
