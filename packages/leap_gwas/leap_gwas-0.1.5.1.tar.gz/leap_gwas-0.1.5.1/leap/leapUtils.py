...
def loadPheno(bed, phenoFile, missingPhenotype='-9', keepDict=False):
	...
	checkIntersction(bed, pheno, 'phenotypes')
	bed, pheno = pstutil.intersect_apply([bed, pheno])
	if (not keepDict): pheno = pheno['vals']
	return bed, pheno
	
	
def checkIntersction(bed, fileDict, fileStr, checkSuperSet=False):
...
def findRelated(bed, relCutoff):
	...
	removeSet = set(np.sort(vc.VertexCut().work(XXT, relCutoff))) #These are the indexes of the IIDs to remove		
