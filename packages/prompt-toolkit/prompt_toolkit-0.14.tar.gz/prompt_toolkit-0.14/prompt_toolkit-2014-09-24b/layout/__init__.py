...
class Layout(object):
    ...
    def _write_input(self, cli, screen, highlight=True):
        # Get tokens
    ...
    def write_input_scrolled(self, cli, screen, write_content, accept_or_abort=False,
        ...
        """
        left_margin_width = self.left_margin.width if self.left_margin else 0
        ...
        for y in range(0, min(max_height, temp_screen.current_height - self.vertical_scroll)):
            if self.left_margin:
                ...
                screen._x = 0
                self.left_margin.write(cli, screen, y + self.vertical_scroll)
        ...
        if not accept_or_abort and self.need_to_show_completion_menu(cli):
    ...
    def write_to_screen(self, cli, screen, min_available_height, accept=False, abort=False):
        """
        ...
        top_toolbars = [b for b in self.top_toolbars if b.is_visible(cli, screen)]
        bottom_toolbars = [b for b in self.bottom_toolbars if b.is_visible(cli, screen)]
        ...
        def write_content(scr):
            ...
            self._write_input(cli, scr, highlight=not (accept or abort))
