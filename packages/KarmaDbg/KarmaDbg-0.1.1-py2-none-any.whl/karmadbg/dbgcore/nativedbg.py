...
class NativeDebugger(object):
    ...
    def debugCommand(self, commandStr, echo=False):
        ...
        elif self.isNativeCmd(commandStr):
           self.debugServer.commandServer.sendAnswer( self.nativeCmd(commandStr) )

        elif self.isWindbgCommand(commandStr):
            self.debugServer.commandServer.sendAnswer( self.windbgCommand(commandStr) )

        elif self.debugServer.isMacroCmd(commandStr):
            self.debugServer.macroCmd(commandStr)

        else:
            self.debugServer.commandServer.sendAnswer( self.pythonCommand(commandStr) )
