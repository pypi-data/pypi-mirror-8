...
class PythonDebugger(Bdb):
    ...
    def __init__(self, debugServer):
        Bdb.__init__(self, skip=["dbgcore.*"])
        self.eventHandler = debugServer.getClientEventHandler()
        self.debugServer = debugServer
        self.origtrace = None

    def __enter__(self):
        if self.eventHandler.onPythonStart():
    ...
    def debugCommand(self, commandStr, echo=False):
        ...
        if tokens[0] == 'h':
            ...
            print  "bp [file] lineno - remove breakpoint"
            print  "w - print stack"
    ...
    def getPythonLocals(self):
        return dict( [ (name, str(val) ) for name, val in self.currentFrame.f_locals.items() ] )
