    ...
    def dataUpdate(self):
        ...
        if expr:
            if self.combo.currentIndex() == 0:
                memdmp = yield( self.uimanager.callFunction(getBytes, expr, 0x1000) )
                if memdmp:
                    text = reduce(lambda x, y: x + "%d " % y, memdmp, "")
                    self.hexTextEdit.setPlainText(text)
            if self.combo.currentIndex() == 1:
                memdmp = yield( self.uimanager.callFunction(getSignBytes, expr, 0x1000) )
                if memdmp:
                    text = reduce(lambda x, y: x + "%d " % y, memdmp, "")
                    self.hexTextEdit.setPlainText(text)
            ...
            if self.combo.currentIndex() == 3:
                memdmp = yield( self.uimanager.callFunction(getChars, expr, 0x1000) )
                if memdmp:
