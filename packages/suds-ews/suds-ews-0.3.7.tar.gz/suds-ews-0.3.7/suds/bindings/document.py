...
class Document(Binding):
    ...
    def make_attribute(self, method, pdef, object):
        if object is not None:
            # FIXME: pass through marshaller
            return Attribute(pdef[0], object)
    ...
    def bodycontent_for_method(self, method, args, kwargs):
        ...
        for n, param_def in enumerate(self.param_defs(method)):
            try:
                value = args[n]
            except IndexError:
                try:
                    value = kwargs.pop(
                        param_def[1].isattr() and '_%s' % param_def[0] or param_def[0],
                    )
                except KeyError:
                    continue

            if param_def[1].isattr():
                attr = self.make_attribute(method, param_def, value)
                if attr is None:
                    continue

                root.attributes.append(attr)
            else:
                param = self.mkparam(method, param_def, value)

                if param is None:
                    continue

                # if has explicit type definition - delete it (EWS workaround)
                param_type = param.attrib('xsi:type')
                if param_type is not None:
                    param.attributes.remove(param_type)

                if not wrapped:
                    ns = param_def[1].namespace('ns0')
                    param.setPrefix(ns[0], ns[1])
                root.append(param)

        n += 1
        if n < len(args):
            raise ValueError('Some arguments didn\'t recognized: %s' % ', '.join(args[n:]))
        if [k for k in kwargs.iterkeys() if not k.startswith('__')]:
            raise ValueError(
                'Some named arguments didn\'t recognized: %s' % ', '.join(
