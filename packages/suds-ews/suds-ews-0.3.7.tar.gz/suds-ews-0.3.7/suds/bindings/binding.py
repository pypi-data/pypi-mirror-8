...
class NotMappedToPort(Exception):
    pass
...
class Binding:
    ...
    def get_message(self, *content):
        """
        Get the soap message for the specified root element name and content.
        This is the entry point for creating the outbound soap message.
        @param body_name: Body's root element name.
        @type args: string
        @param body_content: Element for body.
        @type args: list
        ...
        body = self.body(
            *[self.marshaller().process(Content(tag=c[0], value=c[1])) for c in content]
        )
    ...
    def get_message_for_method(self, method, args, kwargs):
        """
        Get the soap message for the specified method, args and soapheaders.
        This is the entry point for creating the outbound soap message for method call.
        ...
        @rtype: str
        """
        content = self.headercontent_for_method(method)
        header = self.header(content)
        content = self.bodycontent_for_method(method, args, kwargs)
        ...
        for node in nodes:
            ...
            if rt is None:
                if node.get('id') is None:
                    raise NotMappedToPort('<%s/> not mapped to message part' % tag)
                else:
                    continue
            resolved = rt.resolve(nobuiltin=True)
            sobject = unmarshaller.process(node, resolved)
    ...
    def bodycontent_for_method(self, method, args, kwargs):
        """
    ...
    def headercontent(self, additional_childs=None):
        """
        Get the content for the soap I{Header} node.
        @param additional_childs: Additional child elements.
        @type method: I{service.Method}
        @return: The xml content for the <body/>
        @rtype: [L{Element},..]
        """
        ...
        if additional_childs:
            content.extend(additional_childs)

        return content

    def headercontent_for_method(self, method):
        """
        Get the content for the soap I{Header} node for method.
        @param method: A service method.
        @type method: I{service.Method}
        @return: The xml content for the <body/>
        @rtype: [L{Element},..]
        ...
        if isinstance(headers, (tuple,list)):
            for pt, header in zip(pts, headers):
                if isinstance(header, Element):
                    childs.append(deepcopy(header))
                    continue
        ...
        return self.headercontent(childs)
