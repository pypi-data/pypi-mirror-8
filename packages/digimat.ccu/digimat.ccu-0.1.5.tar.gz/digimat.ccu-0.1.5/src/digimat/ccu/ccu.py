...
class CCUDevice(object):
	...
	def manager(self):
		with self._lock:
			if time.time()>=self._timeoutRefresh:
				...
				self._timeoutRefresh=time.time()+30

	def getValue(self, channel, name):
		try:
			return self.ccu.rpc.getValue(self.channelAddress(chanel), name)
		except:
			pass

	def setValue(self, channel, name, value):
		try:
			return self.ccu.rpc.setValue(self.channelAddress(chanel), name, value)
		except:
			pass
	...
	def browseValues(self, channel=1):
		values=self.ccu.rpc.getParamsetDescription(self.channelAddress(channel), 'VALUES')
