...
class CurrentTimestamp(object):
    ...
    def patch(cls, field):
        ...
        orig_get_db_prep_value = field.get_db_prep_value
        orig_get_prep_lookup = field.get_prep_lookup
        orig_get_db_prep_lookup = field.get_db_prep_lookup
        ...
        def get_db_prep_value(self, value, connection, prepared=False):
            result = value if isinstance(value, cls) else orig_get_db_prep_value(self, value, connection, prepared)
            #logger.debug('prep_db_value :: name={} type(result)={} result={}'.format(self.name, type(result), result))
            return result

        def get_prep_lookup(self, lookup_type, value):
            result = value if isinstance(value, cls) else orig_get_prep_lookup(self, lookup_type, value)
            #logger.debug('prep_lookup :: name={} type(result)={} result={}'.format(self.name, type(result), result))
            return result

        def get_prep_db_lookup(self, lookup_type, value, connection, prepared=True):
            result = value if isinstance(value, cls) else orig_get_db_prep_lookup(self, lookup_type, value, connection=connection, prepared=True)
        ...
        field.get_db_prep_lookup = get_prep_db_lookup
