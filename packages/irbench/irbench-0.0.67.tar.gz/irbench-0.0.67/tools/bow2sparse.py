...
def getargs():
    ...
    parser.add_argument('npypath', type=str,
    ...
    parser.add_argument('-t', '--dtype', type=str, nargs='?',
                        default='float32',
                        help='data type')
    parser.add_argument('-b', '--blksz', type=int, nargs='?', default=10000,
                        help='block size')
...
def main(args):
    ...
    feat = lil_matrix((args.rows, args.cols), dtype=dtype)
    pinfo("Done!\n")
    ...
    with open(args.svmpath) as svmf:
        for i in range(0, args.rows, args.blksz):
            ...
            parlst = [(j, svmf.readline(), dtype) for j in range(i, i + nrows)]
    ...
    pinfo("Dumping ... ")
    np.save(args.npypath, (feat.data, feat.nonzero()))
    pinfo("Done!\n")
