...
def clip(dataset, mask_geometry, nodata_value=-1, gdal_driver_name='MEM', ogr_driver_name='Memory'):
    """Clip dataset with respect to a mask_geometry.

    :param dataset: raster
    :param mask_geometry: mask_geometry to be used for clip.
    :param nodata_value: value for no data for clip.
    ...
    # Save the raster's affine transformation and its inverse for later computations.
    a = affine_transformation(dataset)
    a_inv = affine_transformation_inv(dataset)
    ...
    geometry_geo2raster(geometry_raster, a_inv)
    ...
    if xoff + win_xsize > dataset.RasterXSize:
        logging.debug('Correcting win_xsize from "{}" to "{}"'.format(win_xsize, dataset.RasterXSize - xoff))
        win_xsize = dataset.RasterXSize - xoff
    if yoff + win_ysize > dataset.RasterYSize:
        logging.debug('Correcting win_ysize from "{}" to "{}"'.format(win_ysize, dataset.RasterYSize - yoff))
    ...
    # Create an in-memory shape dataset holding the specified mask_geometry.
    ogr_ds = ogr_driver.CreateDataSource('/tmp/test.shp')
    ...
    upper_left = project_raster_to_geo([[xoff, yoff]], affine_trans=a)[0]
    ...
    logging.info('Started burning shape.')
    # Burn mask_geometry to raster layer.
    gdal.RasterizeLayer(dataset=mask_ds, bands=[1], layer=shape_layer, burn_values=[255])
    logging.info('Finished burning shape.')
    logging.info('Started reading burn data...')
    ...
    logging.info('Finished reading burn data.')
    ...
    for band_idx in range(1, dataset.RasterCount+1):
        ...
        result_band = result_ds.GetRasterBand(band_idx)
        ...
        logging.info('Started masking data for band {}'.format(band_idx))
        result_band.WriteArray(result.filled(fill_value=nodata_value))
        logging.info('Finished masking data for band {}'.format(band_idx))

    # Free resources for raster holding intermediate results.
    mask_ds = None
