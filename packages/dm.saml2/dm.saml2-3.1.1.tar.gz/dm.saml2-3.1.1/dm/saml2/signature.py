...
import dm.saml2.config as config
  ...
  def _toDOM_csc(self, dom_support, parent):
    ...
    try:
      ...
      setattr(
        ...
        Signature(
          SignedInfo(
            CanonicalizationMethod(Algorithm=config.canonicalization_method),
            SignatureMethod(Algorithm=config.signature_method),
            Reference(
              Transforms(
                Transform(Algorithm="http://www.w3.org/2000/09/xmldsig#enveloped-signature"),
                Transform(Algorithm=config.canonicalization_method),
                ),
              DigestMethod(Algorithm=config.digest_method),
              DigestValue(''),
...
class _SignatureRequest(object):
  ...
  def sign_binary(self, instance, data):
    ...
    context = self._context
    if context is None: context = default_sign_context
    return context.sign_binary(data, config.signature_method, keyname)
