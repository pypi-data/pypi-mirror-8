...
from converter import Interactions
from sif import SIF
    ...
    * monotony  (col 9)
    ...
    def __init__(self, filename=None, type=2, verbose=False):
        ...
        :param str filename: an optional filename containing reactions in CNA
            format. If not provided, the Reactions object is empty but you can
            add reactions using :meth:`~cellnopt.core.reactions.Reactions.add_reaction`. 
            However, attributes such as :attr:`~cellnopt.core.reactions.Reactions.reacBoxes` 
        ...
        """
        super(Reactions, self).__init__(strict_rules=False)
...
first column after pipe character (%s)""" % (str(i+1), parameters[0]))
    ...
    def write2sif(self, filename=None):
        #__doc__ += self.writeSIF.__doc__
        self.writeSIF(filename=filename)
