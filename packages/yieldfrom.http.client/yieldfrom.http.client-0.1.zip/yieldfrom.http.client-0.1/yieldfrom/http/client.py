...
class HTTPResponse(io.IOBase): #io.BufferedIOBase):
    ...
    def _read_status(self):
        try:
            print('httpc._read_status now. sock %s timeout %s' % (self.soCk, self.TIMEOUT), file=sys.stderr)
    ...
    def begin(self):
        ...
        while True:
            print('httpc.begin() _read_status now. sock %s' % self.soCk, file=sys.stderr)
            ...
            while True:
                try:
                    print('httpc.begin() readline now. sock %s' % self.soCk, file=sys.stderr)
        ...
        print('httpc.begin() parse_headers now. sock %s' % self.soCk, file=sys.stderr)
    ...
    def _close_conn(self):
        print('closing conn ', self.fp, file=sys.stderr)
        if self.fp:
            print('buffer len ', len(self.fp._buffer))
        #raise Exception()
        fp = self.fp
        self.fp = None
        if type(fp) == asyncio.StreamReader:
            fp.feed_eof()
            fp._buffer = b''
        else:
            fp.close()
    ...
    def read(self, amt=None):
        print('resp.read %s. sock %s' % (amt, self.soCk))
        if self.fp is None:
            return b""
        ...
        if amt is not None:
            print('resp.read %s amt. sock %s' % (amt, self.soCk))
            # Amount is given, implement using readinto
            b = bytearray(amt)
            n = yield from self.readinto(b)
            return memoryview(b)[:n].tobytes()
        else:
            print('resp.read %s no-amt %s' % (amt, self.soCk))
            # Amount is not given (unbounded read) so we must check self.length
            # and self.chunked

            if self.chunked:
                print('resp.read chunked. sock %s' % self.soCk)
                return (yield from self._readall_chunked())

            if self.length is None:
                print('resp.read no len. sock %s' % self.soCk)
                s = yield from asyncio.wait_for(self.fp.read(), self.TIMEOUT)
            else:
                print('resp.read len %s  sock %s' % (self.length, self.soCk))
                try:
                    s = yield from self._safe_read(self.length)
                except IncompleteRead:
                    print('resp.read incomplete close_conn. sock %s' % self.soCk)
                    self._close_conn()
                    raise
                self.length = 0

            print('resp.read close_conn')
    ...
    def _safe_read(self, amt):
        ...
        while amt > 0:
            ...
            print('_safe_read amt %s, chunk sz %s' % (amt, len(chunk)))
            if not chunk:
...
class HTTPConnection:
    ...
    def connect(self):
        ...
        print('HttpCon.connect() host %s sock %s' % (host, self.soCk), file=sys.stderr)
    ...
    def close(self):
        ...
        print('httpcon close() called. %s' % self.soCk, file=sys.stderr)
        if self.soCk:
            fNo = self.soCk.fileno()
            if fNo > -1:
                self.loop.remove_reader(fNo)
                self.loop.remove_writer(fNo)
            print('httpcon close closing socket. sock %s' % self.soCk, file=sys.stderr)
    ...
    def send(self, data):
        ...
        if self.soCk is None:
            if self.auto_open:
                print('HttpC.send() sock %s' % self.soCk, file=sys.stderr)
        ...
        if hasattr(data, "read") :
            ...
            while 1:
                ...
                print('HttpC.send() 2 sock %s ' % self.soCk, file=sys.stderr)
                yield from self.loop.sock_sendall(self.soCk, datablock)
            return
        try:
            print('HttpC.send() 3 sock %s ' % self.soCk, file=sys.stderr)
            yield from self.loop.sock_sendall(self.soCk, data)
        except TypeError:
            if isinstance(data, collections.Iterable):
                for d in data:
                    print('HttpC.send() E sock %s ' % self.soCk, file=sys.stderr)
                    yield from self.loop.sock_sendall(self.soCk, d)
            else:
                raise TypeError("data should be a bytes-like object "
                                "or an iterable, got %r" % type(data))

        print('HttpC.send() returning. sock %s ' % self.soCk, file=sys.stderr)
    ...
    def _send_output(self, message_body=None):
        ...
        if isinstance(message_body, bytes) and len(message_body) < self.mss:
            msg += message_body
            message_body = None
        print('HttpC._send_output() sock %s' % self.soCk, file=sys.stderr)
        ...
        print('HttpC._send_output() returning.  sock %s' % self.soCk, file=sys.stderr)
    ...
    def endheaders(self, message_body=None):
        ...
        if self.__state == _CS_REQ_STARTED:
            ...
            raise CannotSendHeader()
        print('HttpC.endheaders() sock %s' % self.soCk, file=sys.stderr)
        yield from self._send_output(message_body)
        print('HttpC.endheaders() returning. sock %s' % self.soCk, file=sys.stderr)


    @asyncio.coroutine
    def request(self, method, url, body=None, headers={}):
        """Send a complete request to the server."""
        print('httpcl.request() url %s sock %s' % (url, self.soCk), file=sys.stderr)
        yield from self._send_request(method, url, body, headers)
        print('httpcl.request() returning.  sock %s' % self.soCk, file=sys.stderr)
        pass
    ...
    def _send_request(self, method, url, body, headers):
        ...
        print('HttpC._sendrequest() sock %s' % self.soCk, file=sys.stderr)
    ...
    def getresponse(self):
        ...
        if self.debuglevel > 0:
            ...
            response = self.response_class(self.soCk, method=self._method)
        print('httpc.getresponse() init() now. sock %s' % self.soCk, file=sys.stderr)
        yield from response.init()

        print('httpc.getresponse() begin() now. sock %s' % self.soCk, file=sys.stderr)
        ...
        print('httpc.getresponse() returning. sock %s' % self.soCk, file=sys.stderr)
        return response
...
try:
    ...
    class HTTPSConnection(HTTPConnection):
        ...
        def connect(self):
            ...
            print('HttpsCon.connect() ', file=sys.stderr)
            if self._tunnel_host:
            ...
            print('HttpsCon.connect() sock %s' % self.soCk, file=sys.stderr)
            if self._tunnel_host:
                yield from self._tunnel()
                self.auto_open = 0
            ...
            if not self._context.check_hostname and self._check_hostname:
                try:
                    print('HttpsCon.connect() match_hostname() sock %s' % self.soCk, file=sys.stderr)
                    ssl.match_hostname(self.soCk.getpeercert(), server_hostname)
                except Exception as e:
                    print('HttpsCon.connect() calling close()', file=sys.stderr)
                    self.close()
                    raise

            print('HttpsCon.connect() returning', file=sys.stderr)
