...
class DataStream(object):
    ...
    def empty(self, consumer):
        return self._streams.get(consumer).empty()

    def get(self, consumer):
        return self._streams.get(consumer).get()
    ...
    def register_consumer(self, consumer):
        assert consumer not in self._streams
        self._streams[consumer] = Queue.Queue()
...
class OutputNode(DataStream):
    ...
    def start_computations(self, do_not_reraise_exceptions=True):
        ...
        results = [self.input_.get(self) for _ in range(self.size())]
    ...
    def _start_computation(self):
        ...
        while manager.input_data_pending(self.size()):
            ...
            for i, stream in enumerate(self.input_streams):
                if not stream.empty(self):
                    item = stream.get(self)
...
class ZipComputation(Computation):
    ...
    def _start_computation(self):
        ...
        if not self._check_and_forward_exceptions(cip):
            return
    ...
    def _collect_data_from_constant_input_streams(self):
        ...
        for i, stream in enumerate(self.input_streams):
            if stream.size() == 1:
                constant_inputs[i] = stream.get(self)
        return constant_inputs

    def _check_and_forward_exceptions(self, args):
        for item in args:
            if item is not None:
                if isinstance(item.value, DelayedException):
                    self.put(item)
        ...
        while n < self.size():
            ...
            if avail:
                for number, args in self._assemble_function_arguments(input_matrix, seen):
                    if not self._check_and_forward_exceptions(args):
                        return
                    Engine.run_async(self.f, args, callback=wrap_callback(number, self.put))
                    n += 1
            else:
