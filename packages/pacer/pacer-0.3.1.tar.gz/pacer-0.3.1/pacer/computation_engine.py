...
def remote_eval(f_s, args_s):
    ...
    try:
        ...
        get_logger(remote_eval).error("got exception when unpicling args: {}".format(e))
        return Exception(e.message)
    try:
        f = dill.loads(f_s)
    except Exception, e:
        get_logger(remote_eval).error("got exception when unpicling f: {}".format(e))
        return Exception(e.message)
    try:
...
class Engine(object):
    ...
    def run_async(f, enumerated_args, callback):
        ...
        if Engine.pool is None:
            ...
            Engine.pool.apply_async(remote_eval, (f_s, args_s),
