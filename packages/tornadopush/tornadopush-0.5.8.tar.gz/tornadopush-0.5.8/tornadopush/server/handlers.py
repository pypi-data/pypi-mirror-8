...
class BaseMixin(PresenceMixin, ChannelMixin, AuthentifyMixin, RedisMixin):
    @property
...
class SSEHandler(BaseMixin, tornado.web.RequestHandler):
    ...
    def on_redis_message(self, msg):
        if msg.kind == 'disconnect':
            ...
            self.finish()
            return
        super(SSEHandler, self).on_redis_message(msg)
...
class WebSocketHandler(BaseMixin, WebSocketMixin, tornado.websocket.WebSocketHandler):
    ...
    def on_redis_message(self, msg):
        if msg.kind == 'disconnect':
            ...
            self.on_close()
            return
        super(WebSocketHandler, self).on_redis_message(msg)
