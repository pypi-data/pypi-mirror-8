...
(function(tp) {
    ...
    tp.WebSocketChannelListener = (function() {
        ...
        _.prototype.send = function(data) {
            this.socket.send(tp.encode_data(data));
            debug("Data submitted");
        };
    ...
    tp.init = function (hostname, secured) {
        tp.hostname = hostname || TORNADOPUSH_HOSTNAME || 'localhost:8888';
        tp.secured = secured || TORNADOPUSH_SECURED || false;
        tp.ws_support = typeof(WebSocket) !== 'undefined';
        if (TORNADOPUSH_TOKEN) {
    ...
    tp.subscribe = function (opts) {
        ...
        if (opts['sse'] || !tp.ws_support) {
            ...
            path += '.sse' + qp;
            return new tp.SSEChannelListener(tp.url(path), presence);
        }
        return tp.WebSocketChannelListener.from_url(
