...
def link_develop(local_sdk):
	print 'preparing for access from python modules ...'
	setup_stub = 'stub_setup.py'
	cwd = os.getcwd()
	os.chdir(local_sdk)
	stub = resource_stream(__name__, setup_stub).read()
	open(os.path.join(local_sdk, setup_stub), 'w').write(stub)
	d = run_setup(setup_stub, ['develop'])
	os.chdir(cwd)
		...
		if cmd == 'upgrade':
			...
			if projpath:
				...
				if cmd in self.commands:
					...
					print "inexistent command %s" % cmd
			else:
				print "your current working directory should be in a gae project"
