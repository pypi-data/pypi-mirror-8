...
from _autoreload import deep_reload
...
def service(urlpath=None, auto_parse=True, static=False, accel=False,
    ...
    def decorator(f):
        ...
        modules_autoreload = conf.MODULES_AUTORELOAD

        if srv_path in registry:
            if force_define is True or modules_autoreload:
                logger.debug("redefine service '%s' %s" % (srv_path, f))
            else:
        ...
        def wrapper(*args, **argd):
            ...
            if modules_autoreload:
                for module in modules_autoreload:
                    deep_reload(module)

            _state_ = get_state()

            if ssl is not False and (conf.USE_SSL or ssl) and not _state_.ssl:
