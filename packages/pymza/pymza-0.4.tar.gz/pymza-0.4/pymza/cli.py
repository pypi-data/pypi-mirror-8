...
def create_topics(repo, zookeeper, partitions, replication_factor):
    topics = set()
...
@pass_repo
@click.argument('task')
def offsets(repo, task):

    task = repo.load_task(task)
    from .state import StateManager
    state_manager = StateManager(repo.state_dir())
...
def topic_tail(topic):
    ...
    # To consume messages
    consumer = SimpleConsumer(kafka, "my-group", str(topic), auto_commit=False, max_buffer_size=10*1024*1024)
    consumer.provide_partition_info()
    consumer.seek(0, 0)
    click.echo("Tailing {0}, {1} messages pending".format(topic, consumer.pending()))
