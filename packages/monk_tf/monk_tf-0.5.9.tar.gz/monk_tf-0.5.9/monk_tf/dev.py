...
class WrongNameException(DeviceException):
    ...
    """
    pass
class NotReadyException(DeviceException):
    """ is raised when no connection was able to establish a connection to the device
    """
    pass
...
class Device(object):
    ...
    def wait_for_ready(self):
        self.log("wait for system ready")
        for c in self.conns:
            try:
                self.log("wait for prompt on connection " + str(c.name))
                c.wait_for_prompt()
                self.log("ready")
                return
            except conn.NoPromptException as e:
                self.log("Timeout Exceeded, try next connection")
        raise NotReadyException("No connection was able to successfully wait for a prompt")
