    ...
    def createInput(self, index, name, signalChangeDelta=0):
        with self._lock:
            ...
            if not item:
                ...
                self.logger.info("input %s created" % item.name)
            return item
    ...
    def createOutput(self, index, name, signalChangeDelta=0, unit=0xFF):
        with self._lock:
            ...
            if not item:
                ...
                self.logger.info("output %s created" % item.name)
    ...
    def _manager(self):
        ...
        while not self._eventStop.isSet():
            try:
                ...
                self.logger.error("manager thread exception occured within manager()")
            time.sleep(0.2)
