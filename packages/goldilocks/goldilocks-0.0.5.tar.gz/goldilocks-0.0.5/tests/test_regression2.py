...
class TestGoldilocksRegression_NucleotideCounter(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        #TODO Relies on dict ordering...
        cls.EXPECTED_REGION_CHR_ORDER = [2, "X", "one", "three"]
        cls.EXPECTED_REGION_COUNT_CHR = [8, 13, 7, 1]

    #TODO Relies on dict ordering...
    def __from_region(self, region_i):
        if region_i <= 7:
            return 2, region_i
        elif region_i <= 20:
            return "X", region_i - 8
        elif region_i <= 27:
            return "one", region_i - 21
        elif region_i <= 29:
            return "three", region_i - 28
        ...
        for group in self.g.group_counts:
            for track in self.g.group_counts[group]:
                for region_i, value in enumerate(self.g.group_counts[group][track]):
                    chrom, ichr = self.__from_region(region_i)
