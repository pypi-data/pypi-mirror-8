...
static void _cffi_check__FOURD_STATEMENT(FOURD_STATEMENT *p)
{
  /* only to generate compile-time warnings or errors */
  { FOURD * *tmp = &p->cnx; (void)tmp; }
  { char(*tmp)[2048] = &p->query; (void)tmp; }
  (void)((p->nb_element) << 1);
...
static void _cffi_check__FOURD_TIMESTAMP(FOURD_TIMESTAMP *p)
{
  /* only to generate compile-time warnings or errors */
  (void)((p->year) << 1);
  { char *tmp = &p->mounth; (void)tmp; }
  { char *tmp = &p->day; (void)tmp; }
  (void)((p->milli) << 1);
}
static PyObject *
...
_cffi_f_resultTypeFromString(PyObject *self, PyObject *arg0)
{
  ...
  return _cffi_from_c_deref((char *)&result, _cffi_type(9));
}
...
PyMODINIT_FUNC
PyInit__Py4d_cffi_709cc20dxa1c3e6fa(void)
{
  PyObject *lib;
...
PyMODINIT_FUNC
init_Py4d_cffi_709cc20dxa1c3e6fa(void)
{
  PyObject *lib;
  lib = Py_InitModule("_Py4d_cffi_709cc20dxa1c3e6fa", _cffi_methods);
  if (lib == NULL)
    return;
  if (0 < 0 || _cffi_init() < 0)
    return;
