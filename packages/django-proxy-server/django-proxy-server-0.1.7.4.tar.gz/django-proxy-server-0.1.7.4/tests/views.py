...
def test(request):
    ...
    return invoke_backend_service_as_proxy('POST', service_url, json_data={ 'email':'admin@test.com', 'password':'password' }, request=request)


@expose_service(['GET'], public=True)
def test_error(request):
    service_url = generate_service_url('/login')
    return invoke_backend_service_as_proxy('POST', service_url, json_data={ 'email':'admin@test.com' }, request=request)
