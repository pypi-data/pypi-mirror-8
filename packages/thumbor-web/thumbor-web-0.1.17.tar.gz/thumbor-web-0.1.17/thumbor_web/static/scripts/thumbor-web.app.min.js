var BreakpointsCtrl, ContributorsCount, Ctrl, HeaderCtrl, TestimonialsCtrl, TestimonialsMobileCtrl,
...
HeaderCtrl = (function(_super) {
  ...
  HeaderCtrl.prototype.bindEvents = function() {
    ...
    this.elements.gettingStarted.on('click', (function(_this) {
      return function(ev) {
        var position;
        position = _this.gettingStartedAnchorTop - _this.anchorOffset;
        if (_this.elements.body.is('.breakpoint-1')) {
          position = _this.gettingStartedAnchorTop - 10;
        }
    ...
    this.elements.contents.on('click', (function(_this) {
      return function(ev) {
        if (_this.isLocked) {
    ...
    return this.elements.burger.on('click', (function(_this) {
      return function(ev) {
        ev.preventDefault();
        ev.stopPropagation();
        return _this.toggleNavigation();
  ...
  HeaderCtrl.prototype.toggleNavigation = function() {
    this.elements.contents.toggleClass('hidden');
    this.elements.mobileNav.toggleClass('visible');
    if (this.isLocked) {
      setTimeout((function(_this) {
        return function() {
          return _this.elements.body.toggleClass('locked');
        };
      })(this), 300);
    } else {
      this.elements.body.toggleClass('locked');
    }
...
TestimonialsMobileCtrl = (function(_super) {
  __extends(TestimonialsMobileCtrl, _super);

  function TestimonialsMobileCtrl() {
    this.handleScrollEnd = __bind(this.handleScrollEnd, this);
    this.handleScrollStart = __bind(this.handleScrollStart, this);
    return TestimonialsMobileCtrl.__super__.constructor.apply(this, arguments);
  }

  TestimonialsMobileCtrl.prototype.gatherElements = function() {
    var i, lis, _i, _ref;
    this.testimonialsScroll = new IScroll(this.element[0], {
      scrollX: true,
      scrollY: false,
    ...
    this.elements.pagerList = $('.testimonials-pager ul');
    this.elements.testimonials = this.element.find('li');
    lis = [];
    for (i = _i = 1, _ref = this.testimonialsScroll.pages.length; 1 <= _ref ? _i <= _ref : _i >= _ref; i = 1 <= _ref ? ++_i : --_i) {
      lis.push('<li class="ib"></li>');
    }
    this.elements.pagerList.html(lis.join(''));
    this.elements.pagerItems = this.elements.pagerList.find('li');
    this.elements.pagerItems.eq(0).addClass('selected');
    this.elements.testimonials.eq(0).addClass('selected');
    return this.testimonialsScroll.goToPage(0, 0, 0);
  };

  TestimonialsMobileCtrl.prototype.bindEvents = function() {
    this.testimonialsScroll.on('scrollStart', this.handleScrollStart);
    this.testimonialsScroll.on('scrollEnd', this.handleScrollEnd);
    return this.elements.pagerItems.on('click', (function(_this) {
      return function(ev) {
        var item;
        item = $(ev.currentTarget);
        _this.elements.pagerItems.removeClass('selected');
        return _this.testimonialsScroll.goToPage(item.index(), 0);
      };
    })(this));
  };

  TestimonialsMobileCtrl.prototype.handleScrollStart = function() {
    return this.elements.pagerItems.removeClass('selected');
  };

  TestimonialsMobileCtrl.prototype.handleScrollEnd = function() {
    var currPage;
    currPage = this.testimonialsScroll.currentPage.pageX;
    this.elements.testimonials.removeClass('selected').eq(currPage).addClass('selected');
    return this.elements.pagerItems.eq(currPage).addClass('selected');
  };

  return TestimonialsMobileCtrl;
