...
def _cert_info(certdata):
    ...
    return dict(
        ...
        subject_id=_decode(hexlify(tc.subject_id())),
...
class hdimage(object):
    ...
    def __init__(self, name):
        ...
        self.configfile = '/var/opt/cprocsp/users/{0}/local.ini'.format(
            self.user)
        ...
        try:
            self.ctx = csp.Crypt(
                self.cont, csp.PROV_GOST_2001_DH, csp.CRYPT_SILENT)
        except ValueError:
            try:
                self.ctx = csp.Crypt(
        ...
        if self.shortcut:
            ...
            self.dirname = '/var/opt/cprocsp/keys/{0}/{1}'.format(
    ...
    def initialize(self):
        ...
        if not self.config.has_section(self.section):
            ...
            cfg.set(self.section, 'shortcut', '"{0}"'.format(
    ...
    def restore(cls, filename, filetype=None, paswd=None, force=False):
        ...
        try:
            res.ctx = csp.Crypt(
                res.cont, csp.PROV_GOST_2001_DH, csp.CRYPT_SILENT)
            if paswd:
