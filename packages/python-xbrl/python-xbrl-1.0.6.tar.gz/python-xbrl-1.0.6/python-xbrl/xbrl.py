...
class XBRLParser(object):
    ...
    def parse(self, file_handle):
        ...
        #for line in xbrl_file.fh:
        #    cool = line.replace("\n", "")
        #    print cool
        #    root = ET.fromstring(cool)
        #    print root.tag

        output = ''.join(xbrl_file.fh.readlines()).replace("\n", "")
        
        xbrl = ET.fromstring(output)

        for element in xbrl.iter():
            if "http://fasb.org/us-gaap/2013-01-31" in element.tag:
