...
def server_mongo(card='lo'):
    ...
    try:
        buf = pkg_resources.resource_string('fabez', 'tpl/mongodb_repo.conf')
    except:
        buf = open(os.path.join(os.path.dirname(__file__), 'tpl', 'mongodb_repo.conf')).read()
        pass

    with tempfile.NamedTemporaryFile('w', delete=False) as fh:
        print>> fh, buf

    put(fh.name, '/etc/yum.repos.d/mongodb.conf')
    os.remove(fh.name)
