...
//        }
    ...
    function add_tag_handlers(tag, handler) {
        tag = tag.toUpperCase();

        if (!_tag_handlers[tag]) {
            _tag_handlers[tag] = [];
        }
        _tag_handlers[tag].push(handler);
    }
...
//                }
//
                var custom_h = _tag_handlers[created.tagName];

                if (custom_h) {
                    for (var i = 0; i < custom_h.length; i++) {
                        custom_h.call(created);
                    }
                }
    ...
    function wombat_init(replay_prefix, capture_date, orig_scheme, orig_host, timestamp, mod) {
        ...
        this.add_tag_handlers = add_tag_handlers;
    }
