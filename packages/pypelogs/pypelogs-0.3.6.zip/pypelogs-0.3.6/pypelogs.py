...
def main():
    ...
    parser.add_argument("-x", "--exec", help="A config file to execute before running.")
    ...
    if args.exec:
        LOG.info("Running config file %s" % args.exec)
        exec(compile(open(args.exec, "rb").read(), args.exec, 'exec'), globals())
