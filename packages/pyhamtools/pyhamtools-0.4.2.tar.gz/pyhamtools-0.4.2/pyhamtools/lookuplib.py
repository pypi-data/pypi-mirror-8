...
class LookupLib(object):
    ...
    """
    def __init__(self, lookuptype = "countryfile", apikey=None, filename=None, logger=None, username=None, pwd=None, redis_instance=None, redis_prefix=None):
    ...
    def _request_callsign_info_from_qrz(self, callsign, apikey):
        qrz_api_version = "1.3.3"
        url = "https://xmldata.qrz.com/xml/" + qrz_api_version + "/"
    ...
    def _request_dxcc_info_from_qrz(self, dxcc_or_callsign, apikey):
        qrz_api_version = "1.3.3"
        url = "https://xmldata.qrz.com/xml/" + qrz_api_version + "/"
    ...
    def _lookup_qrz_dxcc(self, dxcc_or_callsign, apikey):
    ...
    def _lookup_qrz_callsign(self, callsign=None, apikey=None):
        ...
        if root.callsign.call:
            lookup['call'] = root.callsign.call.text
        if root.callsign.xref:
            lookup['xref'] = root.callsign.xref.text
        if root.callsign.aliases:  
            lookup['aliases'] = root.callsign.aliases.text.split(',')
        if root.callsign.dxcc:
            lookup[const.ADIF] = int(root.callsign.dxcc.text)
        if root.callsign.fname:
            lookup['fname'] = root.callsign.fname.text
        if root.callsign.find("name"):
            lookup['name'] = root.callsign.find('name').get_text()
        if root.callsign.addr1: 
            lookup['addr1'] = root.callsign.addr1.text
        if root.callsign.addr2: 
            lookup['addr2'] = root.callsign.addr2.text
        if root.callsign.state:
            lookup['state'] = root.callsign.state.text
        if root.callsign.zip:
            lookup['zip'] = root.callsign.zip.text
        if root.callsign.country:
            lookup[const.COUNTRY] = root.callsign.country.text
        if root.callsign.ccode: 
            lookup['ccode'] = int(root.callsign.ccode.text)
        ...
        if root.callsign.county: 
            lookup['county'] = root.callsign.county.text
        if root.callsign.fips: 
            lookup['fips'] = int(root.callsign.fips.text) # check type
        if root.callsign.land: 
            lookup['land'] = root.callsign.land.text
        if root.callsign.efdate:
            try: 
                lookup['efdate'] = datetime.strptime(root.callsign.efdate.text, '%Y-%m-%d %H:%M:%S')
            except ValueError:
                self._logger.warning("[QRZ.com] efdate: Invalid DateTime; " + callsign)
        if root.callsign.expdate: 
            try:
                lookup['expdate'] = datetime.strptime(root.callsign.expdate.text, '%Y-%m-%d %H:%M:%S')
            except ValueError:
                self._logger.warning("[QRZ.com] expdate: Invalid DateTime; " + callsign)
        if root.callsign.p_call: 
            lookup['p_call'] = root.callsign.p_call.text
        if root.callsign.find('class'):
             lookup['class'] = root.callsign.find('class').get_text()
        if root.callsign.codes: 
            lookup['codes'] = root.callsign.codes.text
        if root.callsign.qslmgr: 
            lookup['qslmgr'] = root.callsign.qslmgr.text
        if root.callsign.email: 
            lookup['email'] = root.callsign.email.text
        if root.callsign.url: 
            lookup['url'] = root.callsign.url.text
        if root.callsign.u_views:
            lookup['u_views'] = int(root.callsign.u_views.text)
        if root.callsign.bio: 
            lookup['bio'] = root.callsign.bio.text
        if root.callsign.biodate:
            try:
                lookup['biodate'] = datetime.strptime(root.callsign.biodate.text, '%Y-%m-%d %H:%M:%S')
            except ValueError:
                self._logger.warning("[QRZ.com] biodate: Invalid DateTime; " + callsign)
        if root.callsign.image: 
            lookup['image'] = root.callsign.image.text
        if root.callsign.imageinfo:
            lookup['imageinfo'] = root.callsign.imageinfo.text
        if root.callsign.serial: 
            lookup['serial'] = long(root.callsign.serial.text)
        if root.callsign.moddate:
            try: 
                lookup['moddate'] = datetime.strptime(root.callsign.moddate.text, '%Y-%m-%d %H:%M:%S')
            except ValueError:
                self._logger.warning("[QRZ.com] efdate: Invalid DateTime; " + callsign)
        if root.callsign.MSA: 
            lookup['MSA'] = int(root.callsign.MSA.text)
        if root.callsign.AreaCode:
            lookup['AreaCode'] = int(root.callsign.AreaCode.text)
        if root.callsign.TimeZone: 
            lookup['TimeZone'] = int(root.callsign.TimeZone.text)
        if root.callsign.GMTOffset: 
            lookup['GMTOffset'] = float(root.callsign.GMTOffset.text)
        ...
        if root.callsign.born: 
            lookup['born'] = int(root.callsign.born.text)
        if root.callsign.user: 
            lookup['user'] = root.callsign.user.text
        ...
        if root.callsign.iota:
            lookup['iota'] = root.callsign.iota.text
        if root.callsign.geoloc: 
            lookup['geoloc'] = root.callsign.geoloc.text
