...
class TerminalDiffRender(object):
	...
	def getText(self,x,y,color):
		val = self.tt.getCellText(x,y)
		if (val is None):
			val = u""
		cell = DiffRender.renderCell(self.tt,x,y)
		if color:
			code = None
			...
			if (code is not None):
				if (cell.rvalue is not None):
					...
					if (cell.pvalue is not None):
						val = ((((HxOverrides.stringOrNull(self.codes.h.get(u"conflict",None)) + HxOverrides.stringOrNull(cell.pvalue)) + HxOverrides.stringOrNull(self.codes.h.get(u"modify",None))) + HxOverrides.stringOrNull(cell.pretty_separator)) + HxOverrides.stringOrNull(val))
				else:
					val = cell.pretty_value
					val = ((HxOverrides.stringOrNull(code) + HxOverrides.stringOrNull(val)) + HxOverrides.stringOrNull(self.codes.h.get(u"done",None)))
		else:
			val = cell.pretty_value
...
TerminalDiffRender = _hx_classes.registerClass(u"TerminalDiffRender", fields=[u"codes",u"tt",u"csv",u"v",u"align_columns"], methods=[u"alignColumns",u"render",u"getText",u"pickSizes"])(TerminalDiffRender)
