...
class Alignment:
	...
	def toOrder3(self):
		...
		while ((((ct_vp > 0) or ((ct_vl > 0))) or ((ct_vr > 0)))):
			ct = (ct + 1)
			if (ct > max_ct):
				haxe_Log.trace("Ordering took too long, something went wrong",_hx_AnonObject({'fileName': "Alignment.hx", 'lineNumber': 263, 'className': "Alignment", 'methodName': "toOrder3"}))
				break
			if (xp >= hp):
				xp = 0
			if (xl >= hl):
...
@_hx_classes.registerClass("CompareFlags", fields=["ordered","show_unchanged","unchanged_context","always_show_order","never_show_order","show_unchanged_columns","unchanged_column_context","always_show_header","acts","ids","columns_to_ignore"], methods=["filter","allowUpdate","allowInsert","allowDelete","getIgnoredColumns","addPrimaryKey","ignoreColumn"])
class CompareFlags:
...
@_hx_classes.registerClass("CompareTable", fields=["comp","indexes"], methods=["run","align","getComparisonState","alignCore","alignCore2","alignColumns","testHasSameColumns","hasSameColumns2","testIsEqual","isEqual2","compareCore","storeIndexes","getIndexes"])
class CompareTable:
...
@_hx_classes.registerClass("Coopy", fields=["format_preference","delim_preference","extern_preference","output_format","io","mv","status","daff_cmd"], methods=["checkFormat","setFormat","saveTable","saveText","loadTable","command","installGitDriver","coopyhx"], statics=["VERSION","compareTables","compareTables3","keepAround","cellFor","jsonToTable","main","show","jsonify"])
class Coopy:
	...
	def show(t):
		...
		while ((_g < h)):
			...
			while ((_g1 < w)):
				...
				txt = (HxOverrides.stringOrNull(txt) + " ")
			txt = (HxOverrides.stringOrNull(txt) + "\n")
		haxe_Log.trace(txt,_hx_AnonObject({'fileName': "Coopy.hx", 'lineNumber': 728, 'className': "Coopy", 'methodName': "show"}))
...
@_hx_classes.registerClass("Csv", fields=["cursor","row_ended","has_structure","delim"], methods=["renderTable","renderCell","parseTable","makeTable","parseCellPart","parseCell"])
class Csv:
...
@_hx_classes.registerClass("IndexPair", fields=["ia","ib","quality"], methods=["addColumns","indexTables","queryByKey","queryByContent","queryLocal","localKey","remoteKey","getTopFreq","getQuality"])
class IndexPair:
...
@_hx_classes.registerClass("Mover", statics=["moveUnits","move","moveWithoutExtras"])
class Mover:
...
@_hx_classes.registerClass("Table", methods=["getCell","setCell","getCellView","isResizable","resize","clear","insertOrDeleteRows","insertOrDeleteColumns","trimBlank","get_width","get_height","getData","clone"])
class Table:
	pass


@_hx_classes.registerClass("SimpleTable", fields=["data","w","h"], methods=["getTable","get_width","get_height","getCell","setCell","toString","getCellView","isResizable","resize","clear","insertOrDeleteRows","insertOrDeleteColumns","trimBlank","getData","clone"], statics=["tableToString","tableIsSimilar"], interfaces=[Table])
class SimpleTable:
...
@_hx_classes.registerClass("View", methods=["toString","equals","toDatum"])
class View:
	pass


@_hx_classes.registerClass("SimpleView", methods=["toString","equals","toDatum"], interfaces=[View])
class SimpleView:
...
@_hx_classes.registerClass("TableText", fields=["tab","view"], methods=["getCellText"])
class TableText:
