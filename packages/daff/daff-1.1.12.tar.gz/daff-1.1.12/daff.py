	...
	def pruneOrder(self,o,ref,rowlike):
		...
		while ((_g1 < _g)):
			...
			if ((unit.l < 0) and ((unit.r >= 0))):
				...
				python_lib_Builtin.len(right_units)
				right_locs.append(i)
				python_lib_Builtin.len(right_locs)
				ct = (ct + 1)
			elif ((unit.r < 0) and ((unit.l >= 0))):
				left_units.append(unit)
				python_lib_Builtin.len(left_units)
				left_locs.append(i)
				python_lib_Builtin.len(left_locs)
				ct = (ct + 1)
			elif (ct > 0):
			...
			while (((python_lib_Builtin.len(left_locs) > 0) and ((python_lib_Builtin.len(right_locs) > 0)))):
				...
				if match:
					...
					python_lib_Builtin.len(eliminate)
...
@_hx_classes.registerClass("Table", methods=["getCell","setCell","getCellView","isResizable","resize","clear","insertOrDeleteRows","insertOrDeleteColumns","trimBlank","get_width","get_height"])
class Table:
	pass


@_hx_classes.registerClass("SimpleTable", fields=["data","w","h"], methods=["getTable","get_width","get_height","get_size","getCell","setCell","toString","getCellView","isResizable","resize","clear","insertOrDeleteRows","insertOrDeleteColumns","trimBlank"], statics=["tableToString"], interfaces=[Table])
class SimpleTable:
