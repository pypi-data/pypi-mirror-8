...
class Alignment(object):
	...
	def toOrder3(self):
		...
		while ((((ct_vp > 0) or ((ct_vl > 0))) or ((ct_vr > 0)))):
			ct = (ct + 1)
			if (ct > max_ct):
				haxe_Log.trace(u"Ordering took too long, something went wrong",_hx_AnonObject({u'fileName': u"Alignment.hx", u'lineNumber': 263, u'className': u"Alignment", u'methodName': u"toOrder3"}))
				break
			if (xp >= hp):
				xp = 0
			if (xl >= hl):
...
CellInfo = _hx_classes.registerClass(u"CellInfo", fields=[u"value",u"pretty_value",u"category",u"category_given_tr",u"separator",u"pretty_separator",u"updated",u"conflicted",u"pvalue",u"lvalue",u"rvalue"], methods=[u"toString"])(CellInfo)
...
class Coopy(object):
	...
	def saveTable(self,name,t):
		...
		if (self.format_preference != u"json"):
	...
	def coopyhx(self,io):
		...
		if (python_lib_Builtin.len(args) < 2):
			...
			io.writeStderr(u"  daff [--output OUTPUT.jsonbook] a.jsonbook b.jsonbook\n")
	...
	def show(t):
		...
		while ((_g < h)):
			...
			while ((_g1 < w)):
				...
				txt = (HxOverrides.stringOrNull(txt) + u" ")
			txt = (HxOverrides.stringOrNull(txt) + u"\n")
		haxe_Log.trace(txt,_hx_AnonObject({u'fileName': u"Coopy.hx", u'lineNumber': 728, u'className': u"Coopy", u'methodName': u"show"}))
...
Coopy = _hx_classes.registerClass(u"Coopy", fields=[u"format_preference",u"delim_preference",u"extern_preference",u"output_format",u"io",u"mv",u"status",u"daff_cmd"], methods=[u"checkFormat",u"setFormat",u"saveTable",u"saveText",u"loadTable",u"command",u"installGitDriver",u"coopyhx"], statics=[u"VERSION",u"compareTables",u"compareTables3",u"keepAround",u"cellFor",u"jsonToTable",u"main",u"show",u"jsonify"])(Coopy)
...
class DiffRender(object):
	...
	def render(self,tab):
		...
		while ((_g1 < _g)):
			...
			if (txt is None):
				txt = u""
			DiffRender.examineCell(0,row,txt,u"",txt,corner,cell)
			...
			while ((_g3 < _g2)):
				c = _g3
				_g3 = (_g3 + 1)
				DiffRender.examineCell(c,row,tt.getCellText(c,row),(tt.getCellText(c,change_row) if ((change_row >= 0)) else u""),txt,corner,cell)
	...
	def completeHtml(self):
		self.text_to_insert.insert(0, u"<html>\n<meta charset='utf-8'>\n<head>\n<style TYPE='text/css'>\n")
		x = self.sampleCss()
...
DiffRender = _hx_classes.registerClass(u"DiffRender", fields=[u"text_to_insert",u"td_open",u"td_close",u"open",u"pretty_arrows"], methods=[u"usePrettyArrows",u"insert",u"beginTable",u"beginRow",u"insertCell",u"endRow",u"endTable",u"html",u"toString",u"render",u"sampleCss",u"completeHtml"], statics=[u"examineCell",u"markSpaces",u"renderCell"])(DiffRender)
...
class HighlightPatch(object):
	...
	def checkAct(self):
		...
		if (self.rowInfo.value != act):
			DiffRender.examineCell(0,0,act,u"",act,u"",self.rowInfo)

	def getPreString(self,txt):
		self.checkAct()
		if (not self.rowInfo.updated):
			return txt
		DiffRender.examineCell(0,0,txt,u"",self.rowInfo.value,u"",self.cellInfo)
...
class View(object):
	pass
View = _hx_classes.registerClass(u"View", methods=[u"toString",u"equals",u"toDatum"])(View)
...
SimpleView = _hx_classes.registerClass(u"SimpleView", methods=[u"toString",u"equals",u"toDatum"], interfaces=[View])(SimpleView)
...
Std = _hx_classes.registerClass(u"Std", statics=[u"string",u"parseInt",u"shortenPossibleNumber",u"parseFloat"])(Std)
...
class haxe_IMap(object):
	pass
haxe_IMap = _hx_classes.registerClass(u"haxe.IMap")(haxe_IMap)
...
haxe_ds_StringMap = _hx_classes.registerClass(u"haxe.ds.StringMap", fields=[u"h"], methods=[u"keys",u"iterator"], interfaces=[haxe_IMap])(haxe_ds_StringMap)
...
class TableIO(object):
	...
	@staticmethod
	def _hx_empty_init(_hx_o):		pass
TableIO = _hx_classes.registerClass(u"TableIO", methods=[u"getContent",u"saveContent",u"args",u"writeStdout",u"writeStderr",u"command",u"async",u"exists"])(TableIO)
...
class TableText(object):
	...
	def getCellText(self,x,y):
		return self.view.toString(self.tab.getCell(x,y))
...
TableText = _hx_classes.registerClass(u"TableText", fields=[u"tab",u"view"], methods=[u"getCellText"])(TableText)
...
class TerminalDiffRender(object):
	...
	def render(self,t):
		...
		codes.h[u"header"] = u"\x1B[0;1m"
		codes.h[u"spec"] = u"\x1B[35;1m"
		codes.h[u"add"] = u"\x1B[32;1m"
		codes.h[u"conflict"] = u"\x1B[33;1m"
		codes.h[u"modify"] = u"\x1B[34;1m"
		codes.h[u"remove"] = u"\x1B[31;1m"
		codes.h[u"minor"] = u"\x1B[2m"
		codes.h[u"done"] = u"\x1B[0m"
		_g = 0
		while ((_g < h)):
			y = _g
			_g = (_g + 1)
			...
			while ((_g1 < w)):
				...
				_g1 = (_g1 + 1)
				if (x > 0):
					txt = (HxOverrides.stringOrNull(txt) + HxOverrides.stringOrNull((((HxOverrides.stringOrNull(codes.h.get(u"minor",None)) + u",") + HxOverrides.stringOrNull(codes.h.get(u"done",None))))))
				...
				if (code is not None):
					if (cell.rvalue is not None):
						val = ((((((HxOverrides.stringOrNull(codes.h.get(u"remove",None)) + HxOverrides.stringOrNull(cell.lvalue)) + HxOverrides.stringOrNull(codes.h.get(u"modify",None))) + HxOverrides.stringOrNull(cell.pretty_separator)) + HxOverrides.stringOrNull(codes.h.get(u"add",None))) + HxOverrides.stringOrNull(cell.rvalue)) + HxOverrides.stringOrNull(codes.h.get(u"done",None)))
						if (cell.pvalue is not None):
							val = ((((HxOverrides.stringOrNull(codes.h.get(u"conflict",None)) + HxOverrides.stringOrNull(cell.pvalue)) + HxOverrides.stringOrNull(codes.h.get(u"modify",None))) + HxOverrides.stringOrNull(cell.pretty_separator)) + HxOverrides.stringOrNull(val))
					else:
						val = cell.pretty_value
						val = ((HxOverrides.stringOrNull(code) + HxOverrides.stringOrNull(val)) + HxOverrides.stringOrNull(codes.h.get(u"done",None)))
	...
	@staticmethod
	def _hx_empty_init(_hx_o):		pass
TerminalDiffRender = _hx_classes.registerClass(u"TerminalDiffRender", methods=[u"render"])(TerminalDiffRender)
...
haxe_ds_IntMap = _hx_classes.registerClass(u"haxe.ds.IntMap", fields=[u"h"], methods=[u"set",u"remove",u"keys",u"toString"], interfaces=[haxe_IMap])(haxe_ds_IntMap)
