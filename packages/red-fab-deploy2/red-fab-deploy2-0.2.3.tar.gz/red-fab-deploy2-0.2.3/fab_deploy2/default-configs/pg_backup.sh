...
#=====================================================================
#set -x
CONFIGFILE="/etc/automysqlbackup/autopgsqlbackup.conf"
...
if [ -r ${CONFIGFILE} ]; then
	...
	# to /etc/autopgsqlbackup/autopgsqlbackup.conf. After that you're able to upgrade this script
	# (copy a new version to its location) without the need for editing it.
	### START CFG ###
	# Username to access the PostgreSQL server e.g. dbuser
	USERNAME=postgres

	# Password to access the PostgreSQL server e.g. password
	PASSWORD=
	...
	DBEXCLUDE="template0 template1 postgres"
...
# it is useful to save bandwidth when backing up a remote PostgreSQL server over
# the network.
#
...
#    Compression on Backup server to PostgreSQL server communications.
# VER 2.2 - (2004-12-05)
...
LOGFILE=${BACKUPDIR}/l${DBHOST}-`${DATEC} +%N`.log		# Logfile Name
...
dbdump () {
PGPASSWORD=${PASSWORD} ${PGDUMP} --username=${USERNAME} --host=${DBHOST} --port=${PORT} ${1} > ${2}
return $?
}
...
if [ "${DBNAMES}" = "all" ]; then
        DBNAMES=$(${PSQL} -l -l -t -A -F,| ${SED} -n 's/,/,/p' | ${CUT} -d, -f1)
...
${ECHO}
${ECHO} ======================================================================
${ECHO} If you find AutoPGBackup valuable please make a donation at
