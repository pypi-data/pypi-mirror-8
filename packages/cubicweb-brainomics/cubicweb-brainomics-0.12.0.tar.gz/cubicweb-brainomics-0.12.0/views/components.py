...
class BrainomicsResultsFileBox(component.CtxComponent):
    ...
    def render(self, w, **kwargs):
        ...
        for _file in _files:
            ...
            if _file.__regid__ == 'File':
                icon = _file.view('icon')
            else:
                icon = u''
            w(u'<li>%s<a href="%s" title="%s">%s</a></li>'
              % (icon, _file.absolute_url(vid='download'),
...
class BrainomicsFilterBox(facets.FilterBox):
    ...
    def layout_widgets(self, w, wdgs):
        ...
        dfreg = self._cw.vreg.get('dynamic-facet')
        if dfreg:
            for comp in dfreg.possible_objects(self._cw, rset=self.cw_rset):
                comp.render(w=w)
