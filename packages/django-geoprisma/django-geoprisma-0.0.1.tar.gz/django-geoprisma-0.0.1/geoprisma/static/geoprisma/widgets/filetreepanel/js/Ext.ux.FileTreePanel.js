...
Ext.ux.FileTreePanel = Ext.extend(Ext.tree.TreePanel, {
	...
	/**
	 * @cfg {String} the number of clicks required to download a file from  
         *               a node.  Can be either 'single', 'double' or null.  
         *               Defaults to 'single'.
	 */
	,downloadClick:'single'
	// }}}
	...
	,initComponent:function() {
		...
		// install event handlers
			...
			,nodedragover:{scope:this, fn:this.onNodeDragOver}
                ...
                if (this.downloadClick == 'single') {
                    callbackHash.click = {scope:this, fn:this.onDblClick}
                } else if (this.downloadClick == 'double') {
                    callbackHash.dblclick = {scope:this, fn:this.onDblClick}
                }
		this.on(callbackHash);
...
//				,menu:this.getContextMenu()
	...
	,deleteNode:function(node) {
		...
		Ext.Msg.show({
			...
			,fn:function(response) {
                ...
                if(node.attributes.osmresource) {
                    options.params.osmresource = node.attributes.osmresource;
                }
	 ...
	 */
	,downloadFile:function(path,resourcename) {
		...
		// append osmresource to form
                if(resourcename) {
                    hidden = document.createElement('input');
                    hidden.type = 'hidden';
                    hidden.name = 'osmresource';
                    hidden.value = resourcename;
                    form.appendChild(hidden);
                }
	...
	,openNode:function(node, mode) {
		...
		if(node.isLeaf()) {
			...
			switch(mode) {
				...
				case 'download':
                                  if(node.attributes.osmresource)
                                      this.downloadFile(path, node.attributes.osmresource);
                                  else
