 ...
 * contents of an "event descriptor", see {@link Ext4.ux.event.Recorder}. The events recorded by the
 * {@link Ext4.ux.event.Recorder} class are designed to serve as input for this class.
 * 
 * The simplest use of this class is to instantiate it with an {@link #eventQueue} and call
 * {@link #method-start}. Like so:
 *
 *      var player = Ext4.create('Ext4.ux.event.Player', {
 *          eventQueue: [ ... ],
 ...
 *      }
 */
Ext4.define('Ext4.ux.event.Player', {
    extend: 'Ext4.ux.event.Driver',
    ...
    constructor: function (config) {
        ...
        me.addEvents(
             ...
             * @param {Ext4.ux.event.Player} this
        ...
        me.eventObject = new Ext4.EventObjectImpl();
    ...
    nextEvent: function (eventDescriptor) {
        ...
        if (me.keyFrameEvents[eventDescriptor.type]) {
            Ext4.Array.insert(me.eventQueue, index, [
    ...
    peekEvent: function () {
        ...
        if (type == 'mduclick') {
            tmp = [
                Ext4.applyIf({ type: 'mousedown' }, eventDescriptor),
                Ext4.applyIf({ type: 'mouseup' }, eventDescriptor),
                Ext4.applyIf({ type: 'click' }, eventDescriptor)
            ];
    ...
    replaceEvent: function (index, events) {
        ...
        Ext4.Array.replace(this.eventQueue, index, 1, events);
    },
    ...
    processEvents: function () {
        var me = this,
            animations = me.pauseForAnimations && me.attachTo.Ext4.fx.Manager.items,
            eventDescriptor;
     ...
     * @param {Ext4.EventObject} The event to inject.
     */
    ...
    translateEvent: function (eventDescriptor, target) {
        ...
        if ('x' in eventDescriptor) {
            event.xy = xy = Ext4.fly(target).getXY();
