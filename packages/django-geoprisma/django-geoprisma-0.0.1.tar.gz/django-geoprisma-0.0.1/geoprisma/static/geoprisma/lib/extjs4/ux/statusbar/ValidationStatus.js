/**
 * A {@link Ext4.ux.statusbar.StatusBar} plugin that provides automatic error
 * notification when the associated form contains validation errors.
 */
Ext4.define('Ext4.ux.statusbar.ValidationStatus', {
    extend: 'Ext4.Component', 
    requires: ['Ext4.util.MixedCollection'],
    /**
     * @cfg {String} errorIconCls
     * The {@link Ext4.ux.statusbar.StatusBar#iconCls iconCls} value to be applied
    ...
    /**
     * @cfg {String} showText
     * The {@link Ext4.ux.statusbar.StatusBar#text text} value to be applied when
     * there is a form validation error.
     */
    showText : 'The form has errors (click for details...)',
    /**
     * @cfg {String} hideText
     * The {@link Ext4.ux.statusbar.StatusBar#text text} value to display when
    ...
    init : function(sb){
        sb.on('render', function(){
            this.statusBar = sb;
            this.monitor = true;
            this.errors = Ext4.create('Ext4.util.MixedCollection');
            this.listAlign = (sb.statusAlign === 'right' ? 'br-tr?' : 'bl-tl?');
            
            if (this.form) {
                this.formPanel = Ext4.getCmp(this.form);
    ...
    getMsgEl : function(){
        if(!this.msgEl){
            this.msgEl = Ext4.DomHelper.append(Ext4.getBody(), {
            ...
            this.msgEl.on('click', function(e){
                ...
                if(t){
                    Ext4.getCmp(t.id.split('x-err-')[1]).focus();
                    this.hideErrors();
                }
