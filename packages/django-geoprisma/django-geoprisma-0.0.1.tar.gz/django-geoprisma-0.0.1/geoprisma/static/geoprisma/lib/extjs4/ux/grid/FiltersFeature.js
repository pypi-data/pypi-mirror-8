/**
 * FiltersFeature is a grid {@link Ext4.grid.feature.Feature feature} that allows for a slightly more
 ...
 * {@link Ext4.data.Store}), and Boolean. Additional custom filter types and menus are easily
 * created by extending {@link Ext4.ux.grid.filter.Filter}.
 *
 ...
 *
 * The filter collection binds to the {@link Ext4.grid.Panel#beforestaterestore beforestaterestore}
 * and {@link Ext4.grid.Panel#beforestatesave beforestatesave} events in order to be stateful.
 *
 * ##GridPanel Changes:##
 *
 ...
 *          height: 400,
 *          width: 700,
 *          bbar: Ext4.create('Ext4.PagingToolbar', {
 *              store: store
 *          })
 *     });
 *
 ...
 *     grid.filters
 */
Ext4.define('Ext4.ux.grid.FiltersFeature', {
    extend: 'Ext4.grid.feature.Feature',
    alias: 'feature.filters',
    uses: [
        'Ext4.ux.grid.menu.ListMenu',
        'Ext4.ux.grid.menu.RangeMenu',
        'Ext4.ux.grid.filter.BooleanFilter',
        'Ext4.ux.grid.filter.DateFilter',
        'Ext4.ux.grid.filter.ListFilter',
        'Ext4.ux.grid.filter.NumericFilter',
        'Ext4.ux.grid.filter.StringFilter'
    ],
    ...
    /**
     * @cfg {Boolean} encode
     * Specify true for {@link #buildQuery} to use Ext4.util.JSON.encode to
    ...
    /**
     * @cfg {Boolean} local
     * <tt>true</tt> to use Ext4.data.Store filter functions (local filtering)
    ...
    constructor : function (config) {
        ...
        me.deferredUpdate = Ext4.create('Ext4.util.DelayedTask', me.reload, me);
    ...
    createFiltersCollection: function () {
        return Ext4.create('Ext4.util.MixedCollection', false, function (o) {
    ...
    createFilters: function() {
        ...
        Ext4.Array.each(me.filterConfigs, function (filterConfig) {
        ...
        Ext4.Array.each(grid.columns, function (column) {
    ...
    initializeFilters: function(filters) {
        ...
        for (i = 0; i < filtersLength; i++) {
            ...
            if (filter) {
                ...
                Ext4.util.Observable.capture(filter, this.onStateChange, this);
            }
        }
    },
    ...
    destroy : function () {
        var me = this;
        Ext4.destroyMembers(me, 'menuItem', 'sep');
        me.removeAll();
        me.clearListeners();
    },
    ...
    removeAll : function () {
        if(this.filters){
            Ext4.destroy.apply(Ext4, this.filters.items);
     ...
     * @param {Ext4.data.Store} store The store to bind to this view
     */
    ...
    onBeforeLoad : function (store, options) {
        ...
        Ext4.apply(options.params, params);
    },
     ...
     * @param {Object/Ext4.ux.grid.filter.Filter} config A filter configuration or a filter object.
     * @return {Ext4.ux.grid.filter.Filter} The existing or newly created filter object.
     */
     ...
     * @return {Ext4.ux.grid.filter.Filter}
     */
    ...
    buildQuery : function (filters) {
        ...
        if (!this.encode){
            ...
            if (tmp.length > 0){
                p[this.paramPrefix] = Ext4.JSON.encode(tmp);
            }
        }
        return p;
    },
     ...
     * @return {Function} The Ext4.ux.grid.filter.Class
     */
    getFilterClass : function (type) {
        // map the supported Ext4.data.Field type values into a supported filter
        switch(type) {
            case 'auto':
              type = 'string';
              break;
        ...
        return Ext4.ClassManager.getByAlias('gridfilter.' + type);
    }
});
