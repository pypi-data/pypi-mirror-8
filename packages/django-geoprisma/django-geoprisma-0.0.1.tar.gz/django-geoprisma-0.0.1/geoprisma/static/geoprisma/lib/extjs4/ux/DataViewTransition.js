/**
 * @class Ext4.ux.DataViewTransition
 * @extends Object
 * @author Ed Spencer (http://sencha.com)
 * Transition plugin for DataViews
 */
Ext4.ux.DataViewTransition = Ext4.extend(Object, {
    ...
    init: function(dataview) {
        ...
        dataview.updateIndexes = Ext4.Function.createSequence(dataview.updateIndexes, function() {
            this.getTargetEl().select(this.itemSelector).each(function(element, composite, index) {
                element.id = element.dom.id = Ext4.util.Format.format("{0}-{1}", dataview.id, dataview.store.getAt(index).get(idProperty));
            }, this);
        }, dataview);
        ...
        dataview.store.on('datachanged', function(store) {
            var parentEl = dataview.getTargetEl(),
                ...
                existing = Ext4.apply({}, previous, added);
            
            //hide old items
            Ext4.each(removed, function(item) {
                Ext4.fly(this.dataviewID + '-' + item.get(this.idProperty)).animate({
                    remove  : false,
                    duration: duration,
                    opacity : 0,
                    useDisplay: true
            ...
            var el = Ext4.get(this.dataviewID + "-" + calcItem.get(this.idProperty));
            ...
            Ext4.iterate(previous, function(id, item) {
                var id = item.get(this.idProperty),
                    el = elCache[id] = Ext4.get(this.dataviewID + '-' + id);
            ...
            Ext4.iterate(previous, function(id, item) {
                ...
                if (el.getStyle('position') != 'absolute') {
                    elCache[id].applyStyles({
                        ...
                        width   : el.getWidth(!Ext4.isIE || Ext4.isStrict),
                        height  : el.getHeight(!Ext4.isIE || Ext4.isStrict)
                    });
                }
            });
            ...
            //get new positions
            var index = 0;
            Ext4.iterate(store.data.items, function(item) {
            ...
            var doAnimate = function() {
                ...
                if (fraction >= 1) {
                    for (var id in newPositions) {
                        Ext4.fly(dataviewID + '-' + id).applyStyles({
                            top : newPositions[id].top + "px",
                            left: newPositions[id].left + "px"
                        });
                    }
                    
                    Ext4.TaskManager.stop(task);
                } else {
                    //move each item
                    for (var id in newPositions) {
            ...
            if (Ext4.global.console && Ext4.global.console.log) {
                Ext4.global.console.log('added:', count);
            }
            //</debug>
            
            //show new items
            Ext4.iterate(added, function(id, item) {
                Ext4.fly(this.dataviewID + '-' + item.get(this.idProperty)).applyStyles({
     ...
     * @param {Ext4.data.Store} store The store to cache data from
     */
     ...
     * @param {Ext4.data.Store} store The updated store instance
