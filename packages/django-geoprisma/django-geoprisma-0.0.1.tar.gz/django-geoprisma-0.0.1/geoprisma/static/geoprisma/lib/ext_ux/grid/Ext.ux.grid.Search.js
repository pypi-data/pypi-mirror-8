...
Ext.extend(Ext.ux.grid.Search, Ext.util.Observable, {
	...
	/**
	 * @cfg {Boolean} readOnlyWhenLoading Disable new character typing during the loading
	 */
	readOnlyWhenLoading: true
    ...
    /**
     * APIProperty: triggerDelay
     * {Integer} Delay in ms after keyup in searchfield to trigger a new
     *           new query.
     */
    ,triggerDelay: 1000
	
	/**
        * @cfg {Boolean} useDelay use the triggerDelay to trigger the search if true (no if false)
        */
	,useDelay:false
	...
	,onRender:function() {
		...
		this.field.on('render', function() {
			...
			if(this.minChars && this.useDelay) {
				this.field.el.on({scope:this, buffer:this.triggerDelay, keyup:this.onKeyUp});
			}
		...
		if(true === this.readOnlyWhenLoading) {
			this.grid.store.on({scope:this, load:function(){this.field.setReadOnly(false); if (Ext.isIE) {this.field.focus();}}});
		}
	...
	,onKeyUp:function(e, t, o) {
		...
		// ignore special keys
		if((e.isNavKeyPress() || e.isSpecialKey()) || e.keyCode == e.BACKSPACE || e.keyCode == e.DELETE || e.keyCode == e.SPACE) {
			return;
		}
		if (this.field.getValue() == this.lastSearch) {
			return;
		}
	...
	,onTriggerSearch:function() {
		...
		this.lastSearch = this.field.value;
