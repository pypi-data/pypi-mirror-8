 ...
 */
Ext4.define('Ext4.ux.DataView.Animated', {
    ...
    init: function(dataview) {
        ...
        dataview.updateIndexes = Ext4.Function.createSequence(dataview.updateIndexes, function() {
            this.getTargetEl().select(this.itemSelector).each(function(element, composite, index) {
                element.id = element.dom.id = Ext4.util.Format.format("{0}-{1}", dataview.id, store.getAt(index).internalId);
            }, this);
        }, dataview);
        ...
        function reDraw(store) {
            ...
            Ext4.each(removed, function(item) {
                ...
                Ext4.fly(id).animate({
                    ...
                    useDisplay: true,
                    callback: function() {
                        Ext4.fly(id).setDisplayed(false);
                    }
                });
            }, this);
            ...
            var el = Ext4.get(this.dataviewID + "-" + calcItem.internalId);
            ...
            Ext4.iterate(added, function(id, item) {
                Ext4.fly(this.dataviewID + '-' + item.internalId).applyStyles({
                    top    : newPositions[item.internalId].top + "px",
                    left   : newPositions[item.internalId].left + "px"
                }).setDisplayed(true);
                
                Ext4.fly(this.dataviewID + '-' + item.internalId).animate({
                    remove  : false,
                    duration: duration,
                    opacity : 1
                });
