...
class PyscesInputFileParser(object):
    ...
    def InitialiseInputFile(self):
        ...
        if self.ModelFile[-4:] == '.psc':
            ...
            SBML2PSC.SBML2PSC(self.ModelFile,self.ModelDir)
    ...
    def InitialiseEvents(self):
        ...
        for e in self.__eDict__:
            ...
            #ev.IsTriggered = False # useful for event with time and species trigger
...
class PySCeS_Connector(PyscesInputFileParser):
    def __init__(self,ModelFile,ModelDir,IsTauLeaping = False, IsNRM = False, IsDelayed = False, IsSMM = False):
      """ 
    ...
    def BuildN(self):
        ...
        self.N_matrix = np.zeros((len(species_self),len(self.Mod.__reactions__))) #,dtype=np.int16) # TODO
    ...
    def BuildX(self):  
      ...
      self.X_matrix = np.zeros((n_species,1)) #,dtype = np.int16)   # TODO 
      for i,s_id in enumerate(self.species):          
                  ...
                  for compartment in L_compartments:
                      if self.Mod.__compartments__[compartment]['size'] != 1:
                          if self.Mod.__sDict__[s_id]['compartment'] == compartment:
                              print("Warning: Species are given in concentrations and the compartment volume is unequal to one, the species concentrations are multiplied by the compartment volume which will result in different numbers. This means that the rate equations also change.")
    ...
    def BuildReactions(self):
        ...
        for r_id in self.Mod.__reactions__:                    
            ...
            #depends_on_indices = [self.species.index(s_id) for s_id in temp_depends_on]            
            #self.depends_on.append(np.unique(depends_on_indices).tolist())
    ...
    def BuildSpeciesDependsOn(self): # 29-10-2013 -> used in Single Molecule Method
        ...
        for s_index, s_id in enumerate(self.species):    
            change = set()                        # Don't allow duplicates of a reaction             
...
class RegularGridDataObj(object):    
    ...
    def setSpeciesConcentrations(self,extracellular=[]):
        ...
        self.species_concentrations = np.array(self.species)/np.array(self.volume)
    ...
    def getVolume(self):
        ...
        output = None
        if self.HAS_VOLUME:
            output = np.hstack((self.time, self.volume))           
        else:
            output = self.time            
        return output        
    ...
    def getSpeciesConcentrations(self, lbls=False):
        ...
        if self.HAS_SPECIES_CONCENTRATIONS:
            output = np.hstack((self.time, self.species_concentrations))
            labels = [self.time_label]+self.species_labels
        else:
            output = self.time
            labels = [self.time_label]
...
class IntegrationStochasticDataObj(object):
    ...
    def setVolume(self,volume):
        ...
        Input:
         - *species* an array of volume vs time data
        """
        self.volume = np.array(volume,'d').reshape(len(volume), 1)
        self.HAS_VOLUME = True               
