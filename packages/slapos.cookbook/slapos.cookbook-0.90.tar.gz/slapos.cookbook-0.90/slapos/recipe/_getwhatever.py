...
class Recipe(GenericSlapRecipe):
    def __init__(self, buildout, name, options):
        ...
        slap.initializeConnection(options['server-url'],
                                  ...
                                  cert_file=options['cert-file'])

        partition = slap.registerComputerPartition(options['computer-id'],
                                                   options['partition-id'])

        import pdb; pdb.set_trace()
        # instance =  partition.getInformation('res-test')
        instance_parameter_dict = partition.getInstanceParameterDict()
        # self.options['ip_list'] = instance_parameter_dict['ip_list']

        # parameters = partition.getConnectionParameterDict()
        import pdb; pdb.set_trace()
        pass
