...
from os.path import abspath, join, lexists, exists, dirname, basename
...
class OpenERPEnvironment:
    def __init__(self, path=".", config_filename=defaults.config_filename, init=False, version='6.0'):
        """
        ...
        """
        if not path is None and not exists(path):
            makedirs(path)
        if exists(config_filename):
            self.config_filename = abspath(config_filename)
            self.root_path = dirname(self.config_filename)
        elif init:
            src_config_filename = defaults.src_config_filename[version]
