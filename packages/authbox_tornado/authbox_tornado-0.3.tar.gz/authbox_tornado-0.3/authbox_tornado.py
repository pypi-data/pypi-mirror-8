...
def authbox_get_local_machine_id_cookie():
  return hashlib.sha256('authbox:local_machine_id:' + AUTHBOX_CONFIG['api_key']).hexdigest()
...
def authbox(request_handler_class):
  ...
  class NewRequestHandler(request_handler_class):
    ...
    def __init__(self, *args, **kwargs):
      ...
      self._fired_action = False
      self._verdict = None
      self._authbox_pixel = random.random() < AUTHBOX_PIXEL_LIKELIHOOD

      self._authbox_local_machine_id = self.get_cookie(authbox_get_local_machine_id_cookie())
      ...
      if self._authbox_local_machine_id is None:
        ...
        self._authbox_pixel = True
        self._authbox_local_machine_id = authbox_get_unique_id()
        self.set_cookie(
          authbox_get_local_machine_id_cookie(),
    ...
    def _get_request_body(self, action, request_data):
      ...
      for remote_ip_header in AUTHBOX_IP_HEADERS:
        if remote_ip_header in self.request.headers:
          remote_ip = self.request.headers[remote_ip_header]
          break

      action.update({
        '$apiKey': AUTHBOX_CONFIG['api_key'],
        '$secretKey': AUTHBOX_CONFIG['secret_key'],
        '$localMachineID': self._authbox_local_machine_id,
        '$ipAddress': remote_ip,
    ...
    def _authbox_check(self, action):
      try:
        ...
        result = yield AUTHBOX_HTTP_CLIENT.fetch(
          AUTHBOX_CONFIG['endpoint'] + '/action',
          method='POST',
        ...
        if not result or result.code != 200 or not result.buffer:
          ...
          self._verdict = {'type': 'ALLOW', 'info': 'Error from server.'}
        else:
          self._verdict = result.buffer.read()
      except Exception, e:
        tb = traceback.format_exc()
        AUTHBOX_LOGGER.warning('Error occurred when sending check to Authbox: %s' % tb)
        self._verdict = {'type': 'ALLOW', 'info': 'Error: %s' % e}
      raise tornado.gen.Return(self._verdict)
    ...
    def authbox_check(self, action=None):
      ...
      if self._fired_action:
        AUTHBOX_LOGGER.warning('You can only fire one action per http request.')
        return self._verdict

      if random.random() < AUTHBOX_CONFIG['rollout']:
        self._fired_action = True
        return self._authbox_check(action)
      else:
        self._verdict = {'type': 'ALLOW', 'info': 'Request not sent.'}
