...
Example composition with shell script::
    ...
    beacon post distribute/email sources=${DATAMAP}, 'recpients=joe@example.com, subject="Your daily datamap"
...
Note there are some convenience features to create arrays used above: if a command line argument contains or ends with \
a comma, then it's assumed to be a comma-separated list of strings. If it starts with {} (like the content of the
DATAMAP variable) and ends with, it's also wrapped in an array.
