...
class IssueManagerTests(TestCase):
    ...
    def test_resolve_issue(self):
        name1 = 'a-name'
        name2 = 'another-name'
        i = G(Issue, name=name1, status=IssueStatus.Resolved.value)
        i2 = G(Issue, name=name2, status=IssueStatus.Resolved.value)

        Issue.objects.filter(name=name1).resolve()
        self.assertEqual(IssueStatus.Resolved.value, Issue.objects.get(pk=i.pk).status)
        self.assertEqual(IssueStatus.Open.value, Issue.objects.get(pk=i2.pk).status)
