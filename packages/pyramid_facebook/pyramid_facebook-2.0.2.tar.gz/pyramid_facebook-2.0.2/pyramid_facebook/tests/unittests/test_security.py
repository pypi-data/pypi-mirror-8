...
class TestAuthPolicy(unittest.TestCase):
    ...
    def _get_request(self, context_cls=SignedRequestContext):
        ...
        request.new_graph_api = mock.Mock()
        request.graph_api = mock.Mock()
        return request
...
class TestAccessTokenAuthenticationPolicy(TestAuthPolicy):
    ...
    def test_validate_access_token_with_no_access_token(self):
    ...
    def test_validate_access_token_with_facepy_error(self):
        from pyramid_facebook.security import (
        ...
        api = request.new_graph_api.return_value = mock.Mock()
    ...
    def test_validate_access_token_malformed_fb_response(self):
        from pyramid_facebook.security import (
            AccessTokenAuthenticationPolicy,
            AccessTokenContext,
            )
...
class TestAdminAuthenticationPolicy(TestAuthPolicy):
    ...
    def test_get_principals_from_admin_context_with_facepy_error(self):
        ...
        request.graph_api.get.side_effect = FacepyError('opps')
    ...
    def test_get_principals_from_admin_context(self):
        from pyramid_facebook.security import AdminContext
        request = self._get_request(context_cls=AdminContext)
        request.context.user_id = 1234
        request.params = {u'access_token': u'123qwerty'}

        request.graph_api.get.return_value = {
            u'data': [{
                u'user': u'1234',
