...
class TokenizeSentence(object):
    ...
    def sentence_tokenizer(self, untokenized_string, language):
        ...
        if language == 'greek':
            pickle_path = os.path.expanduser('~/cltk_data/greek/tokenize/sentence/greek.pickle')
            language_punkt_vars = PunktLanguageVars
            language_punkt_vars.sent_end_chars = ('.', ';')
            language_punkt_vars.internal_punctuation = (',', '·')
        elif language == 'latin':
            pickle_path = os.path.expanduser('~/cltk_data/latin/tokenize/sentence/latin.pickle')
            language_punkt_vars = PunktLanguageVars
            language_punkt_vars.sent_end_chars = ('.', '?', ':')
            language_punkt_vars.internal_punctuation = (',', ';')
        else:
