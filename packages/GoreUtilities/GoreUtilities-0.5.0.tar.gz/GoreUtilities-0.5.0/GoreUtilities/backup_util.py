...
def print_dictionary_as_tree(dictionary, indent=0,
    ...
    TODO: Figure out what to output when skipping (at the moment
    just outputting a ! mark.

    TODO: Allow output of html template?
    """
    for key, value in dictionary.iteritems():
        if use_title_style:
            print '\t' * indent + str(key).title(),
        else:
            print '\t' * indent + str(key),

        if isinstance(value, dict):
            print
            print_dictionary_as_tree(value, indent+1)
