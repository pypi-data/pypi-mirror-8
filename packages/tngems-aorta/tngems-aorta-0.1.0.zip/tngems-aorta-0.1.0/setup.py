...
# Dynamically calculate the version based on aorta.VERSION.
version = __import__(PACKAGE_NAME).get_version()
install_requires = ['libtng','sqlalchemy>=0.9','psycopg2>=2.5']
data_dir = join(dirname(__file__), 'share')
data_files = [(d.replace(data_dir,'share/aorta'), [join(d, f) for f in files]) for d,folders,files in os.walk(data_dir)]
...
setup(
    ...
    description="Converts database notifications to domain events.",
    license='TBA',
    packages=packages,
