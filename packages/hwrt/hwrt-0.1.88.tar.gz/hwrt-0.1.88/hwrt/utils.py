...
def evaluate_model(recording, model_folder, showit=False):
...
def classify_single_recording(raw_data_json, model_folder):
    """Get the classification as a list of tuples. The first value is the
       LaTeX code, the second value is the probability.
    """
    evaluation_file = evaluate_model(raw_data_json, model_folder)
    PROJECT_ROOT = get_project_root()
