...
def heatmap(data, ax, norm=None, cmap=None, xticks=None, yticks=None, meshopts=None):
    mesh_args = {
        'edgecolor': 'face'
    }
    xticks_args = {
    ...
    ax.set_xticklabels(data.columns, rotation=xticks_args['rotation'])

    ax.yaxis.set_ticks_position(yticks_args['pos'])
    ax.set_yticks(numpy.arange(0.5, len(data.index), 1))
    ax.set_yticklabels(data.columns, rotation=yticks_args['rotation'])
...
def heatmap_clustered(data, figsize=(10, 5), cmap=None, norm=None):
    # cmap = matplotlib.colors.ListedColormap(sns.color_palette('Set1', 4))
    # grid_figure ,wspace=0.0,hspace=0.0,width_ratios=[0.25,1]

    #the number of colors should be N-1, where N is the number of boundaries
    # norm = matplotlib.colors.BoundaryNorm([0, 1000, 2000, 7000, 10000], cmap.N)

    #or better use the Normalize class
    # norm = matplotlib.colors.Normalize(vmin=3000, vmax=7000, clip=True)
    ...
    dendr = dendrogram(mesh, fig.add_subplot(gs[1, 0]), orientation='right')
    dendc = dendrogram(mesh.T, fig.add_subplot(gs[0, 1]))
