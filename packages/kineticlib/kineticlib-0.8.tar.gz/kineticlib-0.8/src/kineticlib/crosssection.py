...
def elastic_crosssection_rigid_sphere(g: float, deg: int) -> float:
    """Calculates the velocity-dependent part of the generalized elastic omega integral for the rigid-sphere model
...
def raw_vt_integral_rigid_sphere(T: float, deg: int, sigma: float, mass: float, beta: float,
    ...
    if delta != 0:
        f = lambda g: prob.vt_prob_g_only(g, T, mass, beta, ve_before, ve_after,
                                          i, delta, molecule_vibr_zero + molecule_diss)\
                      * elastic_crosssection_rigid_sphere(g, deg)
        if delta == 1:
            mult = (i + 1)
        elif delta == -1:
            mult = i
...
def raw_vv_integral(T: float, deg: int, mass: float, beta: float, vssc: float, vsso: float,
    ...
    if i_delta != 0:
        f = lambda g: prob.vv_prob_g_only(g, T, mass, beta, ve_before1, ve_after1, ve_before2, ve_after2,
                                          i, k, i_delta) * elastic_crosssection(g, deg, vsso)
        if i_delta == 1:
            mult = (i + 1) * k
        elif i_delta == -1:
            mult = i * (k + 1)
    ...
    *raw* version
    ...
    if i_delta != 0:
        f = lambda g: prob.vv_prob_g_only(g, T, mass, beta, ve_before1, ve_after1, ve_before2,
                                          ve_after2, i, k, i_delta) * elastic_crosssection_rigid_sphere(g, deg)
        if i_delta == 1:
            mult = (i + 1) * k
        elif i_delta == -1:
            mult = i * (k + 1)
