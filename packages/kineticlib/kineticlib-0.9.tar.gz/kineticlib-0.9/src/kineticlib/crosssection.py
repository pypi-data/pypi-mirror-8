...
def crosssection_diss_rigid_sphere(g: float, T: float, idata: np.ndarray, molecule: Molecule, i: int,
    ...
    **Note** if integrating over the crosssection, the integration must be performed not from :math:`0` to
    ...
    """
    return raw_crosssection_diss_rigid_sphere(g, T, idata[1], molecule.vibr[i], molecule.diss_offset, center_of_mass,
                                              vl_dependent)
...
def min_dimensionless_vel_diss(T: float, molecule: Molecule, i: int, vl_dependent: bool=True) -> float:
    ...
    """
    return raw_min_dimensionless_vel_diss(T, molecule.vibr[i], molecule.diss_offset, vl_dependent)
...
def diss_integral_rigid_sphere(T: float, deg: int, idata: np.ndarray, molecule: Molecule, i: int,
    ...
    **AB(i) + P -> A + B + P**
    ...
    """
    return raw_diss_integral_rigid_sphere(T, deg, idata[1], idata[0], molecule.vibr[i], molecule.diss_offset,
                                          center_of_mass, vl_dependent, nokt)
...
def vv_collisions_fho(T: float, T1_1: float, T1_2: float, idata: np.ndarray,
    ...
    **M1(i) + M2(k) -> M1(i + i_delta) + M2(k - i_delta)**,
    ...
    Raises
    KineticlibError
        If ``molecule1`` are of different ``molecule2``
