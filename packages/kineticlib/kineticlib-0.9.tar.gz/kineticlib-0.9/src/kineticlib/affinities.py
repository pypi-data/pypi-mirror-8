...
def Gamma_diss(T: float, T1: float, atom1: Atom, atom2: Atom, molecule: MoleculeQuasi, n_molecule: float, n_atom1: float,
    ...
    **AB(diss_level) + P -> A + B + P** in the multi and one-temperature approximations
    ...
    if isinstance(molecule, MoleculeMultiT):
        return raw_Gamma_diss(T, T1, molecule.vibr[1], molecule.Z_int(T, T1), molecule.mass, atom1.mass, atom2.mass,
                              molecule.diss_offset, n_molecule, n_atom1, n_atom2, diss_level)
    else:
        return raw_Gamma_diss_oneT(T, molecule.Z_int(T), molecule.mass, atom1.mass, atom2.mass,
                                   molecule.diss_offset, n_molecule, n_atom1, n_atom2)
