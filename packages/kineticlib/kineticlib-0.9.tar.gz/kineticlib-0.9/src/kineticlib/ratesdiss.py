...
def diss_rate_rigid_sphere_sts(T: float, idata: np.ndarray, molecule: Molecule, i: int,
    ...
    float
        Non-equilibrium dissociation state-to-state rate constant
    """
    return raw_diss_rate_rigid_sphere_sts(T, idata[1], idata[0], molecule.vibr[i], molecule.diss_offset,
...
def rec_rate_treanor_marrone(T: float, model_data: np.ndarray, molecule: MoleculeQuasi,
    ...
    """
    if isinstance(molecule, MoleculeMultiT):
        return raw_rec_rate_treanor_marrone(T, model_data[0], model_data[1], molecule.Z_int(T, T),
...
def rec_rate_rigid_sphere(T: float, T1: float, idata: np.ndarray, molecule: MoleculeQuasi, atom1: Atom, atom2: Atom,
    ...
    float
        Averaged recombination rate constant
    """
    res = 0.0  # TODO - maybe this can be cleaned up?
    if isinstance(molecule, MoleculeMultiT):
        for i in range(molecule.num_vibr_levels(T, T1, True) + 1):
