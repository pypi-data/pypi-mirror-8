...
class Molecule(Particle):
    ...
    diss_offset : float
        This is equal to the dissociation energy of the molecule for Molecule and MoleculeOneT instance,
        for MoleculeMultiT instances this is equal to the dissociation energy of the molecule offset
        by the energy of the 0-th vibrational level
    rot : ndarray
    ...
    def avg_rot_energy_sq(self, T: float, dimensional: bool=True) -> float:
        ...
        if self._current_T != T:
            ...
            if dimensional:
                ...
                return self._current_raesq / self._current_kT
...
class MoleculeMultiT(MoleculeQuasi):
    ...
    def __init__(self, name, vmodel='anharmonic'):
        ...
        self.vibr -= self.vibr_zero
        self.diss_offset -= self.vibr_zero
...
class MoleculeOneT(MoleculeQuasi):
    ...
    def avg_vibr_energy_sq(self, T: float, dimensional: bool=True) -> float:
        ...
        if self._current_T != T:
            ...
            # else:
            #     if dimensional:
            #         return np.dot(self.vibr_exp(T, a, False), np.power(self.vibr[a], 2)) / self.Z_vibr(T, False)
            #     else:
            #         return np.dot(self.vibr_exp(T, a, False), np.power(self.vibr[a]
            #                       / (constants.k * T), 2)) / self.Z_vibr(T, False)
        else:
            if dimensional:
                return self._current_vaesq
            else:
