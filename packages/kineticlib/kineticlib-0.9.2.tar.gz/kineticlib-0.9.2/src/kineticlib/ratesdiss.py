...
from .particles import Molecule, MoleculeMultiT, Atom, MoleculeOneT, MoleculeQuasi
...
def diss_rate_rigid_sphere_sts(T: float, idata: np.ndarray, molecule: Molecule, i: int,
                               center_of_mass: bool=True, vl_dependent: bool=True) -> float:
    """Calculates the state-to-state non-equilibrium dissociation rate constant using the rigid-sphere crosssection
    ...
    if isinstance(molecule, MoleculeMultiT):
        return raw_diss_rate_rigid_sphere_sts(T, idata[1], idata[0], molecule.vibr[i] + molecule.vibr_zero,
                                              molecule.diss, center_of_mass, vl_dependent)
    else:
        return raw_diss_rate_rigid_sphere_sts(T, idata[1], idata[0], molecule.vibr[i],
                                              molecule.diss, center_of_mass, vl_dependent)
...
def rec_rate_rigid_sphere_sts(T: float, idata: np.ndarray, molecule: Molecule, atom1: Atom, atom2: Atom, i: int,
                              center_of_mass: bool=True, vl_dependent: bool=True) -> float:
    """Calculates the state-to-state recombination rate constant using the rigid-sphere crosssection
    ...
    float
        Recombination state-to-state rate constant
    """
    return raw_rec_rate_rigid_sphere_sts(T, idata[1], idata[0], molecule.Z_rot(T), molecule.mass, atom1.mass,
                                         atom2.mass, molecule.vibr[i], molecule.diss_offset,
...
def diss_rate_rigid_sphere(T: float, T1: float, idata: np.ndarray, molecule: MoleculeQuasi,
                           center_of_mass: bool=True, vl_dependent: bool=True) -> float:
    """Calculates the multi(one)-temperature non-equilibrium dissociaiton rate constant using the
    rigid-sphere crosssection
    ...
    if isinstance(molecule, MoleculeMultiT):
        for i in range(molecule.num_vibr_levels(T, T1, True) + 1):
            res += molecule.vibr_exp(T, T1, i) \
                   * raw_diss_rate_rigid_sphere_sts(T, idata[1], idata[0], molecule.vibr[i] + molecule.vibr_zero,
                                                    molecule.diss_offset, center_of_mass, vl_dependent)
        return res / molecule.Z_vibr(T, T1)
    else:
        for i in range(molecule.num_vibr + 1):
            res += molecule.vibr_exp(T, i) \
                   * raw_diss_rate_rigid_sphere_sts(T, idata[1], idata[0], molecule.vibr[i], molecule.diss_offset,
                                                    center_of_mass, vl_dependent)
        return res / molecule.Z_vibr(T)
...
def rec_rate_treanor_marrone(T: float, T1: float, model_data: np.ndarray, molecule: MoleculeQuasi,
    ...
    if isinstance(molecule, MoleculeMultiT):
        res = 0.0
        for i in range(molecule.num_vibr_levels(T, T1, True) + 1):
            res += raw_rec_rate_treanor_marrone(T, model_data[0], model_data[1], molecule.Z_int(T, T),
                                                molecule.mass, atom1.mass,
                                                atom2.mass, molecule.diss) * molecule.vibr_exp(T, T1, i)
    else:
        return raw_rec_rate_treanor_marrone(T, model_data[0], model_data[1], molecule.Z_int(T),
                                            molecule.mass, atom1.mass, atom2.mass, molecule.diss)


def rec_rate_rigid_sphere(T: float, T1: float, idata: np.ndarray, molecule: MoleculeQuasi, atom1: Atom, atom2: Atom,
                          center_of_mass: bool=True, vl_dependent: bool=True) -> float:
    """Calculates the averaged recombination rate constant using the rigid-sphere crosssection
    ...
    if isinstance(molecule, MoleculeMultiT):
        for i in range(molecule.num_vibr_levels(T, T1, True) + 1):
            res += raw_rec_rate_rigid_sphere_sts(T, idata[1], idata[0], molecule.Z_rot(T), molecule.mass, atom1.mass,
                                                 atom2.mass, molecule.vibr[i], molecule.diss_offset,
                                                 center_of_mass, vl_dependent)
    else:
        for i in range(molecule.num_vibr + 1):
...
def raw_diss_rate_rigid_sphere_sts(T: float, sigma: float, mass: float, molecule_vibr: float, molecule_diss: float,
                                   center_of_mass: bool=True, vl_dependent: bool=True) -> float:
    """Calculates the state-to-state non-equilibrium dissociation rate constant using the rigid-sphere
    crosssection, *raw* version
...
def raw_rec_rate_rigid_sphere_sts(T: float, sigma: float, mass: float, Z_rot: float, mass_molecule: float,
                                  mass_atom1: float, mass_atom2: float, molecule_vibr: float, molecule_diss: float,
                                  center_of_mass: bool=True, vl_dependent: bool=True) -> float:
    """Calculates the state-to-state recombination rate constant using the rigid-sphere
    crosssection, *raw* version
    ...
    return Z_rot * (constants.h ** 3) * ((mass_molecule / (2.0 * mass_atom1
                                                           * mass_atom2 * constants.pi * constants.k * T)) ** 1.5)\
           * exp(-(molecule_diss - molecule_vibr) / (constants.k * T))\
           * raw_diss_rate_rigid_sphere_sts(T, sigma, mass, molecule_vibr, molecule_diss, center_of_mass, vl_dependent)


def raw_rec_rate_treanor_marrone(T: float, arrhenius_n: float, arrhenius_A: float,
                                 Z_int: float, mass_molecule: float, mass_atom1: float,
                                 mass_atom2: float, molecule_diss: float) -> float:
    """Calculates the averaged recombination rate constant using the Treanor-Marrone
    model, *raw* version
    ...
    """
    return Z_int * (constants.h ** 3) * ((mass_molecule / (2.0 * mass_atom1
                                                           * mass_atom2 * constants.pi * constants.k * T)) ** 1.5)\
           * exp(-molecule_diss / (constants.k * T))\
           * raw_k_diss_eq(T, arrhenius_n, arrhenius_A, molecule_diss)