...
def rot_rel_time(T: float, idata: np.ndarray, molecule: Molecule, n: float, infcoll: float,
    ...
    """
    return raw_rot_rel_time(T, omega(T, 2, 2, idata, model=model, nokt=False), n,
                            raw_Zr(T, idata[0], molecule.LJe, idata[6], idata[11], idata[12], molecule.mass, infcoll,
                                   Zr_model))
...
def raw_rot_rel_time(T: float, omega_integral: float, n: float, Zr) -> float:
    """Calculates the rotational relaxation time using Parker's formula and the :math:`\\Omega^{(2,2)}` integral,
    *raw* version
    ...
    float
        Rotational relaxation time
    """
    return 0.15625 * (constants.pi ** 2) * Zr / (n * omega_integral)  # 0.15625 is 5 / 32
