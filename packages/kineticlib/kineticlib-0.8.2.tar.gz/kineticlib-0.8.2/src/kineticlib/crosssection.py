...
def crosssection_vt(g: float, T: float, idata: np.ndarray, molecule: MoleculeSTS, i: int, delta: int,
    ...
    """
    if delta != 0:
        return raw_crosssection_vt(g, T, idata[1], idata[0], idata[4], idata[9], idata[10], molecule.vibr[i],
...
def crosssection_vv(g: float, T: float, idata: np.ndarray, molecule1: MoleculeSTS, molecule2: MoleculeSTS, i: int,
    ...
    **M1(i) + M2(k) -> M1(i + i_delta) + M2(k - i_delta)**
    ...
    """
    if i_delta != 0:
        return raw_crosssection_vv(g, T, idata[1], idata[0], idata[4], idata[9], idata[10],
...
def crosssection_diss_rigid_sphere(g: float, T: float, idata: np.ndarray, molecule: MoleculeSTS, i: int,
    ...
    **Note** if integrating over the crosssection, the integration must be performed not from :math:`0` to
    ...
    """
    return raw_crosssection_diss_rigid_sphere(g, T, idata[1], molecule.vibr[i], molecule.diss, center_of_mass,
                                              vl_dependent)


def min_dimensionless_vel_diss(T: float, molecule: MoleculeSTS, i: int, vl_dependent: bool=True) -> float:
...
def vt_integral(T: float, deg: int, idata: np.ndarray, molecule: MoleculeSTS, i: int, delta: int,
    ...
    """
    if not rig_sphere:
        return raw_vt_integral(T, deg, idata[0], idata[4], idata[9], idata[10], molecule.vibr[i],
                               molecule.vibr[i + delta], molecule.vibr_zero, molecule.diss, i, delta, nokt)
    else:
        return raw_vt_integral_rigid_sphere(T, deg, idata[1], idata[0], idata[4], molecule.vibr[i],
                                            molecule.vibr[i + delta], molecule.vibr_zero, molecule.diss, i, delta, nokt)
...
def vv_integral(T: float, deg: int, idata: np.ndarray, molecule1: MoleculeSTS, molecule2: MoleculeSTS, i: int,
    ...
    **M1(i) + M2(k) -> M1(i + i_delta) + M2(k - i_delta)**
    ...
    if not rig_sphere:
        ...
        raw_vv_integral_rigid_sphere(T, deg, idata[1], idata[0], idata[4],
                                     molecule1.vibr[i], molecule1.vibr[i + i_delta],
                                     molecule2.vibr[k], molecule2.vibr[k - i_delta], i, k, i_delta, nokt)
...
def vv_collisions(T: float, T1_1: float, T1_2: float, idata: np.ndarray, molecule1: Molecule, molecule2: Molecule,
    ...
    **M1(i) + M2(k) -> M1(i + i_delta) + M2(k - i_delta)**,
    ...
    return raw_vv_collisions(T, idata[1], idata[0], idata[4], idata[9], idata[10],
...
def dE_rot_sq(T: float, idata: np.ndarray, molecule: MoleculeSTS, full_integral: bool=True,
              rig_sphere: bool=False, nokt: bool=False) -> float:
...
def raw_crosssection_vt(g: float, T: float, sigma: float, mass: float, beta: float, vssc: float, vsso: float,
    ...
    """
    if rig_sphere:
        return prob.vt_probability(g, T, mass, beta, ve_before, ve_after, molecule_vibr_zero, molecule_diss, i, delta)\
...
def raw_crosssection_vv(g, T: float, sigma: float, mass: float, beta: float, vssc: float, vsso: float,
    ...
    """
    if rig_sphere:
        return prob.vv_probability(g, T, mass, beta, ve_before1, ve_after1, ve_before2, ve_after2, i, k, i_delta)\
               * raw_crosssection_rigid_sphere(sigma)
    else:
        return prob.vv_probability(g, T, mass, beta, ve_before1, ve_after1, ve_before2, ve_after2, i, k, i_delta)\
               * raw_crosssection_VSS(g, T, vssc, vsso)
...
def raw_vt_integral(T: float, deg: int, mass: float, beta: float, vssc: float, vsso: float,
    ...
    if delta != 0:
        ...
        f = lambda g: prob.vt_prob_g_only(g, T, mass, beta, ve_before, ve_after,
...
def raw_vt_integral_rigid_sphere(T: float, deg: int, sigma: float, mass: float, beta: float,
...
def raw_vv_integral(T: float, deg: int, mass: float, beta: float, vssc: float, vsso: float,
    ...
    """
    if i_delta != 0:
        f = lambda g: prob.vv_prob_g_only(g, T, mass, beta, ve_before1, ve_after1, ve_before2, ve_after2,
...
def raw_vv_integral_rigid_sphere(T: float, deg: int, sigma: float, mass: float, beta: float, ve_before1: float,
    ...
    *raw* version
    ...
    """
    if i_delta != 0:
        f = lambda g: prob.vv_prob_g_only(g, T, mass, beta, ve_before1, ve_after1, ve_before2,
...
def raw_vv_collisions(T: float, sigma: float, mass: float, beta: float, vssc: float, vsso: float,
    ...
    for k in range(molecule_num_vibr2):
        for i in range(molecule_num_vibr1):
            if not rig_sphere:
                ...
                cs = raw_vv_integral_rigid_sphere(T, 0, sigma, mass, beta,
                                                  molecule_vibr_array1[i], molecule_vibr_array1[i + 1],
...
def raw_dE_rot_sq(T: float, sigma: float, mass: float, vssc: float, vsso: float,
    ...
    prettyexp = np.exp(-(-1.0 + molecule_num_rot) * molecule_num_rot / A)
    prettyexp2 = np.exp(0.25 * ((1.0 - 2.0 * molecule_num_rot) ** 2) / A)
    prettyexp3 = np.exp(0.25 * ((1.0 + 2.0 * molecule_num_rot) ** 2) / A)
    prettyexp4 = np.exp(-(1.0 + molecule_num_rot) * molecule_num_rot / A)
    ...
    tmp = 0.5 * A * prettyexp\
              * (2.0 * (A - 4 * A * A + A * (5.0 + 4 * A) * np.exp((-1 + molecule_num_rot) * molecule_num_rot / A)
    ...
    tmp += 0.5 * (2.0 * A * (4.0 + 9.0 * A + 2.0 * A * A) - 3.0 * (A ** 1.5)
                  * (1.0 + 2.0 * A) * np.exp(0.25 / A) * (constants.pi ** 0.5) * prettyerf
    ...
    tmp += 0.5 * A * (np.exp(-0.5 / A) * (2.0 * A * (1.0 + 4.0 * A)
                                          + Asqrt * (-1.0 + 6.0 * A) * np.exp(2.25 / A) * (constants.pi ** 0.5)
                                          * prettyerf4)
                      - prettyexp4
    ...
    tmp += 2.0 * A * (2.0 + 2.0 * A + A * A)\
               * np.exp(-2.0 / A) - A * prettyexp4\
                                      * (2.0 * A * A + 2 * A * molecule_num_rot
                                             * (1.0 + molecule_num_rot)
                                         + molecule_num_rot_sq * (1.0 + molecule_num_rot) * (1.0 + molecule_num_rot))
    tmp += 0.5 * (A * Asqrt * (1.0 + 2.0 * A) * np.exp(-2.0 / A)
                  * (2.0 * Asqrt + 3.0 * np.exp(2.25 / A) * (constants.pi ** 0.5) * prettyerf4)
