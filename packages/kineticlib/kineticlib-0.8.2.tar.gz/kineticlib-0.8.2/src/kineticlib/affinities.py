...
def Gamma_VT(T: float, T1: float, molecule: Molecule, delta: int=-1) -> float:
...
def Gamma_VV(T: float, T1_1: float, T1_2: float, molecule1: Molecule, molecule2: Molecule, i_delta: int,
             k_delta: int) -> float:
...
def Gamma_22(T: float, T1_1: float, T1_2: float, T1_new1: float, T1_new2: float, molecule1: Molecule,
             molecule2: Molecule, molecule_new1: Molecule, molecule_new2: Molecule, n_molecule1: float,
...
def Gamma_diss(T: float, T1: float, atom1: Atom, atom2: Atom, molecule: Molecule, n_molecule: float, n_atom1: float,
    ...
    **AB(diss_level) + P -> A + B + P**
    ...
    """
    return raw_Gamma_diss(T, T1, molecule.vibr[1], molecule.Z_int(T, T1), molecule.mass, atom1.mass, atom2.mass,
                          molecule.diss, n_molecule, n_atom1, n_atom2, diss_level)


def find_natom_diss_eq(T: float, molecule: Molecule, atom: Atom, n: float, relative: bool=True) -> float:
    ...
    """
    return raw_find_natom_diss_eq(T, molecule.Z_int(T, T), molecule.mass, atom.mass, molecule.diss, n, relative)
