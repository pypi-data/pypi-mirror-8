...
def diss_rate_rigid_sphere(T: float, T1: float, idata: np.ndarray, molecule: MoleculeQuasi,
    ...
    if isinstance(molecule, MoleculeMultiT):
        ...
        for i in range(molecule.num_vibr + 1):
            res += molecule.vibr_exp(T, i) \
                   * raw_diss_rate_rigid_sphere_sts(T, idata[1], idata[0], molecule.vibr[i], molecule.diss,
                                                    center_of_mass, vl_dependent)
        return res / molecule.Z_vibr(T)
    ...
    if isinstance(molecule, MoleculeMultiT):
        for i in range(molecule.num_vibr_levels(T, T1, True) + 1):
            res += raw_rec_rate_treanor_marrone_sts(T, model_data[0], model_data[1],
                                                    ...
                                                    molecule.diss) * molecule.vibr_exp(T, T1, i)
        return res / molecule.Z_vibr(T, T1)
    else:
        for i in range(molecule.num_vibr + 1):
            res += raw_rec_rate_treanor_marrone_sts(T, model_data[0], model_data[1],
                                                    ...
                                                    molecule.diss) * molecule.vibr_exp(T, i)
        return res / molecule.Z_vibr(T)
    ...
    if isinstance(molecule, MoleculeMultiT):
        for i in range(molecule.num_vibr_levels(T, T1, True) + 1):
            res += raw_rec_rate_rigid_sphere_sts(T, idata[1], idata[0], molecule.Z_rot(T), molecule.mass, atom1.mass,
                                                 atom2.mass, molecule.vibr[i] + molecule.vibr_zero,
                                                 molecule.diss, center_of_mass, vl_dependent)
    else:
        for i in range(molecule.num_vibr + 1):
            res += raw_rec_rate_rigid_sphere_sts(T, idata[1], idata[0], molecule.Z_rot(T), molecule.mass, atom1.mass,
