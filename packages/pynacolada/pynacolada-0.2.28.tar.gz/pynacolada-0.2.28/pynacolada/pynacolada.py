...
def pcd(func,dnamsel,datin,datout,appenddim = False, maxmembytes = 10000000, 
    ...
    for idatin,edatin in enumerate(datin):
        ...
        # print(vsdin[idatin]['dtype'])
        vsdin[idatin]['voffset']  = nctemp.variables[datin[idatin]['varname']]._voffset
        
        # additional attributes of the netcdf files we want to add to the file description
        props = ['add_offset','scale_factor']
        for prop in props:
            if prop in dir(nctemp.variables[datin[idatin]['varname']]):
                vsdin[idatin][prop] = nctemp.variables[datin[idatin]['varname']].__getattribute__(prop)
    ...
    for idatout,edatout in enumerate(datout):
        ...
        if edatout['varname'] in edatout['file'].variables:
            ...
            props = ['add_offset','scale_factor']
            for prop in props:
                if prop in dir(nctemp.variables[datout[idatout]['varname']]):
                    vsdout[idatout][prop] = nctemp.variables[datout[idatout]['varname']].__getattribute__(prop)
    ...
    if appenddim == True:
        ...
        # error this prevented to add also outer dimensions: while ((idnam >= maxarefdfunc) & (membytes <= maxmembytes) & cont):
    ...
    for j in range(lenitermax):
        ...
        for ivsdin,evsdin in enumerate(vsdin):
            ...
            if 'scale_factor' in vsdin[ivsdin]:
                dataicecubein[-1] =  dataicecubein[-1]*vsdin[ivsdin]['scale_factor']
            if 'add_offset' in vsdin[ivsdin]:
                dataicecubein[-1] =  dataicecubein[-1] + vsdin[ivsdin]['add_offset']
        ...
        for ivsdout in range(len(vsdout)):
            # before the write out, we convert the numbers according to the output offset and scalefactor
            if 'add_offset' in vsdout[ivsdout]:
                dataicecibeout[ivsdout] =  dataicecibeout[-1] - vsdout[ivsdout]['add_offset']
            if 'scale_factor' in vsdout[ivsdout]:
                dataicecibeout[ivsdout] =  dataicecibeout[-1]/vsdout[ivsdout]['scale_factor']
