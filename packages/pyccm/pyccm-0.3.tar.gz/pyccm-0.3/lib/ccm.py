...
class setup_CCM:
    def __init__(self,arr,name,E,tau,periods):
        ...
        distance = self.euclidean_distance(arr[name])
    ...
    def reconstructed_correlation(self,arr,name):
        ...
        #reconstructed_arr=sum([self.weigths[...,id]*select_indices(
        #                                                 select_slice(arr[name],0,self.E,self.tau,self.periods),
        #                                                   self.nn_indices[...,id]
        #                                                          ) for id in range(self.weigths.shape[-1])
        #                        ])
        #return simple_correlation(select_slice(arr[name],0,self.E,self.tau,self.periods),reconstructed_arr)
        lagged=self.gen_lagged(arr[name])
        ...
        #return simple_correlation(arr[name],reconstructed_arr)
