...
-  ordereddict for python2.6 (if you like to use the OrderedDict PyYAML
   loader / dumper `ODYLDo <#ODYLDo>`_)
...
-  ``usedefaultyamlloader``: boolean : force the usage of the default
   *PyYAML* loader/dumper instead of *HiYaPyCo*\ s implementation of a
   OrderedDict loader/dumper (see: `ODYLDo <#ODYLDo>`_ (default: False)
...
``test/test_odict.py`` for an example). See `ODYLDo <#ODYLDo>`_
