    ...
    if user is None:
        ...
        if username and password:
            ...
            if not user.check_password(password):
                return HttpResponse(_('Invalid password for user %(u)s') % {'u': username})
        else:
            return HttpResponse(_('You must be logged to upload files. You can provide your credentials by adding '
                                  '&username=USERNAME&password=PASSWORD to your request'), status=401)
