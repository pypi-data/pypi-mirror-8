...
class GlideRecord:
    ...
    # Sets the URL to the servicenow server for future queries (e.g.,
    # ubc.service-now.com)
    def set_server(self, srvname):
        self.query_data['server'] = srvname
        self.query_data['URL'] = '%s/%s.do?JSONv2&sysparm_record_count=%s&sysparm_action=%s&sysparm_query=' % (
            self.query_data['server'], self.query_data['tableName'], self.query_data['rowCount'], self.query_data['actionType'])
    ...
    def get_credentials(self):
        ...
        self.password = getpass.getpass(
            "Please enter ServiceNow password for %s:" % self.username)
        self.reset_credentials()

    # This is a helper function, not to be used directly
    # It basically stored the username and password in encrypted format, for
    # future queries
    ...
    def is_user_creds_valid(self):
        request = re.sub(
            'sysparm_record_count=[^&]*', 'sysparm_record_count=1', self.query_data['URL'])
        result = self.get_url(request)
        if result:
            return True
        return False

    # It checks if there is a record in the table which has key==value,
    # returns a boolean
    ...
    # for example, setValues("u_phone_number", "12345") sets values of all
    # phone numbers to '12345'
    def setValues(self, key, value, show_results=True):
        request = re.sub(
            'sysparm_action=[^&]*', 'sysparm_action=%s' % 'update', self.query_data['URL'])
    ...
    def insert(self, data):
        request = re.sub(
            'sysparm_action=[^&]*', 'sysparm_action=%s' % 'insert', self.query_data['URL'])
    ...
    def delete(self):
        ...
        if not re.findall('syparm_sys_id', query):
            print(
                "ERROR: Could not delete the record. 'sysparm_sys_id' is not specified")
            sys.exit(0)
        url = self.query_data['URL']
        request = re.sub(
            'sysparm_action=[^&]*', 'sysparm_action=%s' % 'deleteRecord', url)
        data = json.dumps({
    ...
    # Deletes ALL records from servicenow that were returned by the query
    # after query is run
    def deleteMultiple(self):
        url = self.query_data['URL']
        request = re.sub(
            'sysparm_action=[^&]*', 'sysparm_action=%s' % 'deleteMultiple', url)
    ...
    # Adds a new query (filter) to the query string (e.g., addQuery("active",
    # "true")
    ...
    # Queries ServiceNow, and stores the results in a variable, for future
    # access
    def query(self):
    ...
    # This function does a bunch of testing to make sure everything works as
    # expected
    def unittest(self):
        ...
        def test_limited_update():
            ...
            self.addQuery(
                "short_description", "Creating a record using GlideRecord API for Python")
            self.setValues(
                "short_description", "Updating a record using GlideRecord API for Python", False)
            ...
            print "%s out of %s records were updated, %s should be left unchanged, but " \
                  ...
                  (num_updated_records, num_records, num_records -
                   num_updated_records, self.getRowCount())
            return 0
        ...
        def test_limited_insert():
            ...
            # Getting all tickets that have a caller whose name starts with
            # letter 'C'
            self.clearQuery()
            self.setRowCount("")
