class Router():
    ...
    def route(self, cls):
        ...
        if not hasattr(cls, 'endpoint') or not hasattr(cls, 'url'):
            raise Exception('declare `{0!r}.endpoint` and `{0!r}.url` to use "flask-routes"'.format(cls))
        nameset = set()
                ...
                if is_callable and is_public and is_routable:
                    if attrvalue.routable and attrname not in nameset:
                        ...
                        for route in getattr(cls, attrname).routes:
                            ...
                            url = cls.endpoint if not cls.url else cls.url
