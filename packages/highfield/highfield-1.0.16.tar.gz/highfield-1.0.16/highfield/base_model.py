...
class BaseModel(object):
    ...
    def has_required_properties(cls, purpose, doc):
        if not doc:
            return False
        for canonical_name, _property in cls.properties.__dict__.iteritems():
            if isinstance(_property, cls.Property) and getattr(_property, purpose, False):
                if not canonical_name in doc:
                    return False
                pass
            pass
        return True
