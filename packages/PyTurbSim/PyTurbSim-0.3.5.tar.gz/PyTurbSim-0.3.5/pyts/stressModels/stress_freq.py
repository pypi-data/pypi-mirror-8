...
class stressModelBase_f(stressModelBase):
    ...
    def __new__(cls,turbModel,*args,**kwargs):
        self=super(stressModelBase_f,cls).__new__(cls,turbModel,*args,**kwargs)
        self._rstrCoh=np.zeros([self.n_comp]+list(self.grid.shape)+[self.n_f],dtype=ts_float)
    ...
    # For now, I have simply copied the code that was here before I simplified the stressModelBase class.
    def _setPhases(self,):
        """
        ...
        rstrmat=self.grid.flatten(self._rstrCoh) # This doesn't currently work
        ...
        if (1+srt[0]-srt[1]-srt[2]<0).any() or (((rstrmat<0).sum(0)==1) & (srt.sum(0)>1)).any():
             # We can't have rem<0, or only one negative correlation if the total correlation is greater than 1.
            raise Exception('The input reynolds stresses are inconsistent.')
        ovr=np.minimum((srt[0]*srt[1]+srt[0]*srt[2])/2,srt[0]) # average the product of the smallest value with the two larger ones. Then take the minimum value of that with the smallest value. This is the 'overlap', i.e. the fraction of points that will have the same phase for all three components.
        ovr[(rstrmat<0).sum(0)==1]=0 # If there is only 1 negative stress than the overlap must be zero.
        ...
        self.rand[2][inds_used]=np.sign(rstrmat[1][inds_used])*self.rand[0][inds_used]
        self.rand[1][inds_used]=np.sign(rstrmat[0][inds_used])*self.rand[0][inds_used]
        ####
