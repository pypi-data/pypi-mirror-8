...
class Container(object):
    ...
    def is_built(self):
        ...
        try:
            ...
            raise ContainerError("{} built check failed: docker is not available".format(self.tag))

    def is_running(self):
        """Return True if the container is running."""
        figcfg = os.path.join(self.path, 'fig.yml')
        if not os.path.isfile(figcfg):
            return False
        try:
            ...
            raise ContainerError("{} running check failed: fig is not available".format(self.tag))

    def is_verified(self):
        """Return True if this version of the container is verified."""
        from .version import verify
        return verify(self)
    ...
    def prebuild(self):
        ...
        try:
            ...
            raise ContainerError("{} prebuild failed: command is not available".format(self.tag))
    ...
    def build(self, prebuild=None, nocache=None):
        ...
        try:
            ...
            raise ContainerError("{} build failed: docker is not available".format(self.tag))
    ...
    def push(self):
        ...
        try:
            ...
            raise ContainerError("{} push failed: docker is not available".format(self.tag))
    ...
    def remove(self):
        ...
        try:
            ...
            msg = "{} remove failed: docker returned non-zero exit code"
            raise ContainerError(msg.format(self.tag))
        except OSError:
            raise ContainerError("{} remove failed: docker is not available".format(self.tag))
    ...
    def run(self, detach=None):
        ...
        try:
            ...
            raise ContainerError("{} run failed: fig returned non-zero exit code".format(self.tag))
        except OSError:
            raise ContainerError("{} run failed: fig is not available".format(self.tag))
    ...
    def kill(self):
        ...
        try:
            ...
            raise ContainerError("{} kill failed: fig is not available".format(self.tag))
        finally:
            if self.allow_remove:
                args = self._get_fig_args('rm', '--force', '-v')
                try:
    ...
    def test(self, rebuild=None):
        ...
        if rebuild and 'build' in self._testing['test']:
            try:
                ...
                raise ContainerError("{} test failed: fig is not available".format(self.tag))
        ...
        try:
            ...
            raise ContainerError("{} test failed: failed to start dependencies".format(self.tag))
        except OSError:
            raise ContainerError("{} test failed: fig is not available".format(self.tag))
