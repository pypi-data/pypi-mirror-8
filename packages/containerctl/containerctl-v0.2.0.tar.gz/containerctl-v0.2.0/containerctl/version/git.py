...
class GitDetector(Detector):
    ...
    def _verify(self, tag):
        ...
        try:
            ...
            raise VersionError("command failed: git {}".format(print_args))
    ...
    def _rev(self):
        """Return the version created using the short SHA hash of HEAD prepended with 'r'."""
        args = ['git', 'rev-parse', '--short', 'HEAD']
        try:
            with open(os.devnull, 'w') as null:
                out = subprocess.check_output(args, cwd=self.container.path, stderr=null)
            return Version('r' + out.strip(), False)
        except OSError:
            print_args = ' '.join(pipes.quote(a) for a in args)
            raise VersionError("command failed: git {}".format(print_args))
