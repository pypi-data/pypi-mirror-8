...
#
	def switchlight(self, jsonInput):
		...
		if entity == "dz_loc_bureau_deux" or entity == "dz_loc_bureau_un" or entity == "dz_loc_carrillon" or entity == "dz_loc_salon"  :
			on_off = jsonInput['outcome']['entities']['on_off']['value']
			location = "VIDE"
		if entity == "dz_loc_frisquet"  :
			on_off = jsonInput['outcome']['entities']['on_off']['value']
		...
		if on_off == "allume" :
			on_off = "on"
		if ( on_off == "eteint" or on_off == "éteint" ) :
			on_off = "off"
		idex= entity + "_" + location + "_" + on_off
		log.msg("index 2 :  %s" % (idex))		
...
#				resp = requests.get (req)
	...
	def devices(self, jsonInput):
		...
		entity = jsonInput['outcome']['intent']
		log.msg("entity :  %s" % (entity))	
		if entity == "dz_batterie" or "dz_temperature"  :
			location = jsonInput['outcome']['entities']['location']['body']
		idex= entity + "_" + location
		log.msg("idex :  %s" % (idex))		
