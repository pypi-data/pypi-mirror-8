...
# an simpler version of avgcyc
def avgcyc(x,n,nanmask=False,stat = mean):
    """ calculate averaged cycle"""
    xout = zeros([n+1]+list(array(x).shape[1:]))
    for ix in range(n):
        xtemp = x[ix::n]
        if type(nanmask) == ndarray:
            masktemp = nanmask[ix::n]
        elif nanmask:
            masktemp = xtemp[:]
        else:
            masktemp = zeros_like(xtemp)

        xout[ix] = stat(xtemp[~isnan(masktemp)],axis=0)
    xout[n] = xout[0] 
    return xout


# a more extensive version of calculating a diurnal cycle
