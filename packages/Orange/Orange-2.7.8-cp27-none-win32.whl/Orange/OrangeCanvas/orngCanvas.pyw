...
#                print time.strftime("%H:%M:%S"), "%15s %15s" % (str(receiver.windowTitle()) ,type(receiver).__name__)  + ": ",      # print only events for QWidget classes and up
#                if eventDict.has_key(event.type()):
#                    print eventDict[event.type()] 
#                else:
#                    print "unknown event name (" + str(event.type()) + ")"
#        else:
#            print time.strftime("%H:%M:%S"), type(receiver).__name__, str(receiver.objectName()),
#            if eventDict.has_key(event.type()):
#                print eventDict[event.type()] 
#            else:
#                print "unknown event name (" + str(event.type()) + ")"
...
def main(argv=None):
    ...
    # Call gc.collect before the app is destroyed to colect any remainig
    # cycles. Some sip objects are stored in it's global state (for instance
    # QGraphicsScene) and can cause a crash in Py_Finalize when sip trys to 
    # deallocate it (a bug in sip?).
