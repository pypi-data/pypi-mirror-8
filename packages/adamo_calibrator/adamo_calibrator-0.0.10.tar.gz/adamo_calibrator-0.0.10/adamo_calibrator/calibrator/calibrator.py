...
class Calibrator:
    ...
    def get_device(self):
        ...
        (device_name, self.device, setted) = devices[-1]

        if device_name == 'eGalax Inc. USB TouchController':
    ...
    def check_axis_new(self, x, y):
        ...
        if (quadrant_exp in [1, 4] and quadrant in [2, 3]) or \
                (quadrant_exp in [2, 3] and quadrant in [1, 4]):
            self.inversex = True
        if (quadrant_exp in [1, 2] and quadrant in [3, 4]) or \
                (quadrant_exp in [3, 4] and quadrant in [1, 2]):
            self.inversey = True
