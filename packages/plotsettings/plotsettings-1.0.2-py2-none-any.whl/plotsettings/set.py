...
def panel_labels(fig = None, position = 'outside', case = 'lower', prefix = '', suffix = '.', 
    ...
    Inputs:
        ...
        * detect_colorbars (bool) - if True, will attempt to detect axes that only contain
          colorbars. This does not appear to work with AxesGrid objects. Optional, defaults
          to True.
    ...
    if len(xy) == 2: # If both x and y offsets are given
        ...
        equaloffsets = False
    choose_colorbardetector = {True: lambda ax: ax.rowNum, False: lambda ax: False}
    colorbar_detector = choose_colorbardetector[detect_colorbars]
    for ax in axis_list:
        try:
            colorbar_detector(ax) # Colorbar axes don't appear to have a rowNum or colNum
            ...
            if equaloffsets:
                ...
                x_display = ax.transAxes.transform((x_offset, y_origin))[0] # offset in display coordinates (pixels)
                ...
                x, y = offsetter(x_axis, y_axis) # Convert back to axis coordinates
            # Draw label:
...
def dummy_figure():
    ...
    titles = ('life pre-plotsettings', 'life post-plotsettings')
    ylabels = ('progress', '')
    xticklabels = (('none', r'$\infty$'), ('none', r'$<1\/minute$'))
    for ax, title, y, ylabel, xticklabel in zip(axes, titles, ys, ylabels, xticklabels):
        ax.plot(x, y, linestyle = '-', color = 'Coral', linewidth = 2) # Plot
        ax.set_title(title)
        ax.set_xlabel(r'time spent futzing with $rcParams$')
...
def demonstrate():
    ...
    fig = dummy_figure() # Create figure
    fig.tight_layout()
    ...
    publishable.set_figsize(1.5, 1) # 1.5 column widths
    fig = dummy_figure() # Create figure
