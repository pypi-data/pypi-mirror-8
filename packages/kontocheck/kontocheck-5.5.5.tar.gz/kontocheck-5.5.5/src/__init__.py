...
libname = 'kontocheck' + (os.name == 'nt' and '%s.dll' % _PY_BIT or '.so')
...
def scl_get_routing(bic):
    ...
    bic = bic.ljust(11, 'X')
    cur = dbscl.cursor()
    cur.execute("SELECT sct, sdd, cor1, b2b FROM scl WHERE bic = ? OR bic = ?;", (bic, bic[:8]))
    result = cur.fetchone()
    cur.close()
    if not result:
...
def scl_get_bankname(bic):
    ...
    bic = bic.ljust(11, 'X')
    cur = dbscl.cursor()
    cur.execute("SELECT name FROM scl WHERE bic = ? OR bic = ?;", (bic, bic[:8]))
    result = cur.fetchone()
    cur.close()
    if not result:
        raise ValueError('unknown bic "%s"' % bic)
    return result['name'].encode('utf-8')
...
def bff_check_vatid(local_vatid, remote_vatid, company='', city='',
    ...
    """
    server = xmlrpclib.ServerProxy('https://evatr.bff-online.de/')
