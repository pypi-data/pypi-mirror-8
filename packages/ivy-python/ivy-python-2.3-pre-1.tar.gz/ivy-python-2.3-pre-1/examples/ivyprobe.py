...
def on_pong(agent, serial, delta):
    info('%s answered to ping #%i in %fs', agent, serial, delta)
...
if __name__ == '__main__':
    ...
    while 1:
        ...
        .who                        - display the names of all applications on
                                                                       the bus
        .ping-mode mode             - if mode is 'ivy-c' or 'c', ping behaves
                                      like in ivy-c
...
""")
        ...
        elif msg[:10] == '.ping-mode':
            bool = msg[11:]
            if bool.lower() in ('ivyc', 'ivy-c', 'c'):
                bool=True
                info('Ping behaviour: mimics ivy-c')
            else:
                bool=False
                info('Ping behaviour: default')
            IvyPingMimicsIvyC(bool)
            ...
            if app is None:
                info('No application named %s', app_name)
                continue
            info('Sent PING #%i', IvySendPing(app))
