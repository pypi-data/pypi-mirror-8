...
def IvyBindPong(on_pong_fct):
    ...
    - the second one being the serial of the original PING message, as
      returned by IvySendPing();

    - the third one is the time elapsed between the sending of the
...
def IvyPingMimicsIvyC(conform_to_ivyc=None):
    """
    Determines the behaviour upon receiving a PONG message.
    Default is to call the pong callback for every pong messages we get.

    If parameter is True, it behaves as in Ivy-C.
    """
    assert _IvyServer is not None
    if conform_to_ivyc is not None:
        _IvyServer.ping_mimics_ivyc = conform_to_ivyc
    return _IvyServer.ping_mimics_ivyc
