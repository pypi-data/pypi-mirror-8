...
class IvyClient:
    ...
    def __init__(self, ip, port, client_socket,
        ...
        self.ping_id = 0
        self.ping_ts = {} # ping_id -> timestamp
    ...
    def send_ping(self):
        ...
        try:
            self.ping_id += 1
            self.ping_ts[self.ping_id]=time.time()
            self._send(PING, self.ping_id)
            return self.ping_id
        finally:
            self.ping_lock.release()

    def get_ping_delta(self, ping_id, timestamp):
        """
        """
        self.ping_lock.acquire()
        try:
            ts0 = self.ping_ts.pop(ping_id, None)
            if ts0 is None:
                return None
    ...
    def get_ping_delta_as_in_ivyc(self, ping_id, timestamp):
        """
        """
        self.ping_lock.acquire()
        try:
            ...
            delta = timestamp - ts0
            if ping_id != self.ping_id:
                delta = - ( timestamp - self.ping_ts[self.ping_id] )
                self.ping_ts = {}
            return delta
        finally:
            self.ping_lock.release()
...
class IvyServer(SocketServer.ThreadingTCPServer):
    ...
    def handle_pong(self, client, serial, delta):
        """
        """
        self.pong_callback(client, serial, delta)
    ...
    def process_ivymessage(self, msg, client):
        ...
        try:
            if msg_id == BYE:
                ...
                debug('Received PONG w/ id: %i from %r', num_id, client)
                if self.server.ping_mimics_ivyc:
                    delta = client.get_ping_delta_as_in_ivyc(num_id, time.time())
                else:
                    delta = client.get_ping_delta(num_id, time.time())
                if delta is None:
                    warn('Client %r sent a PONG with an invalid id: %s',
                         client, num_id)
                else:
                    self.server.handle_pong(client, num_id, delta)
