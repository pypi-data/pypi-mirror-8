...
def begin_group(dct, name, is_expert_group):
    if is_expert_group:
...
def end_group(dct, name):
    item = dt.EndGroup(name)
    item._name = name
    dct["eg_%s" % name] = item


def build_guidata_items(parameter_set, dct):
    """
...
def create_tab(parameter_set):
    dct = collections.OrderedDict()
    build_guidata_items(parameter_set, dct)
