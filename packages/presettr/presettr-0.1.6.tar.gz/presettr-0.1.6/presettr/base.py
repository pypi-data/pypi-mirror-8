...
class ParameterSet(ConfigItem):
    ...
    def check(self, name, value, item):
        ...
        if not ok:
            raise ValueError("can not set %s to %r" % (name, value))
    ...
    def __setattr__(self, name, value):
        ...
        if isinstance(self, ParameterSet) and not name.startswith("_") and item is None:
            raise Exception("value %s not knwon" % name)
        if item is not None:
            # value = item.setup_value(value)
            self.check(name, value, item)
            if item.target is not None:
                to, method = item._target
    ...
    def loads(clz, str_):
        obj = clz()
        obj.overwrite_from_dict(json.loads(str_))
        return obj

    @classmethod
    def load_json(clz, path):
        with open(path, "rU") as fp:
            return clz.loads(fp.read())

    def as_widget(self):
        from .gui_creation import create_tab
        return create_tab(self)
