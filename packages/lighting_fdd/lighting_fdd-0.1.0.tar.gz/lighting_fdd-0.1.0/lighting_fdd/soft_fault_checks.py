...
reduce_scheduled_on = "Potential to improve efficiency by reducing scheduled hours-on."
extend_scheduled_on = "Potential to improve efficiency by extending scheduled hours-on."
reduce_override_timeout = "Potential to improve efficiency by reducing the length of the override time-out."
extend_override_timeout = "Potential to improve efficiency by increasing the length of the override time-out."

def check_total_off_times_for_days_of_week(schedule):
    """
    Checks the "Is duration of scheduled off time < 8 hours on weekdays or < 12 hours on weekends." case
    ...
    for i in range(5,7):    #5 and 6 are Saturday and Sunday
        if total_off_times[i] < timedelta(hours=12):
            res.append((i, reduce_scheduled_on))
...
def check_switch_events_with_schedule_per_schedule_block(schedule, 
    ...
    for key, value in switched_to_off_before_lights_off.iteritems():
        if value > total_schedule_ct[key] * frequency_percent:
            warnings.append((key, reduce_scheduled_on))

    for key, value in switched_to_on_before_lights_on.iteritems():
        if value > total_schedule_ct[key] * frequency_percent:
            warnings.append((key, extend_scheduled_on))
...
def check_overrides_with_timeouts(override_times, override_values, schedule, frequency_percent = .5, override_timeout = timedelta(hours=1), grace_time_before_end_of_timeout = timedelta(minutes=30), after_override_window = timedelta(minutes=5), only_check_when_schedule_is_off = True):
    """
