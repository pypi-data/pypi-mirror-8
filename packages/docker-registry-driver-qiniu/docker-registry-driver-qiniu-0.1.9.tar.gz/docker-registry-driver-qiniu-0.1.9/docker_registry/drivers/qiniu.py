from docker_registry.contrib import qiniusdk.conf
from docker_registry.contrib import qiniusdk.rs
from docker_registry.contrib import qiniusdk.rsf
from docker_registry.contrib import qiniusdk.io
...
class Storage(driver.Base):
    ...
    def __init__(self, path=None, config=None):
        qiniusdk.conf.ACCESS_KEY = config.qiniu_accesskey
        qiniusdk.conf.SECRET_KEY = config.qiniu_secretkey

        self._bucket = config.qiniu_bucket
        self._domain = config.qiniu_domain
        
        self._putpolicy = qiniusdk.rs.PutPolicy(_config.qiniu_bucket)
        self._getpolicy = qiniusdk.rc.GetPolicy()
    ...
    def content_redirect_url(self, path):
        ...
        base_url = qiniusdk.rs.make_base_url(self._domain, path)
    ...
    def put_store(self, path, content, chunk=None, length=None):
        ...
        try:
            ret, err = qiniusdk.io.put(self._uptoken, path, content, None)
            if err is not None:
    ...
    def head_store(self, path):
        ret, err = qiniusdk.rs.Client().stat(self._bucket, path)
        if err is not None:
    ...
    def list_directory(self, path=None):
        try:
            ...
            if err is not qiniusdk.rsf.EOF:
    ...
    def remove(self, path):
        ...
        ret, err = qiniusdk.rs.Client().delete(self._bucket, path)
        if err is not None:
            raise IOError("List path %s err: %s" % (path, err))
