...
if [ ${venv} = yes ]; then
    ...
    echo "Installing EvProc setup in the virtual environment"
    python ${dir}/setup.py install
    if [ $? -ne 0 ]; then
        echo "Failed to install EvProc setup in virtual environment" >&2
        exit 1
    fi
...
if [ ${pep8} != only ]; then
	...
	options="${options} --cover-package=evproc"
