...
def assert_changed(o, clazz = object):
  import editobj2.introsp as introsp
  if isinstance(o, introsp.ObjectPack):
    for oo in o.objects: assert_changed(oo, clazz)
  ...
  if   clazz is object:
    ...
    observation = _observed_seqs.get(i)
    if type(observation.object) is ref:
      for listener in observation.listeners[:]: listener(o, observation.type, o, observation.old)
      observation.old = observation.current_value()
