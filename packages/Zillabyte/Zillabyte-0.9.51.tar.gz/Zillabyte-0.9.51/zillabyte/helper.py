...
class Helper:
  ...
  def check_name(self, operation, name, names):
    ...
    if not isinstance(name, basestring) or re.match('^[\w\.]+$', name) == None:
      msg = ee+"\"Name\" must be a non-empty STRING with only alphanumeric, underscore and period characters at %s. Apps and components must have names. The following methods must have names as well: \"sink\", \"inputs\" and \"outputs\"."%(str(name))
    ...
    if re.match('^[_0-9\.]$', name[0]) != None:
      msg = ee+"The \"name\" \"%s\" cannot begin with a number, underscore or period."%(name)
      self.print_error(msg)
  ...
  def check_file(self, fname):
    ee = "Error in \"source_from_csv\" at \"file\": \n\t "
    pp = self.print_check_source_from_csv

    try:
      f = file(fname)
      f.close()
    except IOError:
      msg = ee+"Invalid \"file\" argument: "+fname+". The specified file could not be found."+pp
      self.print_error(msg)
...
"Source" Syntax:
  Sourcing from a relation:
    app.source("relation_name" )

  Simple Custom source:
    def next_tuple(controller):
        # next_tuple_function

    app.source(next_tuple_function)


  Extended Source Syntax:
  ...
  print_check_source_from_csv = """\n
"SourceFromCSV" Syntax:
  Sourcing from a csv that does not include headers:
    app.source_from_csv("csv_name", headers=["name", "email"] )

  Sourcing from a csv that does include headers:
    app.source_from_csv("csv_name")

  - The "csv_name" is the path of the CSV within the app directory
  - The "headers" field is a list of CSV field names ["field1", "field2"...]:
  - The emitted output is a tuple hash {"field1" : value1, "field2" : value2},...}
"""
...
"Group By" Syntax:
  ...
  class GroupByClass:
    def begin_group(self, controller, group_tuple):
      # begin_group_method
  ...
  stream.group_by( GroupByClass,
                   name = "name", \t\t\t\t => optional
                   fields = ["field_1", "field_2", ...],
                   emits = ["stream_1", "stream_2", ...], \t => optional for single output stream )
  - The "begin_group_method" must take in 3 arguments (the "self", the "controller", and the "grouping tuple", which is emitted at the beginning of each group and contains the values of the fields specified in "group_by"), and should return nothing. This is where initial values for the aggregation are set.
  - The "aggregate_method" must take in 3 arguments (the "self", the "controller", and the "tuple"), and should return nothing. This is where the aggregation is performed.
  - The "end_group_method" must take in a 2 arguments (the "self" and the "controller"), and should return nothing. This is where the final aggregated value is emitted."""
