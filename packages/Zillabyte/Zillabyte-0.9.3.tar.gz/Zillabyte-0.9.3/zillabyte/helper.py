...
class Helper:
  ...
  @classmethod
  def check_loop_back(self, stream, node_name, nodes):
    ...
    for node in nodes:
      if node._name == node_name:
        ...
        if node._type == "source":
          msg = ee+"Cannot loop back to source node \"%s\"!"%(node_name)
          self.print_error(msg)
      if node._name == previous_node_name:
        if len(node._emits) < 2:
          ...
          self.print_error(msg)
    if not found:
      msg = ee+"The specified loop-back node \"%s\" was not found in the operations preceding it."%(node_name)
      self.print_error(msg)
