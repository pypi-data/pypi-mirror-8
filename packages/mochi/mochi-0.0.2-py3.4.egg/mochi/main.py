...
class Translator(object):
    ...
    def _tail_to_return_s(self, exps):
        ...
        def except_tail_to_return(except_exp):
            if issequence_except_str(except_exp) and isinstance(except_exp[0], Symbol) and except_exp[0].name == 'except':
    ...
    def translate_record(self, exp):
        ...
        record_def = ast.ClassDef(name=record_name.name,
                                  bases=[ast.Call(func=ast.Name(id='pclass',
                                                                ...
                                                                ctx=ast.Load()),
                                                  args=[ast.BinOp(left=ast.Attribute(value=ast.Name(id=parent_record.name,
