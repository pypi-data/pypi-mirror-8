...
#import numpy as np
from FuncDesigner import  ooarray, dot, sqrt, cross, norm, oofun
...
#        raise 0
    ...
    def distance(self, *args, **kw):
        ...
        if isinstance(other, (Line, Plane)):
            ...
            return sqrt(PythonSum((self-other)**2), attachConstraints = False)
    ...
    def projection(self, obj):
        ...
        if isinstance(obj, Line):
            ...
            tmp1 = obj.direction*tmp / (0.0+PythonSum(obj.direction**2))
            tmp2 = obj.basePoint
...
def skewLinesNearestPoints(line1, line2):
    ...
    Delta = PythonSum(d1**2)*PythonSum(d2**2) - dot(d1, d2)**2
    Delta1 = dot(d2, p1-p2) * dot(d1, d2) - dot(d1, p1-p2) * PythonSum(d2**2)
    Delta2 = dot(d2, p1-p2) * PythonSum(d1**2) - dot(d1, d2) * dot(d1, p1-p2)
    t1 = Delta1 / Delta
    t2 = Delta2 / Delta
    for t in [t1, t2]:
