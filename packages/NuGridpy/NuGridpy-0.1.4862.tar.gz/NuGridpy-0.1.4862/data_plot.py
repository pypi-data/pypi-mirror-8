    ...
    def plot_isoratios(self,xiso,yiso,fign=1,spec=None,deltax=True,deltay=True,logx=False,logy=False,
        ...
        if self._classTest() == 'se':
            ...
            ### EXPLOSIVE MODELS ###
                ...
                xdata = ratiox
                ydata = ratioy
                style_tmp0= [plt_symb + plt_lt,  plt_col, plt_col, '7.', '1', legend]
                style_tmp = [plt_symb + plt_lt,  plt_col, plt_col, '7.', '1', None]
                style = []
...
#                yin=tmpyps
    ...
    def iso_abund(self, cycle, stable=False, amass_range=None,
                  ...
                  label=None,colour=None,elemaburtn=False):
        ''' 
...
#                    a_dum[ii]=log10(abund_plot[j][ii])
            ...
            if mov:
                ...
                if label is not None and j==0:
                    pl.plot(mass_num[j],a_dum,this_label,markersize=6,label=label)
...
#           savefig('elemental'+str(i)+'.png')
            ...
            if label != '':
                if colour!='':
                    pl.plot(z_el[zmin_ind:zmax_ind],np.log10(el_abu_plot),'o-',label=label,color=colour)
                else:
                    pl.plot(z_el[zmin_ind:zmax_ind],np.log10(el_abu_plot),'o-',label=label)
            else:
                if colour!='':
                    pl.plot(z_el[zmin_ind:zmax_ind],np.log10(el_abu_plot),'o-',color=colour)
                else:
                    pl.plot(z_el[zmin_ind:zmax_ind],np.log10(el_abu_plot),'o-')
            if show_names:
                j=0        # add labels
                for z in z_el[zmin_ind:zmax_ind]:
