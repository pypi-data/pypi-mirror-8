...
class Map:
    def __init__(self, center, pixwidth, pixheight, zoom):
        self.pixwidth = pixwidth
        self.pixheight = pixheight
        self.nodes = []
        self.zoom = zoom
        self.pixcenter = [pixwidth // 2, pixheight // 2]
        self.center = [float(coord) for coord in center.split(', ')]
        self.offset = llpx.lat_lng_to_pixels(self.center[0], self.center[1], self.zoom)
...
class Node:
    ...
    def get_lat_lng(self):
        return llpx.pixels_to_lat_lng(self.map.offset[0] - self.map.pixcenter[0] + self.x,
            self.map.offset[1] - self.map.pixcenter[1] + self.y, self.map.zoom)
