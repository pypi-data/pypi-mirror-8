...
class S3FineView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if 'file_name' not in request_dict or 'mime_type' not in request_dict:
            return HttpResponseBadRequest(
                '"file_name" or "mime_type" are missing.'
            )

        self.file_name = request_dict['file_name']
        self.mime_type = request_dict['mime_type']

        return HttpResponse(json.dumps(self.sign()))
