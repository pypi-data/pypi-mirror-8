...
static void timeout_progressively(uint64_t total_mem, struct parser_base *p, 
        uint64_t min_rx_time) {
    double limit = (total_mem - p->soft_mem_limit) /
            (double) (p->hard_mem_limit - p->soft_mem_limit);
    uint64_t delta = p->last_rx_time - min_rx_time;
    ...
    while ((tcp_chunk = chunk_iter_next(tcp_chunks, &tcp_it)) != NULL) {
        ...
        if (!(flow->info.flow_state & FLOW_STATE_PASSIVE_TIMED_OUT)) {
            double timeout_fraction = 1 -
                    (p->last_rx_time - flow->last_rx_time) / (double) delta;

            if (timeout_fraction < limit) {
    ...
    chunk_iter_unregister(tcp_chunks, &tcp_it);
    ...
    chunk_iter_unregister(udp_chunks, &udp_it);
}
...
static void timeout_flows(struct parser_base *p, struct collector_info *info) {
    ...
    uint64_t min_last_rx = UINT64_MAX;
    ...
    while ((tcp_chunk = chunk_iter_next(tcp_chunks, &tcp_it)) != NULL) {
        ...
        size_t flow_size = get_tcp_flow_size_bytes((struct tcp_flow *) flow);
        total_bytes += flow_size;
        
        uint64_t last_rx_time = flow->last_rx_time;
        if (last_rx_time < min_last_rx) {
            min_last_rx = last_rx_time;
        }
    ...
    while ((udp_chunk = chunk_iter_next(udp_chunks, &udp_it)) != NULL) {
        ...
        size_t flow_size = get_udp_flow_size_bytes((struct udp_flow *) flow);
        total_bytes += flow_size;
    ...
    if (total_bytes > p->soft_mem_limit) {
        timeout_progressively(total_bytes, p, min_last_rx);
        ERROR(p, "Will timeout-collect %d / %d flows "
                "(total %lu bytes, udp chunks %lu, tcp chunks %lu)", 
                kv_size(p->collector_list), kh_size(p->flows_table), 
                total_bytes, chunk_size_bytes(udp_chunks, &p->udp_chunks), 
                chunk_size_bytes(tcp_chunks, &p->tcp_chunks));
    }
