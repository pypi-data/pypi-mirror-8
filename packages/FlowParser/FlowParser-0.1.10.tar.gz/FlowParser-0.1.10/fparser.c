...
static PyObject *fparser_new(PyTypeObject *type, PyObject *args, PyObject *kwds) {
    ...
    int skip_win = 0;
    int skip_flags = 0;
    int soft_mem_limit_mb = 0;
    int hard_mem_limit_mb = 0;

    static char* argnames[] = {"source", "flow_timeout", "fin_timeout", "filter",
        "snaplen", "log_level", "is_file", "skip_id", "skip_ttl", "skip_ack",
        "skip_seq", "skip_win", "skip_flags", "soft_mem_limit", "hard_mem_limit",
        "tcp_callback", "udp_callback", "kill_callback", NULL};

    if (!PyArg_ParseTupleAndKeywords(args, kwds, "s|iisiiiiiiiiiiiOOO",
            argnames, &source, &flow_timeout, &fin_timeout,
            &filter, &snaplen, &log_level, &is_file, &skip_id, &skip_ttl,
            &skip_ack, &skip_seq, &skip_win, &skip_flags, &soft_mem_limit_mb, 
            &hard_mem_limit_mb, &tcp_callback, &udp_callback, 
            &after_kill_callback)) {
        return NULL;
    }
    ...
    self->parser->skip_ack = skip_ack;
    self->parser->skip_flags = skip_flags;
    self->parser->skip_id = skip_id;
    self->parser->skip_seq = skip_seq;
    self->parser->skip_ttl = skip_ttl;
    self->parser->skip_win = skip_win;
    
    if (soft_mem_limit_mb && hard_mem_limit_mb) {
        if (hard_mem_limit_mb <= soft_mem_limit_mb) {
            PyErr_SetString(FlowParserError, "Hard memory limit should be less "
                    "than soft memory limit");
            goto fail;
        }
        
        self->parser->soft_mem_limit = soft_mem_limit_mb * 1024 * 1024;
        self->parser->hard_mem_limit = hard_mem_limit_mb * 1024 * 1024;
    }
...
static PyObject *fparser_get_info(PyObject *self, PyObject *args) {
    ...
    struct parser_info info = {0};
    struct sniff_info sniff_info = {0};
