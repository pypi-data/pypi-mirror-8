...
class JobsResource(Resource):
    ...
    def _run_method(self, backend, method_name, kwargs, head, exposed):
        new_job = tools.get_method_job(method_name, kwargs, exposed)  # Validation is not required
        if new_job is None:
            raise ApiError(404, "Method not found")
        job_id = backend.jobs_control.add_jobs(head, [new_job])[0]
        return {job_id: {"method": method_name, "url": self._get_job_url(job_id)}}

    def _run_handlers(self, backend, kwargs, head, exposed):
        new_jobs = tools.get_handlers_jobs(kwargs, exposed)
        if len(new_jobs) == 0:
            return {}
        else:
            return {
                job_id: {"method": new_job.method_name, "url": self._get_job_url(job_id)}
                for (job_id, new_job) in zip(backend.jobs_control.add_jobs(head, new_jobs), new_jobs)
            }
