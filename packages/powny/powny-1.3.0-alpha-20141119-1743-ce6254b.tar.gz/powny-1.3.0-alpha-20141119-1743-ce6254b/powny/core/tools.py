...
def get_method_job(name, kwargs, exposed):
    method = exposed.get("methods", {}).get(name)
    if method is None:
        return None
    else:
        return NewJob(name, kwargs, context.dump_call(method, kwargs))


def get_handlers_jobs(kwargs, exposed):
    return [
        NewJob(name, kwargs, context.dump_call(method, kwargs))
