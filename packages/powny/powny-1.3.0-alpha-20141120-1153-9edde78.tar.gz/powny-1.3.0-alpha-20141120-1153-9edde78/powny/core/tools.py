...
def make_named_job(name, kwargs, exposed):
    method = exposed.get("methods", {}).get(name)
    if method is None:
        return None
    else:
        return FreshJob(name, kwargs, context.dump_call(method, kwargs))


def make_event_jobs(kwargs, exposed):
    return [
        FreshJob(name, kwargs, context.dump_call(method, kwargs))
