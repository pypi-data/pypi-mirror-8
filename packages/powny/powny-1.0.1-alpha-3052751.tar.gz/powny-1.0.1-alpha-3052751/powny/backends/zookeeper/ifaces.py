...
def _make_lock_info(label):
    return {
        "from":     label,
        "when":     make_isotime(),
        "instance": dict(get_instance_info()._asdict()),  # pylint: disable=protected-access
    }
...
class AppsState:
    ...
    def set_state(self, app_name, app_state):
        ...
        state = {
            "when": make_isotime(),
            "instance": dict(instance_info._asdict()),  # pylint: disable=protected-access
            "state": app_state,
        }
        self._set_raw_state(app_name, instance_info.node, state)
