...
def put(source, dest, tar=False, *args, **kwargs):a
    result = None
    if tar:
        tmp = uuid.uuid1()
    ...
    with settings(warn_only=True):
        result = api.put(tmpsourcepath, dest, *args, **kwargs)
...
def get(source, dest, tar=False, *args, **kwargs):
    ...
    with settings(warn_only=True):
        result = api.get(tmpsourcepath, dest, *args, **kwargs)

    if tar:
        tmpdestpath = "%s/%s.tar.gz" % (dest, tmp)
        untar(tmpdestpath, dest)
