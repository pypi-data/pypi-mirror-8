    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            with settings(warn_only=True):
                result = func(*args, **kwargs)
                if type(result) == dict: rtnvals = { key : value for key, value in result.items() if not keyfunc(key, value)}
...
@emit_exception(keyfunc=lambda key, value : value[0] if value is not None else value, ExceptionClass=RemoteExecutionException)
...
def get(source, dest, use_sudo=False):
    ...
    tmp = uuid.uuid1()
    tmpsourcepath = "%s/%s.tar.gz" % (os.path.dirname(source), tmp)
    tar(source, tmpsourcepath, local=False)
