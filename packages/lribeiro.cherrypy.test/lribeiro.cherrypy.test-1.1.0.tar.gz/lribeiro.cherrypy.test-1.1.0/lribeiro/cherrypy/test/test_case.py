"""
Some code as borrowed from https://bitbucket.org/Lawouach/cherrypy-recipes/src/
"""
...
from .base_test_case import PythonicTestCase
...
class CPTestCase(PythonicTestCase):
    _script_name = '/'
    ...
    def assert_contains(self, text, msg=None):
        if msg is None:
            msg = 'Response body does not contain "{0}"'.format(text)
        self.assertIn(text, self.current_response.text, msg)

    def assert_not_contains(self, text, msg=None):
        if msg is None:
            msg = 'Response body contains "{0}"'.format(text)
        self.assertNotIn(text, self.current_response.text, msg)

    def assert_body(self, text, msg=None):
        if msg is None:
            msg = 'Response body is not "{0}"'.format(text)
        self.assertEqual(text, self.current_response.text, msg)

    def assert_header(self, header, value, msg=None):
        if msg is None:
            if header not in self.current_response.headers:
                msg = 'Response does not have the header "{0}"'.format(header)
            if self.current_response.headers[header] != value:
                msg = 'Response header "{0}" is not "{1}"'.format(header, value)
    ...
    def assert_cookie(self, cookie, value, msg=None):
        if msg is None:
            if cookie not in self.current_response.cookie:
                msg = 'Response does not have the cookie "{0}"'.format(cookie)
            if self.current_response.cookie[cookie] != value:
                msg = 'Response cookie "{0}" is not "{1}"'.format(cookie, value)
    ...
    def assert_path(self, path, msg=None):
        if msg is None:
            msg = 'Current path is not "{0}"'.format(path)
