...
def make_arg_parser(cls, tt=None):
    ...
    #/ 5nLmzFh
    parser = cls(prog='aoikbtf', add_help=False)
...
"""
    ...
    if tmp_args_obj.locale_auto_on:
        ll_s.extend(I18n.get_locale_hints())
    ...
    def path_func(ll):
        path = os.path.join(
            ...
            SixyIO.cae_to_u_safe(ll) + I18n.TT_FILE_EXT_UTXT
        )
        return path
    ...
    if i18n is None:
        ...
        assert tb_utxt is not None
        
        err_typ = exc.args[0]
        
        if err_typ == I18n.LOAD_LOCALE_TT_FILE_ERR:
            path = SixyIO.cae_to_u_safe(path_func(ll))
            msg_ll = ll
        elif err_typ == I18n.LOAD_LOCALE_FALLBACK_TT_FILE_ERR:
            path = SixyIO.cae_to_u_safe(path_func(ll_fb))
            msg_ll = ll_fb
        else:
            assert False
        ...
        msg_fmt = u'Error: Failed loading i18n file for locale |{}|.\nPath is |{}|.\n'\
            .format(SixyIO.cae_to_u_safe(msg_ll), path)
        ...
        #/
        if debug_on:
            SixyIO.stderr_write_fmt_safe(u'---\n{}---\n', tb_utxt)
    ...
    def parser_tt(key, tt=tt):
        ...
        if not SixyIO.IS_PY2:
            ...
            stxt = utxt.encode(sio.stdoe, 'replace') if utxt is not None else None
    ...
    if tmp_args_obj.show_locale_info:
        ...
        sio.stderr_write_fmt_safe(u'{}\n{:<25}{}\n{:<25}{}\n{:<25}{}\n\n',
            ...
            u'L10n locale fb:',
            SixyIO.to_u_safe(i18n.locale2 if i18n.locale2 else u''),
        )
    ...
    if tmp_args_obj.show_locale_choices:
        ...
        for ll in I18n.get_locale_choices(i18n_dir):
            sio.stderr_print_safe(sio.fse_to_u_safe(ll))
        
        sio.stderr_write_safe(u'\n')
    ...
    if not output_dir_path_stxt:
        ...
        try:
            output_dir_path = sio.cae_to_u(output_dir_path_stxt)
        except Exception:
            #/ 9oBgE2N
            sio.stderr_write_fmt_safe(tt('F1HXaB3'), sio.cae_utxt)
            
            if sio.cae != SixyIO.STDIE:
                sio.stderr_print_fmt_safe(tt('Hvvw6GN'), SixyIO.STDIE_UTXT)
        ...
        if not os.path.isdir(output_dir_path):
            #/ 6rG3A5k
            sio.stderr_write_fmt_safe(tt('BwQ2oOo'), output_dir_path)
    ...
    try:
        ...
        #/ 4dKZ0v7
        sio.stderr_write_fmt_safe(tt('Bdi6LgB'), sio.cae_utxt)
        
        if sio.cae != SixyIO.STDIE:
            sio.stderr_print_fmt_safe(tt('AmypZXV'), SixyIO.STDIE_UTXT)
    ...
    try:
        ...
        #/ 6qC1rhv
        sio.stderr_write_fmt_safe(tt('E5U2Hm3'), input_file_path)
    ...
    with input_file:
        ...
        while True:
            ...
            try:
                ...
                #/ 2a4z6kA
                sio.stderr_write_fmt_safe(tt('BbCyL6V'),
                    input_file_path,
                    sio.ife_utxt,
                )
            ...
            if name_by_title:
                ...
                try:
                    ...
                    #/ 5dxuoar
                    sio.stderr_write_fmt_safe(tt('H3EQR2i'), url)
            ...
            #/ 8uZV0xm
            output_file_path = SixyIO.format(u'{}{}.{}',
                output_dir_path,
            ...
            try:
                ...
                #/ 5gc4T4s
                sio.stderr_write_fmt_safe(tt('DZ46vSV'), output_file_path)
            ...
            try:
                ...
                #/ 8nqhZvO
                sio.stderr_write_fmt_safe(tt('AfGS7Yr'), output_file_path)
