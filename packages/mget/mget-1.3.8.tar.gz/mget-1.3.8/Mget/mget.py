...
class MGetDL(FileDownloader):
	...
	def __multiple__ (self):
		currentPage = self.info.get("manga-page")
		...
		for url in self.ie_result.get('urls'):
			self.url = url
			...
			if currentPage != 1:
				common.write_string("[%s] [%s] (Chapter-%s, page %s:%s)" % \
					(self.get_time(), \
					self.info.get("series"), self.info.get("chapter"), \
					self.info.get("cur_download"),self.info.get("tot_download")))
			else:
				common.report("Downloading Series [%s] (Chapter-%s, page %s:%s)" % \
					(self.info.get("series"),self.info.get("chapter"), \
					self.info.get("cur_download"),self.info.get("tot_download")))
		...
		self.done_mult_dl(info.get('tot_download'))
		return True
	...
	def __download__(self):
		...
		except: raise DownloadError("Found downloading error's report it to MGet Developers")
