...
class MGetDL(FileDownloader):
	...
	def __downloadFile__(self, urlObj):
		...
		try:
			with open(info.get("filename"), self.mode) as fileObj:
			...
			self.quitting(info.get("filename").encode(),self.cursize,info.get('filesize'))
			exit(1)
			...
			if info.get('retryerror', False): return self.__retry__()
				...
				self.cursize = self.getLocalFilesize(info.get("filename"))
				self.quitting(info.get("filename").encode(),\
