...
#	post.add_argument('--rename', dest='rename', metavar='NAME', type=str, help="Rename file once the download is completed (100%% download)..")
#	post.add_argument('--recode-video', dest='encode-format', metavar='FORMAT', type=str, help="Encode the video to another format if necessary (currently supported: mp4|flv|ogg|webm|mkv)")
...
#	'v_format'	: opts.v_format,
	...
	for url in urls:
		info['cur_download'] += 1
		cdl = str(info.get('cur_download'))

		info['quit_size'] = 100.0 if cdl in opts.ex_videos else opts.quitsize
