...
class BaseHttpWeaver(BaseWeaver):
    ...
    def convertSizeValue(value):
        if isinstance(value, ( str, unicode )):
            if len(value) <= 0:
                ...
                return int(BaseWeaver.toSafeString(value))
    ...
    def parseProtocol(protocolLine, props=None):
        if BaseWeaver.isEmpty(protocolLine):
            return props
    ...
    def fillRequestDetails(props=None, remoteAddress=None, remoteUser=None, method="UNKNOWN", uri='', userAgent=None, requestSize=-1):
        ...
        props['flavor'] = 'http'
        BaseHttpWeaver.breakDownUri(BaseWeaver.toSafeString(uri), props)
        props['protocol'] = 'HTTP'
        props['version'] = 'unknown'
        props['method'] = BaseWeaver.toSafeString(method).upper()
        ...
        remoteAddress = BaseWeaver.toSafeString(remoteAddress)
        ...
        BaseWeaver.putIfValidString(props, 'remoteuser', BaseWeaver.toSafeString(remoteUser))
        BaseWeaver.putIfValidString(props, 'useragent', BaseWeaver.toSafeString(userAgent))
        return props
