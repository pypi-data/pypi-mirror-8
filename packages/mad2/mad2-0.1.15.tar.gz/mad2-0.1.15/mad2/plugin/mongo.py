...
def waste(app, args):
    ...
    res = db.aggregate(FIND_WASTER_PIPELINE)['result']
    for r in res:
        sha1sum = r['_id']
        cprint(sha1sum, 'red', end=' size:')
        cprint('{:12d}'.format(r['filesize']), 'cyan', end=' waste:')
        cprint('{:12.0f}'.format(r['waste'], 'yellow'))
        for f in db.find({'sha1sum': sha1sum}):
            cprint(f['username'], 'green', end='@')
            cprint(f['nlink'], 'yellow', end='@')
            cprint(f['host'], 'blue', end=':')
            cprint(f['fullpath'])
