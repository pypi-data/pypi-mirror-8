...
TODO:
improve fabric like api for more improvments
make more tests
try to improve dynamic inventory
improve logging
...
__desc__ = "Ansiblator - makes Ansible api more pythonic and usefull inside python itself"
...
class Ansiblator(object):
        ...
        if now:
            ...
            if isinstance(runner, dict):
                ...
                if return_c == 0:
                    ...
                    failed = True
                ret_code = {'contacted':{'local':{'stdout':out, 'failed':failed , 'stderr':err, 'ret_code':return_c}}, 'dark':{}}
            else:
                ret_code = runner.run()
    ...
    def runner(self, cmd="uptime", use_ansible=True, module_name="command", inventory=None, now=None, pattern=None, **kwargs):
        ...
        :param module_name: Zdenek Dlauhy, Michal Dlauhyname of ansible module
        :type module_name: str
