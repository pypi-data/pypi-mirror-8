...
#             logger.info("stream_write,begin to write the content to jss ")
#             while len(l) > 0:
#                 object_instance.upload_flow(None, l, True)
#                 l = fp.read(self.buffer_size)
#         except IOError as e :
#            logger.error("stream_write error:%s" % e)
    ...
    def stream_write(self, path, fp):
        ...
        try:
            tmp_dir = "/tmp/"+uuid.uuid1()
            tmp_file = tempfile.mktemp(suffix=path1, prefix='docker_registry_image', dir=tmp_dir)
